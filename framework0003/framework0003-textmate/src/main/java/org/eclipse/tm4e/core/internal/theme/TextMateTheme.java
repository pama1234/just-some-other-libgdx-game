...
public final class TextMateTheme{
  public static TextMateTheme createFromRawTheme(@Nullable final IRawTheme source,@Nullable final List<String> colorMap) {
    return createFromParsedTheme(parseTheme(source),colorMap);
  }
  public static TextMateTheme createFromParsedTheme(final List<ParsedThemeRule> source,@Nullable final List<String> colorMap) {
  ...
  public TextMateTheme(final ColorMap colorMap,final StyleAttributes defaults,final ThemeTrieElement root) {
  ...
  public StyleAttributes match(@Nullable final ScopeStack scopePath) {
    ...
    final List<ThemeTrieElementRule> matchingTrieElements=this._cachedMatchRoot.computeIfAbsent(scopeName,_root::match);
    // System.out.println(Arrays.toString(matchingTrieElements.toArray()));
    // System.out.println(matchingTrieElements.get(0).parentScopes);
    final ThemeTrieElementRule effectiveRule=findFirstMatching(matchingTrieElements,
      v->_scopePathMatchesParentScopes(scopePath.parent,v.parentScopes));
    if(effectiveRule==null) {
      return null;
    }
    // System.out.println(effectiveRule.foreground+" "+effectiveRule.parentScopes);
  ...
  public static TextMateTheme resolveParsedThemeRules(final List<ParsedThemeRule> _parsedThemeRules,@Nullable final List<String> _colorMap) {
    ...
    return new TextMateTheme(colorMap,defaults,root);
  }
  @Override
  public int hashCode() {
  ...
  public boolean equals(@Nullable final Object obj) {
    ...
    if(obj instanceof final TextMateTheme other) return Objects.equals(_colorMap,other._colorMap)
