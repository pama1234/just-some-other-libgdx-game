...
public final class LineTokenizer{
  ...
  private void scanNext() {
    // System.out.println(TRACE,()->"@@scanNext: |"+lineText.content.replace("\n","\\n").substring(linePos)+'|');
    ...
    if(matchedRuleId.equals(RuleId.END_RULE)) {
      ...
      if(!hasAdvanced&&popped.getEnterPos()==linePos) {
        ...
        System.out.println("[1] - Grammar is in an endless loop - Grammar pushed & popped a rule without advancing");
      ...
      if(rule instanceof final BeginEndRule pushedRule) {
        ...
        if(!hasAdvanced&&beforePush.hasSameRuleAs(stack)) {
          ...
          System.out.println("[2] - Grammar is in an endless loop - Grammar pushed the same rule without advancing");
          stack=castNonNull(stack.pop());
          lineTokens.produce(stack,lineText.content.length());
          stop=true;
          return;
        ...
        if(!hasAdvanced&&beforePush.hasSameRuleAs(stack)) {
          // Grammar pushed the same rule without advancing
          System.out.println("[3] - Grammar is in an endless loop - Grammar pushed the same rule without advancing");
          stack=castNonNull(stack.pop());
          lineTokens.produce(stack,lineText.content.length());
          stop=true;
          return;
        ...
        if(!hasAdvanced) {
          ...
          System.out.println("[4] - Grammar is in an endless loop - Grammar is not advancing, nor is it pushing/popping");
          stack=stack.safePop();
          lineTokens.produce(stack,lineText.content.length());
          stop=true;
          return;
  ...
  private MatchResult matchRuleOrInjections(final Grammar grammar,final OnigString lineText,final boolean isFirstLine,
    ...
    final MatchInjectionsResult injectionResult=matchInjections(injections,grammar,lineText,isFirstLine,linePos,stack,anchorPosition);
  ...
  private MatchInjectionsResult matchInjections(final List<Injection> injections,final Grammar grammar,final OnigString lineText,
    ...
    for(int i=0,len=injections.size();i<len;i++) {
      ...
      //   System.out.println("  matched injection: "+injection.debugSelector);
  ...
  public static TokenizeStringResult tokenizeString(final Grammar grammar,final OnigString lineText,final boolean isFirstLine,
