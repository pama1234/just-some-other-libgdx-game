  ...
  public FolderSourceList(String[] sourcePathList,boolean expectedDirectory) {
    if(sourcePathList!=null) // En el caso de shell interactivo es null
    {
      ...
      for(int i=0;i<sourcePathList.length;i++) {
        ...
        if(!folder.exists()) throw new RelProxyException("Source folder does not exist: "+folder.getAbsolutePath());
        boolean isDirectory=folder.isDirectory();
        if(expectedDirectory) {
          if(!isDirectory) throw new RelProxyException("Source folder is not a directory: "+folder.getAbsolutePath());
        }else {
          if(isDirectory) throw new RelProxyException("Expected a file not a directory: "+folder.getAbsolutePath());
        }
