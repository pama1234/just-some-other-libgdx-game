...
public class JProxyEngineChangeDetectorAndCompiler{
  ...
  private void compile(ClassDescriptorSourceUnit sourceFile,JProxyCompilerContext context) {
    if(sourceFile.getClassBytes()!=null) return; // Ya ha sido compilado seguramente por dependencia de un archivo compilado inmediatamente antes, recuerda que el atributo classBytes se pone a null antes de compilar los archivos cambiados/nuevos
  ...
  public ClassDescriptorSourceScript detectChangesInSources() {
    ...
    if(updatedSourceFiles.isEmpty()&&newSourceFiles.isEmpty()&&deletedSourceFiles.isEmpty()) return scriptFileDesc;
    ...
    synchronized(monitor) {
      if(!firstTime) {
        ...
        {
          for(ClassDescriptorSourceUnit classDesc:deletedSourceFiles) sourceRegistry.removeClassDescriptorSourceUnit(classDesc.getClassName());
        }

        if(!newSourceFiles.isEmpty()) // En firstTime ya están añadidos en sourceRegistry explícitamente al recorrer los sources
        {
          for(ClassDescriptorSourceUnit classDesc:newSourceFiles) sourceRegistry.addClassDescriptorSourceUnit(classDesc);
        }
      }
      ...
      if(!sourceFilesToCompile.isEmpty()) {
        ...
        for(ClassDescriptorSourceUnit sourceFile:sourceFilesToCompile) cleanBeforeCompile(sourceFile);
        ...
        try {
          ...
          for(ClassDescriptorSourceUnit sourceFile:sourceFilesToCompile) {
            ...
            if(compilerListener!=null) {
              ...
              if(srcUnit instanceof SourceFileJavaNormal) file=((SourceFileJavaNormal)srcUnit).getFileExt().getFile();
              else if(srcUnit instanceof SourceScriptRootFile) file=((SourceScriptRootFile)srcUnit).getFileExt().getFile();
            ...
            if(compilerListener!=null&&file!=null) compilerListener.beforeCompile(file);

            compile(sourceFile,context);

            if(compilerListener!=null&&file!=null) compilerListener.afterCompile(file);
          }
        }finally {
          context.close();
        }
      ...
      if(isSaveClassesMode()&&!deletedSourceFiles.isEmpty()) for(ClassDescriptorSourceUnit sourceFile:deletedSourceFiles) deleteClasses(sourceFile);
      ...
      if(setPendingReload) engine.setPendingReload();
    }
