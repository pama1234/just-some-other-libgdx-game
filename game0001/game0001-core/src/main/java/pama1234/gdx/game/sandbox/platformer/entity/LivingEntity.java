...
public class LivingEntity extends GamePointEntity<MassPoint>{
  ...
  public LivingEntity(Screen0011 p,WorldBase2D<? extends WorldType0001Base<?>> pw,MassPoint in,MetaCreature<?> type) {
    ...
    life=new PathVar(type.attr.maxLife);
  ...
  public void displayLife() {
    ...
    if(!flag&&UtilMath.abs(life.des-type.attr.maxLife)<=0.1f) return;
    p.beginBlend();
    p.fill(127,127);
  ...
  public float x1() {
    return x()+type.attr.dx;
  }
  public float y1() {
    return y()+type.attr.dy;
  }
  public float x2() {
    return x()+type.attr.dx+type.attr.w-0.01f;//TODO
  }
  public float y2() {
    return y()+type.attr.dy+type.attr.h-0.01f;//TODO
  }
  public int blockX1() {
  ...
  public boolean inInnerBox(float tx,float ty) {
    return Tools.inBox(tx,ty,x1(),y1(),type.attr.w,type.attr.h);
  }
}