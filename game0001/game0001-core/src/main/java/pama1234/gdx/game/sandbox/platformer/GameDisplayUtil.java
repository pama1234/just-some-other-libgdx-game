...
public class GameDisplayUtil{
  ...
  public static void debugText(Screen0011 p,Game pg) {
    ...
    debugText(p,"---- asynchronous ----");
    //以下是那三个刷新线程的调试信息，格式如下之类的：“执行所消耗的时间ms 和上一次执行相距的时间差ms”
    ...
    if(p.settings.showLog) {
      ...
      p.setTextScale(p.font.styleFast.scale);
  ...
  public static void initDebugText(Screen0011 p) {
    ...
    debugTextCountY=1;
    debugTextH=p.font.styleFast.scale*p.font.styleFast.defaultSize;
    debugTextX=ts;
  ...
  public static void drawLimitBox(Screen0011 p,int bw,int bh,MovementLimitBox limitBox) {
    ...
    rectOutline(p,1,limitBox.leftWall,limitBox.ceiling,limitBox.rightWall,limitBox.floor);
    p.fill(127,255,191,alpha);
    boxOutline(p,1,in.x()+in.type.attr.dx,in.y()+in.type.attr.dy,in.type.attr.w,in.type.attr.h);
    p.fill(94,203,234,alpha);
    rectOutline(p,1,bx1*bw,by1*bh,(bx2+1)*bw,(by2+1)*bh);
  }
  public static void rectOutline(Screen0011 p,float r,float tx1,float ty1,float tx2,float ty2) {
    ...
    tx2+=r;
    ty2+=r;
    p.rect(tx1,ty1+r,r,th1-r*2);
    p.rect(tx1,ty1,tw1-r,r);
    p.rect(tx2-r,ty1,r,th1-r);
    p.rect(tx1,ty2-r,tw1,r);
  }
  public static void boxOutline(Screen0011 p,float r,float tx1,float ty1,float tw1,float th1) {
    float tx2=tx1+tw1+r;
    float ty2=ty1+th1+r;
    tx1-=r;
    ty1-=r;
