...
public class UiGenerator extends UiGeneratorBase{
  ...
  public static <T extends Screen0011> TextButtonCam<?>[] genButtons_0010(T p,WorldBase2D<?> world,Block in) {
    return new TextButtonCam[] {
      new TextButtonCam<T>(p,self->self.text="背包显示方式："+(world.pg.world().yourself.inventory.gridUi?"阵列":"同心圆"),()->true,true)
      ...
      new TextButtonCam<T>(p,self->self.text="重置世界时间",()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
      ...
      new TextButtonCam<T>(p,self->self.text="传送",()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
          world.pg.world().yourself.point.pos.set(4134.0f,3708.0f);
        })
        .rectAuto(()->in.intData[2]-20,()->in.intData[3]+40),
      new TextButtonCam<T>(p,self->self.text="安卓版操作UI",()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
          // world.pg.world().yourself.point.pos.set(4134.0f,3708.0f);
        })
  ...
  public static <T extends Screen0011> TextButtonCam<?>[] genButtons_0009(T p,Block in) {
    ...
    return new TextButtonCam[] {
      new TextButtonCam<T>(p,self->self.text=text0001[in.intData[1]],()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
          in.intData[1]=(in.intData[1]+1)%3;
          self.updateText();
        })
        .rectAuto(()->in.intData[2]-5,()->in.intData[3]),
      new TextButtonCam<T>(p,self->self.text="-",()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
          in.intData[0]-=1;
  ...
  public static <T extends Screen0011> TextButton<?>[] genButtons_0008(T p) {
    return new TextButton[] {
      new TextButton<T>(p,self->self.text="+",()->true,false)
  ...
  @Deprecated
  public static <T extends Screen0011> Button<?>[] genButtons_return(T p,Settings ps,SettingSector ss) {
    return new Button[] {
      new TextButton<T>(p,self->self.text="返回",()->true,true)
        .allTextButtonEvent(self-> {},self-> {},self-> {
