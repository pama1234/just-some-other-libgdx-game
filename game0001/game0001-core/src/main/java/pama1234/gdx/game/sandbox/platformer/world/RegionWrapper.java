...
public class RegionWrapper{
  ...
  public void placeBlock(Block block,MetaBlock<?,?> in,int x,int y) {
    ...
    if(block.type.attr.width==1&&block.type.attr.height==1) block.doItemDrop(pw,pw.p,x,y,in.attr.empty);
  ...
  public void removeAreaIfOff(Block block,int x,int y,MetaBlock<?,?> in) {
    ...
    if(type!=null&&(type.attr.width>1||type.attr.height>1)) {
      block.doItemDrop(pw,pw.p,x,y,in.attr.empty);
      allOffAreaToAir(x,y,type);
    }
  }
  public void testAndRemoveOffBlock(MetaBlock<?,?> type,int x,int y,MetaBlock<?,?> in) {
    for(int i=0;i<type.attr.width;i++) for(int j=0;j<type.attr.height;j++) {
      int tx=x+i,
        ty=y+j;
      ...
      else if(blockOff.type.attr.width>1||blockOff.type.attr.height>1) removeAreaIfOff(blockOff,tx,ty,in);
    }
  }
  ...
  public void putIfOffBlock(Block block,int x,int y,MetaBlock<?,?> in) {
    if(in!=null&&(in.attr.width>1||in.attr.height>1)) {
      for(int i=0;i<in.attr.width;i++) for(int j=0;j<in.attr.height;j++) {
        int tx=x+i,
          ty=y+j;
        Block blockOff=pw.getBlock(tx,ty);
        testAndRemoveOffBlock(blockOff.type,tx,ty,in);
        blockOff.doItemDrop(pw,pw.p,tx,ty,in.attr.empty);
