...
public class TextArea extends TextField{
  ...
  public TextArea(String text,TextFieldStyle style,RectI rectF,GetFloat textSize) {
    super(text,style,rectF,textSize);
  }
  public TextArea(String text,TextFieldStyle style,RectI rectF,GetFloat textSize,String message) {
    this(text,style,rectF,textSize);
  ...
  protected void moveCursor(boolean forward,boolean jump) {
    ...
    if(index>=0&&index+1<linesBreak.size&&linesBreak.items[index]==cursor
      ...
      cursorLine+=count;
      if(jump) {
        super.moveCursor(forward,jump,false);
      }
      showCursor();
    }else {
      super.moveCursor(forward,jump,false);
