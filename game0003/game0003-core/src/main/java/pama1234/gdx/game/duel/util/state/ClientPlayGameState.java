...
public final class ClientPlayGameState extends ClientGameSystemState{
  ...
  public void checkCollision() {
    //TODO
    final ActorGroup myGroup=p.game().core.myGroup();
    final ActorGroup otherGroup=p.game().core.otherGroup();
  ...
  public void killPlayer(AbstractPlayerActor player) {
    p.game().core.addSquareParticles(player.pos.x,player.pos.y,50,16,2,10,4);
    player.group.removePlayer(player);
    p.game().core.screenShakeValue=50;
  }
  public void breakArrow(AbstractArrowActor arrow,ActorGroup group) {
    p.game().core.addSquareParticles(arrow.pos.x,arrow.pos.y,10,7,1,5,1);
  ...
  public void thrustPlayerActor(Actor referenceActor,ClientPlayerActor targetPlayerActor) {
    ...
    targetPlayerActor.state=p.game().core.damagedState.entryState(targetPlayerActor);
