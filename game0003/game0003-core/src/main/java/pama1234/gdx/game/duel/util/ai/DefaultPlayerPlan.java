...
public abstract class DefaultPlayerPlan extends PlayerPlan{
  ...
  public PlayerPlan nextPlan(PlayerActor player) {
    ...
    if(enemy.state.isDamaged()) {
      if(duel.random(1)<0.3f) return killPlan;
    }
    ...
    if(tmpMinDistancePow2<40000) {
      ...
      if(playerAngleInArrowFrame-nearestArrow.directionAngle>0) escapeAngle+=UtilMath.QUARTER_PI+duel.random(UtilMath.QUARTER_PI);
      ...
      if(duel.random(1)<0.7f) return movePlan;
      else return jabPlan;
    }
    // Away from enemy
    ...
    if(duel.random(1)<0.2f) return movePlan;
    else return jabPlan;
  }
  public void setMoveDirection(PlayerActor player,AbstractPlayerActor enemy) {
    float targetX,targetY;
    if(enemy.xPosition>Duel.INTERNAL_CANVAS_SIDE_LENGTH*0.5f) targetX=duel.random(0,Duel.INTERNAL_CANVAS_SIDE_LENGTH*0.5f);
    else targetX=duel.random(Duel.INTERNAL_CANVAS_SIDE_LENGTH*0.5f,Duel.INTERNAL_CANVAS_SIDE_LENGTH);
    if(enemy.yPosition>Duel.INTERNAL_CANVAS_SIDE_LENGTH*0.5f) targetY=duel.random(0,Duel.INTERNAL_CANVAS_SIDE_LENGTH*0.5f);
