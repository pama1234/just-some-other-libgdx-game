...
public final class ClientDrawLongbowPlayerActorState extends ServerDrawLongbowPlayerActorState{
  ...
  public void fire(ServerPlayerActor parentActor) {
    MusicAsset.lFire.play(p.config.data.volume);
    if(!p.online()) {
  ...
  public void displayEffect(ServerPlayerActor parentActor) {
    ...
    p.rotate(+UtilMath.HALF_PI);
    // if(!playAudio&&hasCompletedLongBowCharge(parentActor)) {
    //   MusicAsset.longShotCharged.play();
    //   playAudio=true;
    // }
  }
  @Override
  public void act(ServerPlayerActor parentActor) {
    ...
    final Particle newParticle=p.core().commonParticleSet.builder
      ...
      .build();
    p.core().commonParticleSet.particleList.add(newParticle);
    MusicAsset.longShotCharged.play(p.config.data.volume);
    // playAudio=true;
  }
}