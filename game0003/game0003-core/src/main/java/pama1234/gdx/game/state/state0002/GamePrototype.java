...
public class GamePrototype extends Game{
  ...
  public void display() {
    ...
    p.text("粒子池数量："+p.game().core.commonParticleSet.particlePool.index,p.u/2f,p.u/2f+p.bu+p.u*0.6f);
    p.text("箭池数量："+(p.game().core.myGroup().arrowCenter.list.size()+p.game().core.otherGroup().arrowCenter.list.size()),p.u/2f,p.u/2f+p.bu+p.u*1.2f);
    p.textScale(p.pus);
    super.display();
  }
  @Override
