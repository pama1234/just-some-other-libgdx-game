...
public abstract class DefaultPlayerPlan extends PlayerPlan{
  ...
  public PlayerPlan nextPlan(ServerPlayerActor player) {
    ...
    if(tmpMinDistancePow2<40000) {
      ...
      // if(rng.random(1)<0.7f) return movePlan; else
      return jabPlan();
    }
    // Away from enemy
    ...
    // if(rng.random(1)<0.2f) return movePlan; else 
    return jabPlan();
  }
  public PlayerPlan killPlan() {
    if(level>=1f) return killPlan;
      ...
      else return rng.random(1)<(level-0.5f)*2?killPlan:jabPlan();
    }
  }
  public PlayerPlan jabPlan() {
    ...
    if(level>=0.5f) return jabPlan;
      ...
      if(level<0.25f) return movePlan();
      else return rng.random(1)<(level-0.25f)*4?jabPlan:movePlan();
    }
  }
