...
public class MetaCellConfig<T extends UtilScreen>extends EntityCenter<T,Entity<T>>{
  ...
  public MetaCellConfig(T p,float x,float y,MetaCell cell,int size) {
    super(p);
    colorCtrl=new ColorController<T>(p,
    ...
    for(int i=0;i<sliders.length;i++) {
      final int fi=i;
      MetaInfo metaInfo=cell.list.get(i);
      sliders[i][0]=new Slider<T>(p,self->self.text=Tools.getFloatStringDepc(self.pos,6,3),()->true,false,metaInfo.g) {
        @Override
        public void display() {
          p.textScale(1/3f);
          super.display();
      ...
      sliders[i][1]=new Slider<T>(p,self->self.text=Tools.getFloatStringDepc(self.pos,6,1),()->true,false,metaInfo.min) {
        @Override
        public void display() {
          p.textScale(1/3f);
          super.display();
      ...
      sliders[i][2]=new Slider<T>(p,self->self.text=Tools.getFloatStringDepc(self.pos,6,1),()->true,false,metaInfo.max) {
        @Override
        public void display() {
          p.textScale(1/3f);
          super.display();
  ...
  public void display() {
    ...
    cellPanel(x(),y());
    p.textColor(0xe0);
    p.text(Tools.getIntString(cell.id+1,2)+": ",x()-textSpacing,y());
  }
  ...
  public void textWhite(float x,float y) {
    p.textColor(0x0f);
    p.text(cell.name,x-17,y);
  }
  public void textBlack(float x,float y) {
    p.textColor(0xf0);
    p.text(cell.name,x-16,y);
  }
  public float x() {
    return point.x();
  }
