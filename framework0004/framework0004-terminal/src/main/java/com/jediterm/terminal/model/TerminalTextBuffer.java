...
/**
 * Buffer for storing styled text data. Stores only text that fit into one screen XxY, but has
 * scrollBuffer to save history lines and screenBuffer to restore screen after resize.
 * ScrollBuffer stores all lines before the first line currently shown on the screen. TextBuffer
 * stores lines that are shown currently on the screen and they have there(in TextBuffer) their
 * initial length (even if it doesn't fit to screen width).
 * <p/>
 */
public class TerminalTextBuffer{
  ...
  /**
   * Returns terminal lines. Negative indexes are for history buffer. Non-negative for screen
   * buffer.
   *
  ...
  /**
   * @param scrollOrigin row where a scrolling window starts, should be in the range
