...
/**
 * Terminal that reflects obtained commands and text at {@link TerminalDisplay}(handles change
 * of cursor position, screen size etc) and {@link TerminalTextBuffer}(stores printed text)
 *
 * @author traff
 */
  ...
  public void setLinkUriStarted(@NotNull String uri) {
    ...
    if(textProcessing!=null) {
      ...
      linkResultItems.stream()
        ...
        .findFirst().ifPresent(linkItem->myStyleState.setCurrent(new HyperlinkStyle(style,linkItem.getLinkInfo())));
    }
  }
  @Override
