...
public class JediTermWidget extends JPanel implements TerminalSession,TerminalWidget,TerminalActionProvider{
  ...
  public JediTermWidget(int columns,int lines,SettingsProvider settingsProvider) {
    ...
    JediTermDebouncerImpl typeAheadDebouncer=new JediTermDebouncerImpl(myTypeAheadManager::debounce,TerminalTypeAheadManager.MAX_TERMINAL_DELAY,getExecutorServiceManager());
  ...
  /**
   * @deprecated use {@link #getTtyConnector()} to figure out if session started use
   *             {@link #getTerminal().getCodeForKey(int, int)} instead of
   *             {@link TerminalStarter#getCode(int, int)}
   */
  @Deprecated
