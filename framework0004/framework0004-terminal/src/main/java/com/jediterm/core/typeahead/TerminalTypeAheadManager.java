...
public class TerminalTypeAheadManager{
  ...
  public int getCursorX() {
    ...
    try {
      ...
      int cursorX=predictions.isEmpty()?myTerminalModel.getCurrentLineWithCursor().myCursorX:predictions.get(predictions.size()-1).myPredictedLineWithCursorX.myCursorX;
      return cursorX+1;
    }finally {
      myTerminalModel.unlock();
    }
  ...
  private @NotNull TypeAheadPrediction createPrediction(@NotNull TypeAheadTerminalModel.LineWithCursorX initialLineWithCursorX,
    ...
    switch(keyEvent.myEventType) {
      ...
      case RightArrow:
        ...
        if(newLineWCursorX.myCursorX<0||newLineWCursorX.myCursorX>=Math.max(newLineWCursorX.myLineText.length()+1,myTerminalModel.getTerminalWidth())) {
          return new HardBoundary();
        }
