...
public final class KTerminalRenderer implements Disposable{
  ...
  private final TextureRegion[] getGlyphs() {
    ...
    for(int var4=this.rows;i<var4;++i) {
      ...
      for(int var6=this.columns;j<var6;++j) {
        // TextureRegion var10002 = new TextureRegion(var10004, j * this.glyphWidth, i * this.glyphHeight, this.glyphWidth, this.glyphHeight);
  ...
  public final void render(final float x,final float y,@NotNull final KTerminalData kTerminalData) {
    ...
    var $fun$draw$1=new WithObjectFunction() {
      // $FF: synthetic method
      // $FF: bridge method
    ...
    for(var7=kTerminalData.getHeight();j<var7;++j) {
      ...
      for(var9=kTerminalData.getWidth();i<var9;++i) {
        ...
        float scaleX=glyph.isFlippedX()?-glyph.getScale():glyph.getScale();
        TextureRegion var10001;
        Texture var10003=this.backgroundTexture;
        if(var10003==null) {
          // Intrinsics.throwUninitializedPropertyAccessException("backgroundTexture");
    ...
    for(var7=kTerminalData.getHeight();j<var7;++j) {
      ...
      for(var9=kTerminalData.getWidth();i<var9;++i) {
        ...
        if(glyph.getValue()>=this.columns*this.rows) {
          throw new IllegalArgumentException("glyph value ["+glyph.getValue()+"] exceeds found glyph count ["+(this.columns*this.rows-1)+']');
        }
        TextureRegion[] var21;
  ...
  // $FF: synthetic method
  public KTerminalRenderer(SpriteBatch var1,String var2,int var3,int var4,float var5,int var6) {
    if((var6&2)!=0) {
      var2=null;
    }
    if((var6&4)!=0) {
