...
/**
 * A circular buffer of {@link TerminalRow}:s which keeps notes about what is visible on a
 * logical screen and the scroll history.
 * <p>
 * See {@link #externalToInternalRow(int)} for how to map from logical screen rows to array
 * indices.
 */
public final class TerminalBuffer{
  ...
  public void setChar(int column,int row,int codePoint,long style) {
    if(row<0||row>=mScreenRows||column<0||column>=mColumns) throw new IllegalArgumentException("TerminalBuffer.setChar(): row="+row+", column="+column+", mScreenRows="+mScreenRows+", mColumns="+mColumns);
