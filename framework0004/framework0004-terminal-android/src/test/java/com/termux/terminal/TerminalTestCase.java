...
public abstract class TerminalTestCase extends TestCase{
  ...
  protected void assertLineIs(int line,String expected) {
    ...
    if(textLen!=expected.length()) fail("Expected '"+expected+"' (len="+expected.length()+"), was='"+new String(chars,0,textLen)+"' (len="+textLen+")");
    for(int i=0;i<textLen;i++) {
      if(expected.charAt(i)!=chars[i]) fail("Expected '"+expected+"', was='"+new String(chars,0,textLen)+"' - first different at index="+i);
    }
  }
  ...
  public TerminalTestCase assertEffectAttributesSet(EffectLine... lines) {
    ...
    for(int i=0;i<lines.length;i++) {
      ...
      for(int j=0;j<line.length;j++) {
        ...
        if((effectsAtCell&attributes)!=attributes) fail("Line="+i+", column="+j+", expected "+describeStyle(attributes)+" set, was "+describeStyle(effectsAtCell));
      }
    }
    return this;
  }
  public TerminalTestCase assertForegroundIndices(EffectLine... lines) {
    ...
    for(int i=0;i<lines.length;i++) {
      ...
      for(int j=0;j<line.length;j++) {
        ...
        if(actualColor!=expectedColor) fail("Line="+i+", column="+j+", expected color "+Integer.toHexString(expectedColor)+" set, was "+Integer.toHexString(actualColor));
      }
    }
    return this;
  }
