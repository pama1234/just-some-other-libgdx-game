...
/**
 * This class holds a 3D model composed of vertices, normals, colors (per vertex) and texture
 * coordinates (also per vertex). All this data is stored in Vertex Buffer Objects (VBO) in GPU
 * memory for very fast access. OBJ loading implemented using code from Saito's OBJLoader
 * library: http://code.google.com/p/saitoobjloader/ and OBJReader from Ahmet Kizilay
...
public class PShapeOpenGL extends PShape{
  ...
  /*
   * static public PShapeOpenGL createShape2D(PGraphicsOpenGL pg, PShape src) { PShapeOpenGL dest
   * = null; if (src.getFamily() == GROUP) { //dest = PGraphics2D.createShapeImpl(pg, GROUP); dest
   * = (PShapeOpenGL) pg.createShapeFamily(GROUP); copyGroup2D(pg, src, dest); } else if
   * (src.getFamily() == PRIMITIVE) { //dest = PGraphics2D.createShapeImpl(pg, src.getKind(),
   * src.getParams()); dest = (PShapeOpenGL) pg.createShapePrimitive(src.getKind(),
   * src.getParams()); PShape.copyPrimitive(src, dest); } else if (src.getFamily() == GEOMETRY) {
   * //dest = PGraphics2D.createShapeImpl(pg, PShape.GEOMETRY); dest = (PShapeOpenGL)
   * pg.createShapeFamily(PShape.GEOMETRY); PShape.copyGeometry(src, dest); } else if
   * (src.getFamily() == PATH) { //dest = PGraphics2D.createShapeImpl(pg, PATH); dest =
   * (PShapeOpenGL) pg.createShapeFamily(PShape.PATH); PShape.copyPath(src, dest); }
   * dest.setName(src.getName()); dest.width = src.width; dest.height = src.height; return dest; }
   */
  ...
  /*
   * static public void copyGroup2D(PGraphicsOpenGL pg, PShape src, PShape dest) { copyMatrix(src,
   * dest); copyStyles(src, dest); copyImage(src, dest);
   * 
   * for (int i = 0; i < src.getChildCount(); i++) { PShape c = createShape2D(pg,
   * src.getChild(i)); dest.addChild(c); } }
   */
  ///////////////////////////////////////////////////////////
  //
  // Query methods
  ...
  public PVector getAttribPosition(String name,int index,PVector vec) {
    VertexAttribute attrib=polyAttribs.get(name);
    if(attrib==null) throw new RuntimeException("Trying to get values of non existing attribute");
    if(vec==null) vec=new PVector();
    if(root.tessUpdate) {
  ...
  public void setAttribPosition(String name,int index,float x,float y,float z) {
    ...
    VertexAttribute attrib=attribImpl(name,VertexAttribute.POSITION,PGL.FLOAT,3);
    if(attrib==null) throw new RuntimeException("Trying to set values of non existing attribute");
    if(root.tessUpdate) {
      float[] tessAttrib=tessGeo.fpolyAttribs.get(name);
      int tessIdx=firstPolyVertex+index;
  ...
  public void setAttrib(String name,int index,float... values) {
    ...
    if(attrib==null) throw new RuntimeException("Trying to set values of non existing attribute");
    if(root.tessUpdate) {
      if(attrib.tessSize!=values.length) throw new RuntimeException("Length of values array is different from attribute tesselated size");
      float[] tessAttrib=tessGeo.fpolyAttribs.get(name);
      int tessIdx=firstPolyVertex+index;
      PApplet.arrayCopy(values,0,tessAttrib,attrib.tessSize*tessIdx,attrib.tessSize);
    }else {
      if(attrib.size!=values.length) throw new RuntimeException("Length of values array is different from attribute size");
      float[] array=inGeo.fattribs.get(name);
          ...
          if(0<size) { // round point
            ...
            perim=PApplet.min(PGraphicsOpenGL.MAX_POINT_ACCURACY,
              PApplet.max(PGraphicsOpenGL.MIN_POINT_ACCURACY,
                (int)(TWO_PI*weight/
                  PGraphicsOpenGL.POINT_ACCURACY_FACTOR)))
              +1;
          }else { // Square point
            weight=-size/0.5f;
            perim=5;
    ...
    if(family==PATH||family==GEOMETRY) {
      ...
      for(int i=0,j=inGeo.vertexCount-1;i<inGeo.vertexCount;j=i++) {
        if(((inGeo.vertices[3*i+1]>y)!=(inGeo.vertices[3*j+1]>y))&&
          (x<(inGeo.vertices[3*j]-inGeo.vertices[3*i])*
  ...
  protected void post(PGraphics g) {
    if(g instanceof PGraphicsOpenGL) {}else {
      super.post(g);
    }
  }
  @Override
