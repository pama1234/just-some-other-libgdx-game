 ...
 * 
 * Copyright (c) 2012-23 The Processing Foundation Copyright (c) 2004-12 Ben Fry and Casey Reas
...
import processing.opengl.element.CursorInfo;
import processing.opengl.newt.NEWTKeyListener;
import processing.opengl.newt.NEWTMouseListener;
import processing.opengl.newt.NEWTWindowListener;
...
public class PSurfaceJOGL implements PSurface{
  ...
  Object getSyncMutex(GLAutoDrawable drawable) {
    pgl.getGL(drawable);
  ...
  protected void initWindow() {
    ...
    windowScaleFactor=(PApplet.platform==PConstants.MACOS)?1:sketch.pixelDensity;
    ...
    float surfaceScale=(graphics.is2X()&&PApplet.platform==PConstants.MACOS)?ScalableSurface.AUTOMAX_PIXELSCALE:ScalableSurface.IDENTITY_PIXELSCALE;
  ...
  protected void initListeners() {
    window.addMouseListener(new NEWTMouseListener(this));
    window.addKeyListener(new NEWTKeyListener(this));
    window.addWindowListener(new NEWTWindowListener(this));
    window.addGLEventListener(new DrawListener(this));
  }
  ...
  /*
   * public void setupExternalMessages() { external = true; }
   */
  ...
  public void nativeMouseEvent(com.jogamp.newt.event.MouseEvent nativeEvent,
    int peAction) {
    ...
    /*
     * int peModifiers = modifiers & (InputEvent.SHIFT_MASK | InputEvent.CTRL_MASK |
  ...
  public void nativeKeyEvent(com.jogamp.newt.event.KeyEvent nativeEvent,
    int peAction) {
    ...
    if(cursor==null) {
      ...
      if(name!=null) {
        ...
        if(url!=null) {
          ...
          cursor=new CursorInfo(this,img,x,y);
  ...
  public void setCursor(PImage image,int hotspotX,int hotspotY) {
    ...
    int[] imagePixels=((DataBufferInt)img.getData().getDataBuffer()).getData();
    ...
    PixelRectangle rect=new PixelRectangle.GenericPixelRect(format,size,0,false,pixels);
