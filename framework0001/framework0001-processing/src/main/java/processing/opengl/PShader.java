...
/**
 * This class encapsulates a GLSL shader program, including a vertex and a fragment shader. It
 * is compatible with P2D and P3D, but not with the default renderer.
 *
 * Use the <b>loadShader()</b> function to load your shader code. Note: It's strongly encouraged
 * to use <b>loadShader()</b> to create a <b>PShader</b> object, rather than calling the
 * <b>PShader</b> constructor manually.
 *
 * @webref rendering:shaders
 * @webBrief This class encapsulates a GLSL shader program, including a vertex and a fragment
 *           shader
 */
public class PShader implements PConstants{
  ...
  /**
   * Sets the uniform variables inside the shader to modify the effect while the program is
   * running.
   *
   * @webref rendering:shaders
  ...
  /**
   * @param y second component of the variable to modify. The variable has to be declared with an
   *          array/vector type in the shader (i.e.: int[2], vec2)
   */
  public void set(String name,int x,int y) {
    setUniformImpl(name,UniformValue.INT2,new int[] {x,y});
  }
  /**
   * @param z third component of the variable to modify. The variable has to be declared with an
   *          array/vector type in the shader (i.e.: int[3], vec3)
   */
  public void set(String name,int x,int y,int z) {
    setUniformImpl(name,UniformValue.INT3,new int[] {x,y,z});
  }
  /**
   * @param w fourth component of the variable to modify. The variable has to be declared with an
   *          array/vector type in the shader (i.e.: int[4], vec4)
   */
  ...
  /**
   * @param vec modifies all the components of an array/vector uniform variable. PVector can only
   *            be used if the type of the variable is vec3.
   */
  ...
  /**
   * Extra initialization method that can be used by subclasses, called after compiling and
   * attaching the vertex and fragment shaders, and before linking the shader program.
   *
   */
      ...
      for(String name:uniformValues.keySet()) {
        ...
        if(loc==-1) {
          PGraphics.showWarning("The shader doesn't have a uniform called \""+
            name+
            "\" OR the uniform was removed during "+
