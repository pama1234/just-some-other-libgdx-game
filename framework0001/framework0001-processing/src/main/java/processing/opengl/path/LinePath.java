...
/*
 * Copyright 2006 Sun Microsystems, Inc. All Rights Reserved. DO NOT ALTER OR REMOVE COPYRIGHT
 * NOTICES OR THIS FILE HEADER.
 *
 * This code is free software; you can redistribute it and/or modify it under the terms of the
 * GNU General Public License version 2 only, as published by the Free Software Foundation. Sun
 * designates this particular file as subject to the "Classpath" exception as provided by Sun in
 * the LICENSE file that accompanied this code.
 *
 * This code is distributed in the hope that it will be useful, but WITHOUT ANY WARRANTY;
 * without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See
 * the GNU General Public License version 2 for more details (a copy is included in the LICENSE
 * file that accompanied this code).
 *
 * You should have received a copy of the GNU General Public License version 2 along with this
 * work; if not, write to the Free Software Foundation, Inc., 51 Franklin St, Fifth Floor,
 * Boston, MA 02110-1301 USA.
 *
 * Please contact Sun Microsystems, Inc., 4150 Network Circle, Santa Clara, CA 95054 USA or
 * visit www.sun.com if you need additional information or have any questions.
 */
package processing.opengl.path;

import processing.core.PMatrix2D;
import processing.opengl.LineStroker;
import processing.opengl.PathIterator;

/**
 * The {@code LinePath} class allows to represent polygonal paths, potentially composed by
 * several disjoint polygonal segments. It can be iterated by the {@link PathIterator} class
 * including all of its segment types and winding rules
 *
 */
public class LinePath{
  ...
  public float[] floatCoords;
  public int[] pointColors;
  public int numTypes;
  protected int numCoords;
  protected int windingRule;
  /**
   * Constructs a new empty single precision {@code LinePath} object with a default winding rule
   * of {@link #WIND_NON_ZERO}.
   */
  public LinePath() {
    this(WIND_NON_ZERO,INIT_SIZE);
  }
  /**
   * Constructs a new empty single precision {@code LinePath} object with the specified winding
  ...
  /**
   * Constructs a new {@code LinePath} object from the given specified initial values. This method
   * is only intended for internal use and should not be made public if the other constructors for
   * this class are ever exposed.
   *
   * @param rule            the winding rule
   * @param initialCapacity the size to make the initial array to store the path segment types
   */
   ...
   * method on the base {@code LinePath} class.
   *
  ...
  /**
   * The iterator for this class is not multi-threaded safe, which means that the {@code LinePath}
   * class does not guarantee that modifications to the geometry of this {@code LinePath} object
  ...
  /**
   * Resets the path to empty. The append position is set back to the beginning of the path and
  ...
  public static float[] copyOf(float[] source,int length) {
    ...
    for(int i=0;i<target.length;i++) {
      if(i>source.length-1) target[i]=0f;
  ...
  public static byte[] copyOf(byte[] source,int length) {
    ...
    for(int i=0;i<target.length;i++) {
      if(i>source.length-1) target[i]=0;
      else target[i]=source[i];
    }
    return target;
  }
  ...
  public static int FloatToS15_16(float flt) {
    flt=flt*65536f+0.5f;
