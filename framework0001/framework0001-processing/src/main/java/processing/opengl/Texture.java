...
public class Texture implements PConstants{
  ...
  /**
   * Bilinear sampling: both magnification filtering is set to linear and minification either to
   * linear-mipmap-nearest (linear interpolation is used within a mipmap, but not between
   * different mipmaps).
   */
  protected static final int BILINEAR=4;
  /**
   * Trilinear sampling: magnification filtering set to linear, minification to
   * linear-mipmap-linear, which offers the best mipmap quality since linear interpolation to
  ...
  protected LinkedList<TextureBufferData> bufferCache=null;
  protected LinkedList<TextureBufferData> usedBuffers=null;
  ...
  /**
   * Creates an instance of PTexture with size width x height. The texture is initialized (empty)
   * to that size.
   * 
   * @param width  int
   * @param height int
  ...
  public Texture(PGraphicsOpenGL pg,int width,int height) {
    this(pg,width,height,new TextureParameters());
  }
  /**
  ...
  public Texture(PGraphicsOpenGL pg,int width,int height,Object params) {
    ...
    colorBuffer=false;
    glName=0;
    init(width,height,(TextureParameters)params);
  }
  ////////////////////////////////////////////////////////////
  // Init, resize methods
  /**
   * Sets the size of the image and texture to width x height. If the texture is already
   * initialized, it first destroys the current OpenGL texture object and then creates a new one
  ...
  /**
   * Sets the size of the image and texture to width x height, and the parameters of the texture
   * to params. If the texture is already initialized, it first destroys the current OpenGL
   * texture object and then creates a new one with the specified size.
   * 
   * @param width  int
   * @param height int
   * @param params GLTextureParameters
   */
  public void init(int width,int height,TextureParameters params) {
  ...
  /**
   * Returns true or false whether or not the texture is using repeat wrap mode along either U or
   * V directions.
   * 
   * @return boolean
   */
  ...
  public void copyBufferFromSource(Object natRef,ByteBuffer byteBuf,
    ...
    if(bufferCache.size()+1<=MAX_BUFFER_CACHE_SIZE) {
      bufferCache.add(new TextureBufferData(this,natRef,byteBuf.asIntBuffer(),w,h));
    }else {
      // The buffer cache reached the maximum size, so we just dispose
      // the new buffer by adding it to the list of used buffers.
      if(usedBuffers==null) {
        usedBuffers=new LinkedList<TextureBufferData>();
      }
      usedBuffers.add(new TextureBufferData(this,natRef,byteBuf.asIntBuffer(),w,h));
    }
  }
  public void disposeSourceBuffer() {
    if(usedBuffers==null) return;
    while(0<usedBuffers.size()) {
      TextureBufferData data=null;
      try {
  ...
  protected void getSourceMethods() {
    try {
      disposeBufferMethod=bufferSource.getClass().getMethod("disposeBuffer",Object.class);
    }catch(Exception e) {
  ...
  /**
   * Reorders a pixel array in the given format into the order required by OpenGL (RGBA) and
   * stores it into rgbaPixels. The width and height parameters are used in the YUV420 to RBGBA
   * conversion.
   * 
   * @param pixels int[]
   * @param format int
  ...
  public TextureParameters getParameters() {
    TextureParameters res=new TextureParameters();
   ...
   * @param params GLTextureParameters
   */
  protected void setParameters(TextureParameters params) {
