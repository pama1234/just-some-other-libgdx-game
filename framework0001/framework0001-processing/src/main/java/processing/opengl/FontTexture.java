...
/**
 * All the infrastructure needed for optimized font rendering in OpenGL. Basically, this special
 * class is needed because fonts in Processing are handled by a separate PImage for each glyph.
 * For performance reasons, all these glyphs should be stored in a single OpenGL texture
 * (otherwise, rendering a string of text would involve binding and un-binding several textures.
 * PFontTexture manages the correspondence between individual glyphs and the large OpenGL
 * texture containing them. Also, in the case that the font size is very large, one single
 * OpenGL texture might not be enough to store all the glyphs, so PFontTexture also takes care
 * of spreading a single font over several textures.
 * 
...
class FontTexture implements PConstants{
  ...
  public boolean addTexture(PGraphicsOpenGL pg) {
    ...
    if(is3D) {
      ...
      tex=new Texture(pg,w,h,
        new TextureParameters(ARGB,Texture.BILINEAR,false));
    }else {
      ...
      tex=new Texture(pg,w,h,
        new TextureParameters(ARGB,Texture.LINEAR,false));
    }
    if(textures==null) {
  ...
  protected void addToTexture(PGraphicsOpenGL pg,int idx,PFont.Glyph glyph) {
    ...
    TextureInfo tinfo=new TextureInfo(this,lastTex,offsetX,offsetY,w,h,rgba);
    offsetX+=w;
