...
/**
 * Helper class for a list of <b>long</b> values. Lists are designed to have some features of
 * <b>ArrayList</b>, but to maintain the simplicity and efficiency of working with arrays.
 * Functions such as <b>sort()</b> and <b>shuffle()</b> always act on the list itself. To get a
 * sorted copy, use <b>list.copy().sort()</b>.
 *
 * @nowebref
 * @see IntList
 * @see StringList
...
public class LongList implements Iterable<Long>{
  ...
  /**
   * Construct an IntList from an iterable pile of objects. For instance, a float array, an array
   * of strings, who knows. Un-parsable or null values will be set to 0.
   * 
   * @nowebref
   */
  @SuppressWarnings("unused")
  ...
  /**
   * Construct an IntList from a random pile of objects. Un-parsable or null values will be set to
   * zero.
   */
  @SuppressWarnings("unused")
  ...
  /**
   * Set the entry at a particular index. If the index is past the length of the list, it'll
   * expand the list to accommodate, and fill the intermediate entries with 0s.
   *
   * @webref intlist:method
   ...
   * <b>randomSeed()</b> function in PApplet.
   *
   * @webref intlist:method
  ...
  /**
   * Returns the actual array being used to store the data. For advanced users, this is the
   * fastest way to access a large list. Suitable for iterating with a for() loop, but modifying
   * the list will have terrible consequences.
   */
  public long[] values() {
    crop();
    return data;
  ...
  /**
   * Copy values into the specified array. If the specified array is null or not the same size, a
   ...
   * classes. It's a get method because it needs to return a new list (because IntList/Dict can't
   * do percentages or normalization in place on int values).
   */
  @SuppressWarnings("unused")
