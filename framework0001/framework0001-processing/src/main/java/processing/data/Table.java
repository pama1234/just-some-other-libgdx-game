 ...
 * 
 * Copyright (c) 2011-13 Ben Fry and Casey Reas Copyright (c) 2006-11 Ben Fry
 * 
 * This library is free software; you can redistribute it and/or modify it under the terms of
 * the GNU Lesser General Public License version 2.1 as published by the Free Software
 * Foundation.
 * 
...
/**
 * <b>Table</b> objects store data with multiple rows and columns, much like in a traditional
 * spreadsheet. Tables can be generated from scratch, dynamically, or using data from an
 * existing file. Tables can also be output and saved to disk, as in the example above.<br />
 * <br />
 ...
 * <h3>Advanced</h3>
 * <p>
 * Generic class for handling tabular data, typically from a CSV, TSV, or other sort of
 * spreadsheet file.
 * </p>
 * <p>
 * CSV files are <a href="http://en.wikipedia.org/wiki/Comma-separated_values">comma separated
 * values</a>, often with the data in quotes. TSV files use tabs as separators, and usually
 ...
 * </p>
 * <p>
 * A rough "spec" for CSV can be found <a href="http://tools.ietf.org/html/rfc4180">here</a>.
 * </p>
 *
 * @webref data:composite
 * @webBrief Generic class for handling tabular data, typically from a CSV, TSV, or other sort
 *           of spreadsheet file
...
public class Table{
  ...
  /**
   * version that uses a File object; future releases (or data types) may include additional
   * optimizations here
   *
   * @nowebref
   */
  ...
  /*
   * protected String checkOptions(File file, String options) throws IOException { String
   * extension = null; String filename = file.getName(); int dotIndex = filename.lastIndexOf('.');
   * if (dotIndex != -1) { extension = filename.substring(dotIndex + 1).toLowerCase(); if
   * (!extension.equals("csv") && !extension.equals("tsv") && !extension.equals("html") &&
   * !extension.equals("bin")) { // ignore extension extension = null; } } if (extension == null)
   * { if (options == null) { throw new
   * IOException("This table filename has no extension, and no options are set."); } } else { //
   * extension is not null if (options == null) { options = extension; } else { // prepend the
   * extension, it will be overridden if there's an option for it. options = extension + "," +
  ...
  protected void parse(InputStream input,String options) throws IOException {
    ...
    if(binary) {
      ...
      /*
       * if (awfulCSV) { parseAwfulCSV(reader, header); } else if ("tsv".equals(extension)) {
       * parseBasic(reader, header, true); } else if ("csv".equals(extension)) { parseBasic(reader,
       * header, false); }
       */
  ...
  protected void parseBasic(BufferedReader reader,
    ...
    try {
      while((line=reader.readLine())!=null) {
        ...
        if(row%10000==0) {
          /*
           * // this is problematic unless we're going to calculate rowCount first if (row < rowCount) {
           * int pct = (100 * row) / rowCount; if (pct != prev) { // also prevents "0%" from showing up
           * System.out.println(pct + "%"); prev = pct; } }
           */
          try {
   ...
   * char[] c = new char[100]; int count = 0; boolean insideQuote = false;
   * 
   * int alloc = 100; setRowCount(100);
   * 
   * int row = 0; int col = 0; int ch; while ((ch = reader.read()) != -1) { if (insideQuote) { if
   * (ch == '\"') { // this is either the end of a quoted entry, or a quote character
   * reader.mark(1); if (reader.read() == '\"') { // it's "", which means a quote character if
   * (count == c.length) { c = PApplet.expand(c); } c[count++] = '\"'; } else { // nope, just the
   * end of a quoted csv entry reader.reset(); insideQuote = false; // TODO nothing here that
   * prevents bad csv data from showing up // after the quote and before the comma... // set(row,
   * col, new String(c, 0, count)); // count = 0; // col++; // insideQuote = false; } } else { //
   * inside a quote, but the character isn't a quote if (count == c.length) { c =
   * PApplet.expand(c); } c[count++] = (char) ch; } } else { // not inside a quote if (ch == '\"')
   * { insideQuote = true;
   * 
   * } else if (ch == '\r' || ch == '\n') { if (ch == '\r') { // check to see if next is a '\n'
   * reader.mark(1); if (reader.read() != '\n') { reader.reset(); } } setString(row, col, new
   * String(c, 0, count)); count = 0; row++; if (row == 1 && header) { // Use internal row removal
   * (efficient because only one row). removeTitleRow(); // Un-set the header variable so that
   * next time around, we don't // just get stuck into a loop, removing the 0th row repeatedly.
   * header = false; // Reset the number of rows (removeTitleRow() won't reset our local 'row'
   * counter) row = 0; } // if (row % 1000 == 0) { // PApplet.println(PApplet.nfc(row)); // } if
   * (row == alloc) { alloc *= 2; setRowCount(alloc); } col = 0;
   * 
   * } else if (ch == ',') { setString(row, col, new String(c, 0, count)); count = 0; // starting
   * a new column, make sure we have room col++; ensureColumn(col);
   * 
   * } else { // just a regular character, add it if (count == c.length) { c = PApplet.expand(c);
   * } c[count++] = (char) ch; } } } // catch any leftovers if (count > 0) { setString(row, col,
   * new String(c, 0, count)); } row++; // set row to row count (the current row index + 1) if
   * (alloc != row) { setRowCount(row); // shrink to the actual size } }
   */
   ...
   */
  /*
   * static protected int nextComma(char[] c, int index) { if (index == c.length) { // we're
   * already at the end return c.length; } boolean quoted = c[index] == '\"'; if (quoted) {
   * index++; // step over the quote } for (int i = index; i < c.length; i++) { if (c[i] == '\"')
   * { // if this fella started with a quote if (quoted) { if (i == c.length-1) { //return -1; //
   * ran out of chars // closing quote for field; last field on the line return c.length; } else
   * if (c[i+1] == '\"') { // an escaped quote inside a quoted field, step over it i++; } else if
   * (c[i+1] == ',') { // that's our closing quote, get outta here return i+1; }
   * 
   * } else { // not a quoted line if (i == c.length-1) { // we're at the end of the line, can't
   * have an unescaped quote //return -1; // ran out of chars throw new
   * RuntimeException("Unterminated quoted field at end of line"); } else if (c[i+1] == '\"') { //
   * step over this crummy quote escape ++i; } else { throw new
   * RuntimeException("Unterminated quoted field mid-line"); } } } else if (!quoted && c[i] ==
   * ',') { return i; } if (!quote && (c[i] == ',')) { // found a comma, return this location
   * return i; } else if (c[i] == '\"') { // if it's a quote, then either the next char is another
   * quote, // or if this is a quoted entry, it better be a comma quote = !quote; } } // if still
   * inside a quote, indicate that another line should be read if (quote) { return -1; } // made
   * it to the end of the array with no new comma return c.length; }
   */
  /**
   * Read a .ods (OpenDoc spreadsheet) zip file from an InputStream, and return the InputStream
   * for content.xml contained inside.
   */
    ...
    try {
      ...
      if(!found) {
        if(worksheet==null) {
          ...
          throw new RuntimeException("No worksheet named "+
            worksheet+
  ...
  protected void saveODS(OutputStream os) throws IOException {
    ...
    zos.closeEntry();
    /*
     * entry = new ZipEntry("meta.xml"); lines = new String[] { xmlHeader,
     * "<office:document-meta office:version=\"1.0\"" +
     * " xmlns:office=\"urn:oasis:names:tc:opendocument:xmlns:office:1.0\" />" };
     * zos.putNextEntry(entry); zos.write(PApplet.join(lines, "\n").getBytes()); zos.closeEntry();
     * 
     * entry = new ZipEntry("meta.xml"); lines = new String[] { xmlHeader,
     ...
     * " xmlns:xlink=\"http://www.w3.org/1999/xlink\" />" }; zos.putNextEntry(entry);
     * zos.write(PApplet.join(lines, "\n").getBytes()); zos.closeEntry();
     * 
     * entry = new ZipEntry("settings.xml"); lines = new String[] { xmlHeader,
     * "<office:document-settings office:version=\"1.0\"" +
     * " xmlns:config=\"urn:oasis:names:tc:opendocument:xmlns:config:1.0\"" +
     * " xmlns:office=\"urn:oasis:names:tc:opendocument:xmlns:office:1.0\"" +
     * " xmlns:ooo=\"http://openoffice.org/2004/office\"" +
     ...
     * zos.write(PApplet.join(lines, "\n").getBytes()); zos.closeEntry();
     * 
     * entry = new ZipEntry("styles.xml"); lines = new String[] { xmlHeader,
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Use <b>addColumn()</b> to add a new column to a <b>Table</b> object. Typically, you will want
   * to specify a title, so the column may be easily referenced later by name. (If no title is
   * specified, the new column's title will be <b>null</b>.) A column type may also be specified,
   * in which case all values stored in this column must be of the same type (e.g.,
   * <b>Table.INT</b> or <b>Table.FLOAT</b>). If no type is specified, the default type of
   * <b>STRING</b> is used.
   *
   * @webref table:method
  ...
  /**
   * Use <b>removeColumn()</b> to remove an existing column from a <b>Table</b> object. The column
   * to be removed may be identified by either its title (a <b>String</b>) or its index value (an
   * <b>int</b>). <b>removeColumn(0)</b> would remove the first column, <b>removeColumn(1)</b>
  ...
  /**
   * Removes all rows from a <b>Table</b>. While all rows are removed, columns and column titles
   * are maintained.
   *
   * @webref table:method
  ...
  /**
   * Use <b>addRow()</b> to add a new row of data to a <b>Table</b> object. By default, an empty
   * row is created. Typically, you would store a reference to the new row in a <b>TableRow</b>
   * object (see <b>newRow</b> in the example above), and then set individual values using
   * <b>setInt()</b>, <b>setFloat()</b>, or <b>setString()</b>. If a <b>TableRow</b> object is
   ...
   * pieces.length; col++) { setRowCol(row, col, pieces[col]); } }
   * 
   * 
   * protected void setRowCol(int row, int col, String piece) { switch (columnTypes[col]) { case
   * STRING: String[] stringData = (String[]) columns[col]; stringData[row] = piece; break; case
   * INT: int[] intData = (int[]) columns[col]; intData[row] = PApplet.parseInt(piece,
   * missingInt); break; case LONG: long[] longData = (long[]) columns[col]; try { longData[row] =
   * Long.parseLong(piece); } catch (NumberFormatException nfe) { longData[row] = missingLong; }
   * break; case FLOAT: float[] floatData = (float[]) columns[col]; floatData[row] =
   * PApplet.parseFloat(piece, missingFloat); break; case DOUBLE: double[] doubleData = (double[])
   * columns[col]; try { doubleData[row] = Double.parseDouble(piece); } catch
   * (NumberFormatException nfe) { doubleData[row] = missingDouble; } break; case CATEGORY: int[]
   * indexData = (int[]) columns[col]; indexData[row] = columnCategories[col].index(piece); break;
   * default: throw new IllegalArgumentException("That's not a valid column type."); } }
   */
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Returns a reference to the specified <b>TableRow</b>. The reference can then be used to get
   * and set values of the selected row, as illustrated in the example above.
   *
   * @webref table:method
  ...
  /**
   * Gets all rows from the table. Returns an iterator, so <b>for</b> must be used to iterate
   * through all the rows, as shown in the example above.
   *
   * @webref table:method
  ...
  /*
   * static public Iterator<TableRow> createIterator(final ResultSet rs) { return new
   * Iterator<TableRow>() { boolean already;
   * 
   * public boolean hasNext() { already = true; try { return rs.next(); } catch (SQLException e) {
   * throw new RuntimeException(e); } }
   * 
   * 
   * public TableRow next() { if (!already) { try { rs.next(); } catch (SQLException e) { throw
   * new RuntimeException(e); } } else { already = false; }
   * 
   * return new TableRow() { public double getDouble(int column) { try { return
   * rs.getDouble(column); } catch (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public double getDouble(String columnName) { try { return rs.getDouble(columnName); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public float getFloat(int column) { try { return rs.getFloat(column); } catch (SQLException
   * e) { throw new RuntimeException(e); } }
   * 
   * public float getFloat(String columnName) { try { return rs.getFloat(columnName); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public int getInt(int column) { try { return rs.getInt(column); } catch (SQLException e) {
   * throw new RuntimeException(e); } }
   * 
   * public int getInt(String columnName) { try { return rs.getInt(columnName); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public long getLong(int column) { try { return rs.getLong(column); } catch (SQLException e) {
   * throw new RuntimeException(e); } }
   * 
   * public long getLong(String columnName) { try { return rs.getLong(columnName); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public String getString(int column) { try { return rs.getString(column); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public String getString(String columnName) { try { return rs.getString(columnName); } catch
   * (SQLException e) { throw new RuntimeException(e); } }
   * 
   * public void setString(int column, String value) { immutable(); } public void setString(String
   * columnName, String value) { immutable(); } public void setInt(int column, int value) {
   * immutable(); } public void setInt(String columnName, int value) { immutable(); } public void
   * setLong(int column, long value) { immutable(); } public void setLong(String columnName, long
   * value) { immutable(); } public void setFloat(int column, float value) { immutable(); } public
   * void setFloat(String columnName, float value) { immutable(); } public void setDouble(int
   * column, double value) { immutable(); } public void setDouble(String columnName, double value)
   * { immutable(); }
   * 
   * private void immutable() { throw new
   * IllegalArgumentException("This TableRow cannot be modified."); }
   * 
   * public int getColumnCount() { try { return rs.getMetaData().getColumnCount(); } catch
   * (SQLException e) { e.printStackTrace(); return -1; } }
   * 
   * 
   * public int getColumnType(String columnName) { // unimplemented }
   * 
   * 
   * public int getColumnType(int column) { // unimplemented }
   * 
   * }; }
   * 
   * public void remove() { throw new IllegalArgumentException("remove() not supported"); } }; }
   */
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Retrieves an integer value from the <b>Table</b>'s specified row and column. The row is
   * specified by its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Stores an integer value in the <b>Table</b>'s specified row and column. The row is specified
   * by its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Retrieves a float value from the <b>Table</b>'s specified row and column. The row is
   * specified by its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Stores a float value in the <b>Table</b>'s specified row and column. The row is specified by
   * its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Retrieves a String value from the <b>Table</b>'s specified row and column. The row is
   * specified by its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Stores a String value in the <b>Table</b>'s specified row and column. The row is specified by
   * its ID, while the column may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Retrieves all values in the specified column, and returns them as a <b>String</b> array. The
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Finds the first row in the <b>Table</b> that contains the value provided, and returns a
   * reference to that row. Even if multiple rows are possible matches, only the first matching
   * row is returned. The column to search may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Finds the rows in the <b>Table</b> that contain the value provided, and returns references to
   * those rows. Returns an iterator, so <b>for</b> must be used to iterate through all the rows,
   * as shown in the example above. The column to search may be specified by either its ID or
   * title.
   *
   * @webref table:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Finds the first row in the <b>Table</b> that matches the regular expression provided, and
   * returns a reference to that row. Even if multiple rows are possible matches, only the first
   * matching row is returned. The column to search may be specified by either its ID or title.
   *
   * @webref table:method
  ...
  /**
   * Finds the rows in the <b>Table</b> that match the regular expression provided, and returns
   * references to those rows. Returns an iterator, so <b>for</b> must be used to iterate through
   * all the rows, as shown in the example above. The column to search may be specified by either
   * its ID or title.
   *
   * @webref table:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Removes any of the specified characters (or "tokens"). The example above removes all commas,
   * dollar signs, and spaces from the table.<br />
   * <br />
   * If no column is specified, then the values in all columns and rows are processed. A specific
   * column may be referenced by either its ID or title.
   *
   * @webref table:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Trims leading and trailing whitespace, such as spaces and tabs, from String table values. If
   * no column is specified, then the values in all columns and rows are trimmed. A specific
   * column may be referenced by either its ID or title.
   *
   * @webref table:method
  ...
  /*
   * protected boolean isEmptyColumn(int column) { String[] contents = getStringColumn(column);
   * for (String entry : contents) { if (entry != null && entry.length() > 0) { return false; } }
   * return true; }
   * 
   * 
   * protected boolean isEmptyRow(int row) { String[] contents = getStringRow(row); for (String
   * entry : contents) { if (entry != null && entry.length() > 0) { return false; } } return true;
   * }
   */
  /**
