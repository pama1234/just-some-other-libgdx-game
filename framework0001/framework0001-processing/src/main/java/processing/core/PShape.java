 ...
 * <b>loadShape()</b> or created with the <b>createShape()</b>. The <b>shape()</b> function is
 * used to draw the shape to the display window. Processing can currently load and display SVG
 * (Scalable Vector Graphics) and OBJ shapes. OBJ files can only be opened using the <b>P3D</b>
 * renderer. The <b>loadShape()</b> function supports SVG files created with Inkscape and Adobe
 * Illustrator. It is not a full SVG implementation, but offers some straightforward support for
 * handling vector data. <br />
 * <br />
 * The <b>PShape</b> object contains a group of methods that can operate on the shape data. Some
 * of the methods are listed below, but the full list used for creating and modifying shapes is
 * <a href="http://processing.github.io/processing-javadocs/core/">available here in the
 * Processing Javadoc</a>.<br />
 * <br />
 * To create a new shape, use the <b>createShape()</b> function. Do not use the syntax <b>new
 * PShape()</b>.
 *
 * <h3>Advanced</h3>
 *
 * In-progress class to handle shape data, currently to be considered of alpha or beta quality.
 * Major structural work may be performed on this class after the release of Processing 1.0.
 * Such changes may include:
 *
 * <ul>
 * <li>addition of proper accessors to read shape vertex and coloring data (this is the second
 * most important part of having a PShape class after all).
 ...
 * <li>change of package declaration, for instance moving the code into package processing.shape
 * (if the code grows too much).
 * </ul>
 *
 * <p>
 ...
 *
 * <p>
 * Library developers are encouraged to create PShape objects when loading shape data, so that
 * they can eventually hook into the bounty that will be the PShape interface, and the ease of
 * loadShape() and shape().
 * </p>
 *
 * @webref shape
...
public class PShape implements PConstants{
  ...
  static public final String NO_VERTICES_ERROR="getVertexCount() only works with PATH or GEOMETRY shapes";
  public static final String NOT_A_SIMPLE_VERTEX="%1$s can not be called on quadratic or bezier vertices";
  static public final String PER_VERTEX_UNSUPPORTED="This renderer does not support %1$s for individual vertices";
  /**
   *
  ...
  /**
   *
   * Returns a boolean value <b>true</b> if the image is set to be visible, <b>false</b> if not.
   * This value can be modified with the <b>setVisible()</b> method.<br />
   * <br />
   * The default visibility of a shape is usually controlled by whatever program created the SVG
   * file. For instance, this parameter is controlled by showing or hiding the shape in the layers
   * palette in Adobe Illustrator.
   *
   * @webref pshape:method
   * @usage web_application
   * @webBrief Returns a boolean value <b>true</b> if the image is set to be visible, <b>false</b>
   *           if not
  ...
  /**
   *
   * Sets the shape to be visible or invisible. This is determined by the value of the
   * <b>visible</b> parameter.<br />
   * <br />
   * The default visibility of a shape is usually controlled by whatever program created the SVG
   * file. For instance, this parameter is controlled by showing or hiding the shape in the layers
   * palette in Adobe Illustrator.
  ...
  /**
   *
   * Disables the shape's style data and uses Processing's current styles. Styles include
   * attributes such as colors, stroke weight, and stroke joints.
   *
   * <h3>Advanced</h3> Overrides this shape's style information and uses PGraphics styles and
   * colors. Identical to ignoreStyles(true). Also disables styles for all child shapes.
   * 
   * @webref pshape:method
   * @usage web_application
  ...
  /**
   *
   * Enables the shape's style data and ignores Processing's current styles. Styles include
   * attributes such as colors, stroke weight, and stroke joints.
   *
   *
   * @webref pshape:method
  ...
  /*
   * // TODO unapproved protected PVector getTop() { return getTop(null); }
   * 
   * 
   * protected PVector getTop(PVector top) { if (top == null) { top = new PVector(); } return top;
   * }
   * 
   * 
   * protected PVector getBottom() { return getBottom(null); }
   * 
   * 
   * protected PVector getBottom(PVector bottom) { if (bottom == null) { bottom = new PVector(); }
   * return bottom; }
   */
  /**
  ...
  /**
   * The <b>beginContour()</b> and <b>endContour()</b> methods make it possible to define shapes
   * with other shapes cut out of them. For example, the inside of a letter 'O'. These two
   * functions are always used together, you'll never use one without the other. Between them,
   * define the geometry you want to create. As you'll see when you run the example above, the
   * second smaller shape is cut out of the first larger shape.<br />
   * <br />
   * The exterior shape and the interior contour must <em>wind</em> in opposite directions. This
   * means that if the points of the geometry for the exterior shape are described in a clockwise
   * order, the points on the interior shape are defined in a counterclockwise order.
   *
   * @webref shape:vertex
  ...
  /**
   * This method is used to start a custom shape created with the <b>createShape()</b> function.
   * It's always and only used with <b>createShape()</b>.
   *
   * @webref pshape:method
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /*
   * boolean strokeSaved; int strokeColorSaved; float strokeWeightSaved; int strokeCapSaved; int
   * strokeJoinSaved;
   * 
  ...
  protected void pre(PGraphics g) {
    ...
    /*
     * strokeSaved = g.stroke; strokeColorSaved = g.strokeColor; strokeWeightSaved = g.strokeWeight;
     * strokeCapSaved = g.strokeCap; strokeJoinSaved = g.strokeJoin;
     * 
  ...
  protected void post(PGraphics g) {
     ...
     * strokeCapSaved; g.strokeJoin = strokeJoinSaved;
     * 
   ...
   * if (j > 0) g.breakShape(); int count = children[j].vertexCount; float[][] vert =
   * children[j].vertices; int[] code = children[j].vertexCodes;
   * 
   * for (int i = 0; i < count; i++) { if (style) { if (children[j].fill) { g.fill(vert[i][R],
   * vert[i][G], vert[i][B]); } else { g.noFill(); } if (children[j].stroke) {
   * g.stroke(vert[i][R], vert[i][G], vert[i][B]); } else { g.noStroke(); } } g.edge(vert[i][EDGE]
   * == 1);
   * 
   * if (code[i] == VERTEX) { g.vertex(vert[i]);
   * 
   * } else if (code[i] == BEZIER_VERTEX) { float z0 = vert[i+0][Z]; float z1 = vert[i+1][Z];
   * float z2 = vert[i+2][Z]; if (z0 == 0 && z1 == 0 && z2 == 0) { g.bezierVertex(vert[i+0][X],
   ...
   * vert[i+2][Y]); } } else if (code[i] == CURVE_VERTEX) { float z = vert[i][Z]; if (z == 0) {
  ...
  /**
   *
   * Extracts a child shape from a parent shape. Specify the name of the shape with the
   * <b>target</b> parameter. The shape is returned as a <b>PShape</b> object, or <b>null</b> is
  ...
  /**
   * Adds a child PShape to a parent PShape that is defined as a GROUP. In the example, the three
   * shapes <b>path</b>, <b>rectangle</b>, and <b>circle</b> are added to a parent PShape variable
   * named <b>house</b> that is a GROUP.
   * 
   * @webref pshape:method
  ...
  /**
   * Returns a PShape holding the tessellated geometry of this shape, composed entirely of
   * triangles.
   */
  ...
  /**
   * The <b>getVertexCount()</b> method returns the number of vertices that make up a
   * <b>PShape</b>. In the above example, the value 4 is returned by the <b>getVertexCount()</b>
   * method because 4 vertices are defined in <b>setup()</b>.
   *
   * @webref pshape:method
  ...
  /**
   * The <b>getVertex()</b> method returns a <b>PVector</b> with the coordinates of the vertex
   * point located at the position defined by the <b>index</b> parameter. This method works when
   * shapes are created as shown in the example above, but won't work properly when a shape is
   * defined explicitly (e.g. <b>createShape(RECT, 20, 20, 80, 80)</b>.
   *
   * @webref pshape:method
  ...
  /**
   * The <b>setVertex()</b> method defines the coordinates of the vertex point located at the
   * position defined by the <b>index</b> parameter. This method works when shapes are created as
   * shown in the example above, but won't work properly when a shape is defined explicitly (e.g.
   * <b>createShape(RECT, 20, 20, 80, 80)</b>.
   *
   * @webref pshape:method
  ...
  /**
   *
   * The <b>setFill()</b> method defines the fill color of a <b>PShape</b>. This method is used
   * after shapes are created or when a shape is defined explicitly (e.g. <b>createShape(RECT, 20,
   * 20, 80, 80)</b>) as shown in the above example. When a shape is created with
   * <b>beginShape()</b> and <b>endShape()</b>, its attributes may be changed with <b>fill()</b>
   * and <b>stroke()</b> within <b>beginShape()</b> and <b>endShape()</b>. However, after the
   * shape is created, only the <b>setFill()</b> method can define a new fill value for the
   * <b>PShape</b>.
   *
   *
   * @webref
  ...
  /**
   *
   * The <b>setStroke()</b> method defines the outline color of a <b>PShape</b>. This method is
   * used after shapes are created or when a shape is defined explicitly (e.g.
   * <b>createShape(RECT, 20, 20, 80, 80)</b>) as shown in the above example. When a shape is
   * created with <b>beginShape()</b> and <b>endShape()</b>, its attributes may be changed with
   * <b>fill()</b> and <b>stroke()</b> within <b>beginShape()</b> and <b>endShape()</b>. However,
   * after the shape is created, only the <b>setStroke()</b> method can define a new stroke value
   * for the <b>PShape</b>.
   *
   *
   * @webref
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Return true if this x, y coordinate is part of this shape. Only works with PATH shapes or
   * GROUP shapes that contain other GROUPs or PATHs. This method is not imperfect and doesn't
   * account for all cases (not all complex shapes: concave shapes or holes may have issues).
   */
  ...
  /**
   *
   * Specifies an amount to displace the shape. The <b>x</b> parameter specifies left/right
   * translation, the <b>y</b> parameter specifies up/down translation, and the <b>z</b> parameter
   * specifies translations toward/away from the screen. Subsequent calls to the method
   * accumulates the effect. For example, calling <b>translate(50, 0)</b> and then
   * <b>translate(20, 0)</b> is the same as <b>translate(70, 0)</b>. This transformation is
   * applied directly to the shape, it's not refreshed each time <b>draw()</b> is run. <br />
   * <br />
   ...
   * should be specified in radians (values from 0 to TWO_PI) or converted to radians with the
   * <b>radians()</b> method. <br />
   * <br />
   ...
   * <b>size()</b> function as shown in the example above.
   *
   * @webref pshape:method
   * @usage web_application
  ...
  /**
   *
   * Increases or decreases the size of a shape by expanding and contracting vertices. Shapes
   * always scale from the relative origin of their bounding box. Scale values are specified as
   * decimal percentages. For example, the method call <b>scale(2.0)</b> increases the dimension
   * of a shape by 200%. Subsequent calls to the method multiply the effect. For example, calling
   * <b>scale(2.0)</b> and then <b>scale(1.5)</b> is the same as <b>scale(3.0)</b>. This
   * transformation is applied directly to the shape, it's not refreshed each time <b>draw()</b>
   * is run. <br />
   * <br />
   * Using this method with the <b>z</b> parameter requires using the P3D parameter in combination
  ...
  /**
   *
   * Replaces the current matrix of a shape with the identity matrix. The equivalent function in
   * OpenGL is <b>glLoadIdentity()</b>.
   *
   * @webref pshape:method
  ...
  //
  /**
   * Make sure that the shape's matrix is 1) not null, and 2) has a matrix that can handle <em>at
   * least</em> the specified number of dimensions.
   */
