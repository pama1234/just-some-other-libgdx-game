...
/**
 * 3x2 affine matrix implementation. Matrices are used to describe a transformation; see
 * {@link PMatrix} for a general description. This matrix looks like the following when
 * multiplying a vector (x, y) in {@code mult()}.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * (x', y') is returned. The values in the matrix determine the transformation. They are
...
public class PMatrix2D implements PMatrix{
  ...
  /**
   * Copies the matrix contents into a 6 entry float array. If target is null (or not the correct
   * size), a new array will be created. Returned in the order {@code {m00, m01, m02, m10, m11,
   * m12}}.
   */
   ...
   * float array will be returned. The values for vec and out can be the same (though that's less
   * efficient).
   */
