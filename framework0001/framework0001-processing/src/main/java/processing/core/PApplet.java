 ...
 * 
 * Copyright (c) 2012-22 The Processing Foundation Copyright (c) 2004-12 Ben Fry and Casey Reas
 ...
 * Window Size and Full Screen</A> page on the Wiki has useful information about sizing,
 * multiple displays, full screen, etc.
 * <p/>
 * Processing uses active mode rendering. All animation tasks happen on the "Processing
 * Animation Thread". The setup() and draw() methods are handled by that thread, and events
 * (like mouse movement and key presses, which are fired by the event dispatch thread or EDT)
 * are queued to be safely handled at the end of draw().
 * <p/>
 * Starting with 3.0a6, blit operations are on the EDT, so as not to cause GUI problems with
 * Swing and AWT. In the case of the default renderer, the sketch renders to an offscreen image,
 * then the EDT is asked to bring that image to the screen.
 * <p/>
 * For code that needs to run on the EDT, use EventQueue.invokeLater(). When doing so, be
 * careful to synchronize between that code and the Processing animation thread. That is, you
 * can't call Processing methods from the EDT or at any random time from another thread. Use of
 * a callback function or the registerXxx() methods in PApplet can help ensure that your code
 * doesn't do something naughty.
 * <p/>
 * As of Processing 3.0, we have removed Applet as the base class for PApplet. This means that
 * we can remove lots of legacy code, however one downside is that it's no longer possible
 * (without extra code) to embed a PApplet into another Java application.
 * <p/>
 * As of Processing 3.0, we have discontinued support for versions of Java prior to 1.8. We
 * don't have enough people to support it, and for a project of our (tiny) size, we should be
...
public class PApplet implements PConstants{
  ...
  /**
   * Do not use; javaPlatform or javaVersionName are better options. For instance, javaPlatform is
   * useful when you need a number for comparison, i.e. "if (javaPlatform >= 9)".
   */
  @Deprecated
  ...
  public String[] args;
  /**
   * Path to sketch folder. Previously undocumented, and made private in 3.0 alpha 5 so that
   * people use the sketchPath() method which will initialize it properly. Call sketchPath() once
   * to set it.
   */
  ...
  /**
   * The <b>settings()</b> function is new with Processing 3.0. It's not needed in most sketches.
   * It's only useful when it's absolutely necessary to define the parameters to <b>size()</b>
   * with a variable. Alternately, the <b>settings()</b> function is necessary when using
   * Processing code outside the Processing Development Environment (PDE). For example, when using
   * the Eclipse code editor, it's necessary to use <b>settings()</b> to define the <b>size()</b>
   * and <b>smooth()</b> values for a sketch.</b>. <br />
   * <br />
   * The <b>settings()</b> method runs before the sketch has been set up, so other Processing
   * functions cannot be used at that point. For instance, do not use loadImage() inside
   * settings(). The settings() method runs "passively" to set a few variables, compared to the
   * <b>setup()</b> command that call commands in the Processing API.
   *
   * @webref environment
   * @webBrief Used when absolutely necessary to define the parameters to <b>size()</b> with a
   *           variable
  ...
  /**
   *
   * This function returns the number "2" if the screen is a high-density screen (called a Retina
   * display on OS X or high-dpi on Windows and Linux) and a "1" if not. This information is
   * useful for a program to adapt to run at double the pixel density on a screen that supports
   * it.
   *
   * @webref environment
  ...
  /**
   * @param display the display number to check (1-indexed to match the Preferences dialog box)
   */
  public int displayDensity(int display) {
    ...
    /*
     * if (display > 0 && display <= displayDevices.length) { GraphicsConfiguration graphicsConfig =
     * displayDevices[display - 1].getDefaultConfiguration(); AffineTransform tx =
     * graphicsConfig.getDefaultTransform(); return (int) Math.round(tx.getScaleX()); }
     * 
  ...
  /**
   * This function makes it possible to render using all the pixels on high resolutions screens
   * like Apple Retina and Windows HiDPI. This function can only be run once within a program, and
   * must be called right after <b>size()</b> in a program without a <b>setup()</b> function, or
   * within <b>setup()</b> if present.
   * <p/>
   * <b>pixelDensity()</b> should only be used with hardcoded numbers (in almost all cases this
   * number will be 2) or in combination with <b>displayDensity()</b> as in the third example
   * above.
   * <p/>
   * When the pixel density is set to more than 1, it changes the pixel operations including the
   * way <b>get()</b>, <b>set()</b>, <b>blend()</b>, <b>copy()</b>, and <b>updatePixels()</b> all
   * work. See the reference for <b>pixelWidth</b> and pixelHeight for more information.
   * <p/>
   * To use variables as the arguments to <b>pixelDensity()</b> function, place the
   * <b>pixelDensity()</b> function within the <b>settings()</b> function. There is more
   * information about this on the <b>settings()</b> reference page.
   *
   * @webref environment
   * @webBrief It makes it possible for Processing to render using all the pixels on high
   *           resolutions screens
  ...
  /**
   * Called by PSurface objects to set the width and height variables, and update the pixelWidth
   * and pixelHeight variables.
   */
   ...
   * <b>smooth()</b> only needs to be used when a program needs to set the smoothing in a
   * different way. The level parameter increases the amount of smoothness. This is the level of
   * over sampling applied to the graphics buffer.
   * <p/>
   * With the P2D and P3D renderers, <b>smooth(2)</b> is the default, this is called "2x
   * anti-aliasing." The code <b>smooth(4)</b> is used for 4x anti-aliasing and <b>smooth(8)</b>
   * is specified for "8x anti-aliasing." The maximum anti-aliasing level is determined by the
   * hardware of the machine that is running the software, so <b>smooth(4)</b> and
   * <b>smooth(8)</b> will not work with every computer.
   * <p/>
   * The default renderer uses <b>smooth(3)</b> by default. This is bicubic smoothing. The other
   * option for the default renderer is <b>smooth(2)</b>, which is bilinear smoothing.
   * <p/>
   * With Processing 3.0, <b>smooth()</b> is handled differently than in earlier releases. In 2.x
   * and earlier, it was possible to use <b>smooth()</b> and <b>noSmooth()</b> to turn on and off
   * antialiasing within a sketch. Now, because of how the software has changed, <b>smooth()</b>
   * can only be set once within a sketch. It can be used either at the top of a sketch without a
   * <b>setup()</b>, or after the <b>size()</b> function when used in a sketch with
   * <b>setup()</b>. The <b>noSmooth()</b> function also follows the same rules.
   * <p/>
   * When <b>smooth()</b> is used with a PGraphics object, it should be run right after the object
   * is created with <b>createGraphics()</b>, as shown in the Reference in the third example.
   *
   * @webref environment
  ...
  /**
   * Draws all geometry and fonts with jagged (aliased) edges and images with hard edges between
   * the pixels when enlarged rather than interpolating pixels. Note that <b>smooth()</b> is
   * active by default, so it is necessary to call <b>noSmooth()</b> to disable smoothing of
   * geometry, fonts, and images. Since the release of Processing 3.0, the <b>noSmooth()</b>
   * function can only be run once for each sketch, either at the top of a sketch without a
   * <b>setup()</b>, or after the <b>size()</b> function when used in a sketch with
   * <b>setup()</b>. See the examples above for both scenarios.
   *
   * @webref environment
   * @webBrief Draws all geometry and fonts with jagged (aliased) edges and images with hard edges
   *           between the pixels when enlarged rather than interpolating pixels
   */
  ...
  /**
   * Sketch has been paused. Called when switching tabs in a browser or swapping to a different
   * application on Android. Also called just before quitting. Use to safely disable things like
   * serial, sound, or sensors.
   */
  public void pause() {}
  /**
   * Sketch has resumed. Called when switching tabs in a browser or swapping to this application
   * on Android. Also called on startup. Use this to safely disable things like serial, sound, or
   * sensors.
   */
  public void resume() {}
  //////////////////////////////////////////////////////////////
  ...
  class RegisteredMethods{
    /**
     * List of the objects for which the method is registered. This is an ordered collection because
     * the order of calls likely matters, or at a minimum, needs to be stable.
     */
    Queue<Object> entries=new ConcurrentLinkedQueue<>();
    /**
     * A reference to the Method inside each Object, stored so that we're not redoing the same
     * reflection call inside a tight loop like draw().
     */
    ...
    void add(Object object,Method method) {
      if(!entries.contains(object)) {
        ...
        die(method.getName()+
          "() already added for this instance of "+
          object.getClass().getName());
      }
    }
    /**
     * Removes first object/method pair matched (and only the first, must be called multiple times
     * if object is registered multiple times).
     */
  ...
  /**
   * Register a built-in event so that it can be fired for libraries, etc. Supported events
   * include:
   * <ul>
   ...
   * <ul>
   * In addition, the new (for 2.0) <tt>processing.event</tt> classes are passed to the following
   * event types:
   * <ul>
   * <li>mouseEvent
   * <li>keyEvent
   * <li>touchEvent
   * </ul>
   * The older java.awt events are no longer supported. See the Library Wiki page for more
   * details.
   * 
  ...
  /**
   *
   * The <b>setup()</b> function is run once, when the program starts. It's used to define initial
   * environment properties such as screen size and to load media such as images and fonts as the
   * program starts. There can only be one <b>setup()</b> function for each program, and it
   * shouldn't be called again after its initial execution.<br />
   * <br />
   * If the sketch is a different dimension than the default, the <b>size()</b> function or
   * <b>fullScreen()</b> function must be the first line in <b>setup()</b>.<br />
   * <br />
   * Note: Variables declared within <b>setup()</b> are not accessible within other functions,
   * including <b>draw()</b>.
   *
   * @webref structure
  ...
  /**
   *
   * Called directly after <b>setup()</b>, the <b>draw()</b> function continuously executes the
   * lines of code contained inside its block until the program is stopped or <b>noLoop()</b> is
   * called. <b>draw()</b> is called automatically and should never be called explicitly. All
   * Processing programs update the screen at the end of draw(), never earlier.<br />
   * <br />
   * To stop the code inside of <b>draw()</b> from running continuously, use <b>noLoop()</b>,
   * <b>redraw()</b> and <b>loop()</b>. If <b>noLoop()</b> is used to stop the code in
   * <b>draw()</b> from running, then <b>redraw()</b> will cause the code inside <b>draw()</b> to
   * run a single time, and <b>loop()</b> will cause the code inside <b>draw()</b> to resume
   * running continuously.<br />
   * <br />
   * The number of times <b>draw()</b> executes in each second may be controlled with the
   * <b>frameRate()</b> function.<br />
   * <br />
   * It is common to call <b>background()</b> near the beginning of the <b>draw()</b> loop to
   * clear the contents of the window, as shown in the first example above. Since pixels drawn to
   * the window are cumulative, omitting <b>background()</b> may result in unintended
   * results.<br />
   * <br />
   * There can only be one <b>draw()</b> function for each sketch, and <b>draw()</b> must exist if
   * you want the code to run continuously, or to process events such as <b>mousePressed()</b>.
   * Sometimes, you might have an empty call to <b>draw()</b> in your program, as shown in the
   * second example above.
   *
   * @webref structure
   * @webBrief Called directly after <b>setup()</b> and continuously executes the lines of code
   *           contained inside its block until the program is stopped or <b>noLoop()</b> is
   *           called
   * @usage web_application
   * @see PApplet#setup()
   * @see PApplet#loop()
   ...
   * debug("resizeRenderer request for " + newWidth + " " + newHeight); if (width != newWidth ||
   * height != newHeight) { debug("  former size was " + width + " " + height);
   * g.setSize(newWidth, newHeight); width = newWidth; height = newHeight; } }
   */
  /**
  ...
  /**
   * This function is new for Processing 3.0. It opens a sketch using the full size of the
   * computer's display. This function must be the first line in <b>setup()</b>. The <b>size()</b>
   * and <b>fullScreen()</b> functions cannot both be used in the same program, just choose
   * one.<br />
   * <br />
   * When <b>fullScreen()</b> is used without a parameter, it draws the sketch to the screen
   * currently selected inside the Preferences window. When it is used with a single parameter,
   * this number defines the screen to display to program on (e.g. 1, 2, 3...). When used with two
   * parameters, the first defines the renderer to use (e.g. P2D) and the second defines the
   * screen. The <b>SPAN</b> parameter can be used in place of a screen number to draw the sketch
   * as a full-screen window across all the attached displays if there are more than one.<br />
   * <br />
   * Prior to Processing 3.0, a full-screen program was defined with <b>size(displayWidth,
   * displayHeight)</b>.
   *
   * @webref environment
  ...
  /**
   * @param display the screen to run the sketch on (1, 2, 3, etc. or on multiple screens using
   *                SPAN)
   */
  ...
  /**
   * Defines the dimension of the display window width and height in units of pixels. In a program
   * that has the <b>setup()</b> function, the <b>size()</b> function must be the first line of
   * code inside <b>setup()</b>, and the <b>setup()</b> function must appear in the code tab with
   * the same name as your sketch folder.<br />
   * <br />
   * The built-in variables <b>width</b> and <b>height</b> are set by the parameters passed to
   * this function. For example, running <b>size(640, 480)</b> will assign 640 to the <b>width</b>
   * variable and 480 to the height <b>variable</b>. If <b>size()</b> is not used, the window will
   * be given a default size of 100 x 100 pixels.<br />
   * <br />
   * The <b>size()</b> function can only be used once inside a sketch, and it cannot be used for
   * resizing. Use <b>windowResize()</b> instead.<br />
   * <br />
   * To run a sketch that fills the screen, use the <b>fullScreen()</b> function, rather than
   * using <b>size(displayWidth, displayHeight)</b>.<br />
   * <br />
   * The <b>renderer</b> parameter selects which rendering engine to use. For example, if you will
   * be drawing 3D shapes, use <b>P3D</b>. The default renderer is slower for some situations (for
   * instance large or high-resolution displays) but generally has higher quality than the other
   * renderers for 2D drawing. <br />
   * <br />
   * In addition to the default renderer, other renderers are:<br />
   * <br />
   * <b>P2D</b> (Processing 2D): 2D graphics renderer that makes use of OpenGL-compatible graphics
   * hardware.<br />
   * <br />
   * <b>P3D</b> (Processing 3D): 3D graphics renderer that makes use of OpenGL-compatible graphics
   * hardware.<br />
   * <br />
   * <b>FX2D</b> (JavaFX 2D): A 2D renderer that uses JavaFX, which may be faster for some
   * applications, but has some compatibility quirks. Use “Manage Libraries” to download and
   * install the JavaFX library.<br />
   * <br />
   * <b>PDF</b>: The PDF renderer draws 2D graphics directly to an Acrobat PDF file. This produces
   * excellent results when you need vector shapes for high-resolution output or printing. You
   * must first use Import Library &rarr; PDF to make use of the library. More information can be
   * found in the PDF library reference.<br />
   * <br />
   * <b>SVG</b>: The SVG renderer draws 2D graphics directly to an SVG file. This is great for
   * importing into other vector programs or using for digital fabrication. It is not as
   * feature-complete as other renderers. Like PDF, you must first use Import Library &rarr; SVG
   * Export to make use the SVG library.<br />
   * <br />
   * As of Processing 3.0, to use variables as the parameters to <b>size()</b> function, place the
   * <b>size()</b> function within the <b>settings()</b> function (instead of <b>setup()</b>).
   * There is more information about this on the <b>settings()</b> reference page.<br />
   * <br />
   * The maximum width and height is limited by your operating system, and is usually the width
   * and height of your actual screen. On some machines it may simply be the number of pixels on
   * your current screen, meaning that a screen of 800 x 600 could support <b>size(1600, 300)</b>,
   * since that is the same number of pixels. This varies widely, so you'll have to try different
   * rendering modes and sizes until you get what you're looking for. If you need something
   * larger, use <b>createGraphics</b> to create a non-visible drawing surface.<br />
   * <br />
   * The minimum width and height is around 100 pixels in each direction. This is the smallest
   * that is supported across Windows, macOS, and Linux. We enforce the minimum size so that
   * sketches will run identically on different machines. <br />
   * <br />
   *
   * @webref environment
  ...
  /**
   *
   * Creates and returns a new <b>PGraphics</b> object. Use this class if you need to draw into an
   * offscreen graphics buffer. The first two parameters define the width and height in pixels.
   * The third, optional parameter specifies the renderer. It can be defined as P2D, P3D, PDF, or
   * SVG. If the third parameter isn't used, the default renderer is set. The PDF and SVG
   * renderers require the filename parameter.<br />
   * <br />
   * It's important to consider the renderer used with <b>createGraphics()</b> in relation to the
   * main renderer specified in <b>size()</b>. For example, it's only possible to use P2D or P3D
   * with <b>createGraphics()</b> when one of them is defined in <b>size()</b>. Unlike Processing
   * 1.0, P2D and P3D use OpenGL for drawing, and when using an OpenGL renderer it's necessary for
   * the main drawing surface to be OpenGL-based. If P2D or P3D are used as the renderer in
   * <b>size()</b>, then any of the options can be used with <b>createGraphics()</b>. If the
   * default renderer is used in <b>size()</b>, then only the default, PDF, or SVG can be used
   * with <b>createGraphics()</b>.<br />
   * <br />
   * It's important to run all drawing functions between the <b>beginDraw()</b> and
   * <b>endDraw()</b>. As the exception to this rule, <b>smooth()</b> should be run on the
   * PGraphics object before <b>beginDraw()</b>. See the reference for <b>smooth()</b> for more
   * detail.<br />
   * <br />
   * The <b>createGraphics()</b> function should almost never be used inside <b>draw()</b> because
   * of the memory and time needed to set up the graphics. One-time or occasional use during
   * <b>draw()</b> might be acceptable, but code that calls <b>createGraphics()</b> at 60 frames
   * per second might run out of memory or freeze your sketch.<br />
   * <br />
   * Unlike the main drawing surface which is completely opaque, surfaces created with
   * <b>createGraphics()</b> can have transparency. This makes it possible to draw into a graphics
   * and maintain the alpha channel. By using <b>save()</b> to write a PNG or TGA file, the
   * transparency of the graphics object will be honored.
   *
   * <h3>Advanced</h3> Create an offscreen PGraphics object for drawing. This can be used for
   * bitmap or vector images drawing or rendering.
   * <UL>
   * <LI>Do not use "new PGraphicsXxxx()", use this method. This method ensures that internal
   * variables are set up properly that tie the new graphics context back to its parent PApplet.
   * <LI>The basic way to create bitmap images is to use the
   * <A HREF="http://processing.org/reference/saveFrame_.html">saveFrame()</A> function.
   * <LI>If you want to create a really large scene and write that, first make sure that you've
   * allocated a lot of memory in the Preferences.
   * <LI>If you want to create images that are larger than the screen, you should create your own
   * PGraphics object, draw to that, and use
   ...
   * </PRE>
   *
   * <LI>It's important to always wrap drawing to createGraphics() with beginDraw() and endDraw()
   * (beginFrame() and endFrame() prior to revision 0115). The reason is that the renderer needs
   * to know when drawing has stopped, so that it can update itself internally. This also handles
   * calling the defaults() method, for people familiar with that.
   * <LI>With Processing 0115 and later, it's possible to write images in formats other than the
   * default .tga and .tiff. The exact formats and background information can be found in the
   * developer's reference for <A HREF=
  ...
  /**
   * Create an offscreen graphics surface for drawing, in this case for a renderer that writes to
   * a file (such as PDF or DXF).
   * 
   ...
   * 
   * @param path A path (or null if none), can be absolute or relative ({@link PApplet#savePath}
   *             will be called)
   */
  protected PGraphics makeGraphics(int w,int h,
    ...
    try {
      ...
      if((msg!=null)&&
        ...
        /*
         * // removing for 3.2, we'll see if (platform == MACOSX) { target.printStackTrace(System.out);
         * // OS X bug (still true?) }
         */
      ...
      if((e instanceof IllegalArgumentException)||
        ...
        /*
         * if (platform == MACOSX) { e.printStackTrace(System.out); // OS X bug (still true?) }
         */
        printStackTrace(e);
  ...
  /**
   *
   * Creates a new PImage (the datatype for storing images). This provides a fresh buffer of
   * pixels to play with. Set the size of the buffer with the <b>width</b> and <b>height</b>
   * parameters. The <b>format</b> parameter defines how the pixels are stored. See the PImage
   * reference for more information. <br/>
   * <br/>
   * Be sure to include all three parameters, specifying only the width and height (but no format)
   * will produce a strange error. <br/>
   * <br/>
   * Advanced users please note that createImage() should be used instead of the syntax
   * <tt>new PImage()</tt>.
   *
   * <h3>Advanced</h3> Preferred method of creating new PImage objects, ensures that a reference
   * to the parent PApplet is included, which makes save() work without needing an absolute path.
   *
   * @webref image
  ...
  /**
   *
   * Executes the code within <b>draw()</b> one time. This functions allows the program to update
   * the display window only when necessary, for example when an event registered by
   * <b>mousePressed()</b> or <b>keyPressed()</b> occurs. <br/>
   * <br/>
   * In structuring a program, it only makes sense to call redraw() within events such as
   * <b>mousePressed()</b>. This is because <b>redraw()</b> does not run <b>draw()</b> immediately
   * (it only sets a flag that indicates an update is needed). <br/>
   * <br/>
   * The <b>redraw()</b> function does not work properly when called inside <b>draw()</b>. To
   * enable/disable animations, use <b>loop()</b> and <b>noLoop()</b>.
   *
   * @webref structure
  ...
  /**
   *
   * By default, Processing loops through <b>draw()</b> continuously, executing the code within
   * it. However, the <b>draw()</b> loop may be stopped by calling <b>noLoop()</b>. In that case,
   * the <b>draw()</b> loop can be resumed with <b>loop()</b>.
   *
   * @webref structure
  ...
  /**
   *
   * Stops Processing from continuously executing the code within <b>draw()</b>. If <b>loop()</b>
   * is called, the code in <b>draw()</b> begin to run continuously again. If using
   * <b>noLoop()</b> in <b>setup()</b>, it should be the last line inside the block. <br/>
   * <br/>
   * When <b>noLoop()</b> is used, it's not possible to manipulate or access the screen inside
   * event handling functions such as <b>mousePressed()</b> or <b>keyPressed()</b>. Instead, use
   * those functions to call <b>redraw()</b> or <b>loop()</b>, which will run <b>draw()</b>, which
   * can update the screen properly. This means that when noLoop() has been called, no drawing can
   * happen, and functions like saveFrame() or loadPixels() may not be used. <br/>
   * <br/>
   * Note that if the sketch is resized, <b>redraw()</b> will be called to update the sketch, even
   * after <b>noLoop()</b> has been specified. Otherwise, the sketch would enter an odd state
   * until <b>loop()</b> was called.
   *
   * @webref structure
  ...
  /**
   * Add an event to the internal event queue, or process it immediately if the sketch is not
   * currently looping.
   */
  ...
  //////////////////////////////////////////////////////////////
  /**
   * Actually take action based on a mouse event. Internally updates mouseX, mouseY, mousePressed,
   * and mouseEvent. Then it calls the event type with no params, i.e. mousePressed() or
   * mouseReleased() that the user may have overloaded to do something more useful.
   */
  protected void handleMouseEvent(MouseEvent event) {
    ...
    mouseButton=button;
    /*
     * // Compatibility for older code (these have AWT object params, not P5) if (mouseEventMethods
     * != null) { // Probably also good to check this, in case anyone tries to call // postEvent()
     * with an artificial event they've created. if (event.getNative() != null) {
  ...
  /**
   *
   * The <b>mousePressed()</b> function is called once after every time a mouse button is pressed.
   * The <b>mouseButton</b> variable (see the related reference entry) can be used to determine
   ...
   * <h3>Advanced</h3>
   *
   * If you must, use int button = mouseEvent.getButton(); to figure out which button was clicked.
   * It will be one of: MouseEvent.BUTTON1, MouseEvent.BUTTON2, MouseEvent.BUTTON3 Note, however,
   * that this is completely inconsistent across platforms.
   *
   * @webref input:mouse
  ...
  /**
   *
   * The <b>mouseReleased()</b> function is called every time a mouse button is released. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
   *
  ...
  /**
   *
   * The <b>mouseClicked()</b> function is called <i>after</i> a mouse button has been pressed and
   * then released. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
   *
   * <h3>Advanced</h3> When the mouse is clicked, mousePressed() will be called, then
   * mouseReleased(), then mouseClicked(). Note that mousePressed is already false inside
   * mouseClicked().
   *
   * @webref input:mouse
  ...
  /**
   *
   * The <b>mouseDragged()</b> function is called once every time the mouse moves while a mouse
   * button is pressed. (If a button <i>is not</i> being pressed, <b>mouseMoved()</b> is called
   * instead.) <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
  ...
  /**
   *
   * The <b>mouseMoved()</b> function is called every time the mouse moves and a mouse button is
   * not pressed. (If a button <i>is</i> being pressed, <b>mouseDragged()</b> is called instead.)
   * <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
  ...
  /**
   * The code within the <b>mouseWheel()</b> event function is run when the mouse wheel is moved.
   * (Some mice don't have wheels and this function is only applicable with mice that have a
   * wheel.) The <b>getCount()</b> function used within <b>mouseWheel()</b> returns positive
   * values when the mouse wheel is rotated down (toward the user), and negative values for the
   * other direction (up or away from the user). On OS X with "natural" scrolling enabled, the
   * values are opposite. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
   *
   * @webref input:mouse
   * @webBrief The code within the <b>mouseWheel()</b> event function is run when the mouse wheel
   *           is moved
  ...
  protected void handleKeyEvent(KeyEvent event) {
    ...
    /*
     * if (keyEventMethods != null) { keyEventMethods.handle(new Object[] { event.getNative() }); }
     */
   ...
   *
   *
   * The <b>keyPressed()</b> function is called once every time a key is pressed. The key that was
   * pressed is stored in the <b>key</b> variable. <br />
   * <br />
   * For non-ASCII keys, use the <b>keyCode</b> variable. The keys included in the ASCII
   * specification (BACKSPACE, TAB, ENTER, RETURN, ESC, and DELETE) do not require checking to see
   * if the key is coded; for those keys, you should simply use the <b>key</b> variable directly
   * (and not <b>keyCode</b>). If you're making cross-platform projects, note that the ENTER key
   * is commonly used on PCs and Unix, while the RETURN key is used on Macs. Make sure your
   * program will work on all platforms by checking for both ENTER and RETURN. <br />
   * <br />
   * Because of how operating systems handle key repeats, holding down a key may cause multiple
   * calls to <b>keyPressed()</b>. The rate of repeat is set by the operating system, and may be
   * configured differently on each computer. <br />
   * <br />
   * Note that there is a similarly named boolean variable called <b>keyPressed</b>. See its
   * reference page for more information. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events. <br />
   * <br />
   * With the release of macOS Sierra, Apple changed how key repeat works, so keyPressed may not
   * function as expected. See <a href=
   ...
   * <h3>Advanced</h3>
   *
   * Called each time a single key on the keyboard is pressed. Because of how operating systems
   * handle key repeats, holding down a key will cause multiple calls to keyPressed(), because the
   * OS repeat takes over.
   * <p>
   * Examples for key handling: (Tested on Windows XP, please notify if different on other
   * platforms, I have a feeling macOS and Linux may do otherwise)
   *
   * <PRE>
  ...
  /**
   *
   * The <b>keyReleased()</b> function is called once every time a key is released. The key that
   * was released will be stored in the <b>key</b> variable. See <b>key</b> and <b>keyCode</b> for
   * more information. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
  ...
  /**
   *
   * The <b>keyTyped()</b> function is called once every time a key is pressed, but action keys
   * such as Ctrl, Shift, and Alt are ignored. <br />
   * <br />
   * Because of how operating systems handle key repeats, holding down a key may cause multiple
   * calls to <b>keyTyped()</b>. The rate of repeat is set by the operating system, and may be
   * configured differently on each computer. <br />
   * <br />
   * Mouse and keyboard events only work when a program has <b>draw()</b>. Without <b>draw()</b>,
   * the code is only run once and then stops listening for events.
   *
   * @webref input:keyboard
   * @webBrief Called once every time a key is pressed, but action keys such as Ctrl, Shift, and
   *           Alt are ignored
   * @see PApplet#keyPressed
   * @see PApplet#key
   * @see PApplet#keyCode
  ...
  /**
   *
   * Returns the number of milliseconds (thousandths of a second) since starting the sketch. This
   * information is often used for timing animation sequences.
   *
   * <h3>Advanced</h3> This is a function, rather than a variable, because it may change multiple
   * times per frame.
   *
   * @webref input:time date
   * @webBrief Returns the number of milliseconds (thousandths of a second) since the sketch
   *           started.
   * @see PApplet#second()
   * @see PApplet#minute()
   * @see PApplet#hour()
  ...
  /**
   *
   * Processing communicates with the clock on your computer. The <b>second()</b> function returns
   * the current second as a value from 0 to 59.
   *
   * @webref input:time date
  ...
  /**
   *
   * Processing communicates with the clock on your computer. The <b>minute()</b> function returns
   * the current minute as a value from 0 to 59.
   * 
   * @webref input:time date
  ...
  /**
   *
   * Processing communicates with the clock on your computer. The <b>hour()</b> function returns
   * the current hour as a value from 0 to 23.
   *
   * @webref input:time date
  ...
  /**
   *
   * Processing communicates with the clock on your computer. The <b>day()</b> function returns
   * the current day as a value from 1 to 31.
   *
   * <h3>Advanced</h3> Get the current day of the month (1 through 31).
   * <p>
   * If you're looking for the day of the week (M-F or whatever) or day of the year (1..365) then
   * use java's Calendar.get()
   *
   * @webref input:time date
  ...
  /**
   *
   * Processing communicates with the clock on your computer. The <b>month()</b> function returns
   * the current month as a value from 1 to 12.
   *
   * @webref input:time date
  ...
  /**
   * Processing communicates with the clock on your computer. The <b>year()</b> function returns
   * the current year as an integer (2003, 2004, 2005, etc).
   *
   * @webref input:time date
   * @webBrief Returns the current year as an integer (2003, 2004, 2005, etc)
   * @see PApplet#millis()
   * @see PApplet#second()
   * @see PApplet#minute()
   * @see PApplet#hour()
  ...
  /**
   *
   * The <b>delay()</b> function causes the program to halt for a specified time. Delay times are
   * specified in thousandths of a second. For example, running <b>delay(3000)</b> will stop the
   * program for three seconds and <b>delay(500)</b> will stop the program for a half-second.
   * <p/>
   * The screen only updates when the end of <b>draw()</b> is reached, so <b>delay()</b> cannot be
   * used to slow down drawing. For instance, you cannot use <b>delay()</b> to control the timing
   * of an animation.
   * <p/>
   * The <b>delay()</b> function should only be used for pausing scripts (i.e. a script that needs
   * to pause a few seconds before attempting a download, or a sketch that needs to wait a few
   * milliseconds before reading from the serial port).
   *
   * @webref environment
   ...
   * <b>frameRate(30)</b> will attempt to refresh 30 times a second. If the processor is not fast
   * enough to maintain the specified rate, the frame rate will not be achieved. Setting the frame
   * rate within <b>setup()</b> is recommended. The default rate is 60 frames per second.
   *
   * @webref environment
  ...
  //////////////////////////////////////////////////////////////
  /**
   * Links to a webpage either in the same window or in a new window. The complete URL must be
   * specified.
   *
  ...
  /**
   *
   * Attempts to open an application or file using your platform's launcher. The <b>filename</b>
   * parameter is a String specifying the file name and location. The location parameter must be a
   * full path name, or the name of an executable in the system's PATH. In most cases, using a
   * full path is the best option, rather than relying on the system PATH. Be sure to make the
   * file executable before attempting to open it (chmod +x).<br />
   * <br />
   * This function (roughly) emulates what happens when you double-click an application or
   * document in the macOS Finder, the Windows Explorer, or your favorite Linux file manager. If
   * you're trying to run command line functions directly, use the <b>exec()</b> function instead
   * (see below).<br />
   * <br />
   * This function behaves differently on each platform. On Windows, the parameters are sent to
   * the Windows shell via "cmd /c". On Mac OS X, the "open" command is used (type "man open" in
   * Terminal.app for documentation). On Linux, it first tries gnome-open, then kde-open, but if
   * neither are available, it sends the command to the shell and prays that something useful
   * happens.<br />
   * <br />
   * For users familiar with Java, this is not the same as Runtime.exec(), because the launcher
   * command is prepended. Instead, the <b>exec(String[])</b> function is a shortcut for
   * Runtime.getRuntime.exec(String[]). The <b>exec()</b> function is documented in the
   * <a href="http://processing.github.io/processing-javadocs/core/">JavaDoc</a> in the
   * <b>PApplet</b> class.
   *
   * @webref input:files
   * @webBrief Attempts to open an application or file using your platform's launcher
   * @param args arguments to the launcher, e.g. a filename.
   * @usage Application
   */
   ...
   * <A HREF="https://processing.org/reference/launch_.html">launch()</A> method, which uses the
   * operating system's launcher to open the files. It's always a good idea to use a full path to
   * the executable here.
   * 
   * <pre>
   ...
   * </pre>
   * 
   * You can also get the system output and error streams from the Process object, but that's more
   * that we'd like to cover here.
   * 
   * @return a <A HREF=
   *         "https://docs.oracle.com/javase/8/docs/api/java/lang/Process.html">Process</A> object
   */
  ...
  /**
   * Alternative version of exec() that retrieves stdout and stderr into the StringList objects
   * provided. This is a convenience function that handles simple exec() calls. If the results
   * will be more than a couple lines, you shouldn't use this function, you should use a more
   * elaborate method that makes use of proper threading (to drain the shell output) and error
   ...
   * @return the result returned from the application, or -1 if an Exception occurs before the
   *         application is able to return a result.
   */
  ...
  /*
   * static private final String shellQuoted(String arg) { if (arg.indexOf(' ') != -1) { // check
   * to see if already quoted if ((arg.charAt(0) != '\"' || arg.charAt(arg.length()-1) != '\"') &&
   * (arg.charAt(0) != '\'' || arg.charAt(arg.length()-1) != '\'')) {
   * 
   * // see which quotes we can use if (arg.indexOf('\"') == -1) { // if no double quotes, try
   * those first return "\"" + arg + "\"";
   * 
   * } else if (arg.indexOf('\'') == -1) { // if no single quotes, let's use those return "'" +
   * arg + "'"; } } } return arg; }
   */
  //////////////////////////////////////////////////////////////
  /**
   * Better way of handling e.printStackTrace() calls so that they can be handled by subclasses as
   * necessary.
   */
  ...
  /**
   * Function for an application to kill itself and display an error. Mostly this is here to be
   * improved later.
   */
  ...
  /**
   *
   * Quits/stops/exits the program. Programs without a <b>draw()</b> function exit automatically
   * after the last line has run, but programs with <b>draw()</b> run continuously until the
   * program is manually stopped or <b>exit()</b> is run.<br />
   * <br />
   * Rather than terminating immediately, <b>exit()</b> will cause the sketch to exit after
   * <b>draw()</b> has completed (or after <b>setup()</b> completes if called during the
   * <b>setup()</b> function).<br />
   * <br />
   * For Java programmers, this is <em>not</em> the same as System.exit(). Further, System.exit()
   * should not be used because closing out an application while <b>draw()</b> is running may
   * cause a crash (particularly with P3D).
   *
   * @webref structure
  ...
  /**
   * Some subclasses (I'm looking at you, processing.py) might wish to do something other than
   * actually terminate the JVM. This gives them a chance to do whatever they have in mind when
   * cleaning up.
   */
  ...
  /**
   * Called to dispose of resources and shut down the sketch. Destroys the thread, dispose the
   * renderer,and notify listeners.
   * <p>
   * Not to be called or overridden by users. If called multiple times, will only notify listeners
   * once. Register a "dispose" listener instead.
   */
  public void dispose() {
   ...
   * <p/>
   * Note that the function being called must be public. Inside the PDE, 'public' is automatically
   * added, but when used without the preprocessor, (like from Eclipse) you'll have to do it
   * yourself.
   */
  public void method(String name) {
    try {
      ...
      System.err.println("There is no public "+
        name+
        "() method "+
        "in the class "+
        getClass().getName());
  ...
  /**
   * Processing sketches follow a specific sequence of steps: <b>setup()</b> first, followed by
   * <b>draw()</b> over and over and over again in a loop. A thread is also a series of steps with
   * a beginning, a middle, and an end. A Processing sketch is a single thread, often referred to
   * as the "Animation" thread. Other threads' sequences, however, can run independently of the
   * main animation loop. In fact, you can launch any number of threads at one time, and they will
   * all run concurrently. <br />
   * <br />
   * You cannot draw to the screen from a function called by <b>thread()</b>. Because it runs
   * independently, the code will not be synchronized to the animation thread, causing strange or
   * at least inconsistent results. Use <b>thread()</b> to load files or do other tasks that take
   * time. When the task is finished, set a variable that indicates the task is complete, and
   * check that from inside your <b>draw()</b> method. <br />
   * <br />
   * Processing uses threads quite often, such as with library functions like
   * <b>captureEvent()</b> and <b>movieEvent()</b>. These functions are triggered by a different
   * thread running behind the scenes, and they alert Processing whenever they have something to
   * report. This is useful when you need to perform a task that takes too long and would slow
   * down the main animation's frame rate, such as grabbing data from the network. If a separate
   * thread gets stuck or has an error, the entire program won't grind to a halt, since the error
   * only stops that individual thread. <br />
   * <br />
   * Writing your own thread can be a complex endeavor that involves extending the Java <a href=
   * "https://docs.oracle.com/javase/tutorial/essential/concurrency/threads.html">Thread</a>
   * class. However, the <b>thread()</b> method is a quick and dirty way to implement a simple
   * thread in Processing. By passing in a <b>String</b> that matches the name of a function
   * declared elsewhere in the sketch, Processing will execute that function in a separate thread.
   *
   * @webref structure
  ...
  /**
   *
   * Saves an image from the display window. Append a file extension to the name of the file, to
   * indicate the file format to be used: either TIFF (.tif), TARGA (.tga), JPEG (.jpg), or PNG
   * (.png). If no extension is included in the filename, the image will save in TIFF format and
   * <b>.tif</b> will be added to the name. These files are saved to the sketch's folder, which
   * may be opened by selecting "Show sketch folder" from the "Sketch" menu. Alternatively, the
   * files can be saved to any location on the computer by using an absolute path (something that
   * starts with / on Unix and Linux, or a drive letter on Windows).<br />
   * <br />
   * All images saved from the main drawing window will be opaque. To save images without a
  ...
  /**
   *
   * Saves a numbered sequence of images, one image each time the function is run. To save an
   * image that is identical to the display window, run the function at the end of <b>draw()</b>
   * or within mouse and key events such as <b>mousePressed()</b> and <b>keyPressed()</b>. Use the
   * Movie Maker program in the Tools menu to combine these images to a movie.<br />
   * <br />
   * If <b>saveFrame()</b> is used without parameters, it will save files as screen-0000.tif,
   * screen-0001.tif, and so on. You can specify the name of the sequence with the <b>filename</b>
   * parameter, including hash marks (####), which will be replaced by the current
   * <b>frameCount</b> value. (The number of hash marks is used to determine how many digits to
   * include in the file names.) Append a file extension, to indicate the file format to be used:
   * either TIFF (.tif), TARGA (.tga), JPEG (.jpg), or PNG (.png). Image files are saved to the
   * sketch's folder, which may be opened by selecting "Show Sketch Folder" from the "Sketch"
   * menu.<br />
   * <br />
   * Alternatively, the files can be saved to any location on the computer by using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows).<br />
   * <br />
   * All images saved from the main drawing window will be opaque. To save images without a
   * background, use <b>createGraphics()</b>.
   *
   * @webref output:image
   * @webBrief Saves a numbered sequence of images, one image each time the function is run
   * @see PApplet#save(String)
   * @see PApplet#createGraphics(int, int, String, String)
   * @see PApplet#frameCount
   * @param filename any sequence of letters or numbers that ends with either ".tif", ".tga",
   *                 ".jpg", or ".png"
   */
  ...
  /**
   * Check a string for #### signs to see if the frame number should be inserted. Used for
   * functions like saveFrame() and beginRecord() to replace the # marks with the frame number. If
   * only one # is used, it will be ignored, under the assumption that it's probably not intended
  ...
  /**
   * Replace the cursor with the specified PImage. The x- and y- coordinate of the center will be
  ...
  /**
   *
   * Sets the cursor to a predefined symbol or an image, or makes it visible if already hidden. If
   * you are trying to set an image as the cursor, the recommended size is 16x16 or 32x32 pixels.
   * The values for parameters <b>x</b> and <b>y</b> must be less than the dimensions of the
   * image. <br />
   * <br />
   * Setting or hiding the cursor does not generally work with "Present" mode (when running
   * full-screen). <br />
   * <br />
   * With the P2D and P3D renderers, a generic set of cursors are used because the OpenGL renderer
   * doesn't have access to the default cursor images for each platform
   * (<a href="https://github.com/processing/processing/issues/3791">Issue 3791</a>).
   *
   * @webref environment
   * @webBrief Sets the cursor to a predefined symbol, an image, or makes it visible if already
   *           hidden
  ...
  /**
   * Show the cursor after noCursor() was called. Notice that the program remembers the last set
   * cursor type
   */
  public void cursor() {
    surface.showCursor();
  ...
  /**
   *
   * Hides the cursor from view. Will not work when running the program in a web browser or when
   * running in full screen (Present) mode.
   *
   * <h3>Advanced</h3> Hide the cursor by creating a transparent image and using it as a custom
   * cursor.
   * 
   * @webref environment
   ...
   * route, since the other // object types are also used frequently. static public void
  ...
  //
  /*
   * // not very useful, because it only works for public (and protected?) // fields of a class,
   * not local variables to methods public void printvar(String name) { try { Field field =
   * getClass().getDeclaredField(name); println(name + " = " + field.get(this)); } catch
  ...
  /*
   * static public final double map(double value, double istart, double istop, double ostart,
  ...
  /**
   *
   * Returns a float from a random series of numbers having a mean of 0 and standard deviation of
   * 1. Each time the <b>randomGaussian()</b> function is called, it returns a number fitting a
   * Gaussian, or normal, distribution. There is theoretically no minimum or maximum value that
   * <b>randomGaussian()</b> might return. Rather, there is just a very low probability that
   * values far from the mean will be returned; and a higher probability that numbers near the
   * mean will be returned.
   *
   * @webref math:random
   * @webBrief Returns a float from a random series of numbers having a mean of 0 and standard
   *           deviation of 1
  ...
  /**
   *
   * Generates random numbers. Each time the <b>random()</b> function is called, it returns an
   * unexpected value within the specified range. If only one parameter is passed to the function,
   * it will return a float between zero and the value of the <b>high</b> parameter. For example,
   * <b>random(5)</b> returns values between 0 and 5 (starting at zero, and up to, but not
   * including, 5).<br />
   * <br />
   * If two parameters are specified, the function will return a float with a value between the
   * two values. For example, <b>random(-5, 10.2)</b> returns values starting at -5 and up to (but
   * not including) 10.2. To convert a floating-point random number to an integer, use the
   * <b>int()</b> function.
   *
   * @webref math:random
  ...
  /**
   *
   * Sets the seed value for <b>random()</b>. By default, <b>random()</b> produces different
   * results each time the program is run. Set the <b>seed</b> parameter to a constant to return
  ...
  /**
   * Return a random integer from 0 up to (but not including) the specified value for “high”. This
   * is the same as calling random() and casting the result to an <b>int</b>.
   */
   ...
   * This is the same as calling random() and casting the result to an <b>int</b>.
   */
   ...
   * generator producing a more natural, harmonic succession of numbers than that of the standard
   * <b>random()</b> function. It was developed by Ken Perlin in the 1980s and has been used in
   * graphical applications to generate procedural textures, shapes, terrains, and other seemingly
   * organic forms.<br />
   * <br />
   * In contrast to the <b>random()</b> function, Perlin noise is defined in an infinite
   * n-dimensional space, in which each pair of coordinates corresponds to a fixed semi-random
   * value (fixed only for the lifespan of the program). The resulting value will always be
   * between 0.0 and 1.0. Processing can compute 1D, 2D and 3D noise, depending on the number of
   * coordinates given. The noise value can be animated by moving through the noise space, as
   * demonstrated in the first example above. The 2nd and 3rd dimensions can also be interpreted
   * as time.<br />
   * <br />
   * The actual noise structure is similar to that of an audio signal, in respect to the
   * function's use of frequencies. Similar to the concept of harmonics in physics, Perlin noise
   * is computed over several octaves which are added together for the final result.<br />
   * <br />
   * Another way to adjust the character of the resulting sequence is the scale of the input
   * coordinates. As the function works within an infinite space, the value of the coordinates
   * doesn't matter as such; only the <em>distance</em> between successive coordinates is
   * important (such as when using <b>noise()</b> within a loop). As a general rule, the smaller
   * the difference between coordinates, the smoother the resulting noise sequence. Steps of
   * 0.005-0.03 work best for most applications, but this will differ depending on use.<br />
   * <br />
   * There have been debates over the accuracy of the implementation of noise in Processing. For
   * clarification, it's an implementation of "classic Perlin noise" from 1983, and not the newer
   * "simplex noise" method from 2001.
   *
   * @webref math:random
  ...
  /**
   *
   * Adjusts the character and level of detail produced by the Perlin noise function. Similar to
   * harmonics in physics, noise is computed over several octaves. Lower octaves contribute more
   * to the output signal and as such define the overall intensity of the noise, whereas higher
   * octaves create finer-grained details in the noise sequence.<br />
   * <br />
   * By default, noise is computed over 4 octaves with each octave contributing exactly half than
   * its predecessor, starting at 50% strength for the first octave. This falloff amount can be
   * changed by adding a function parameter. For example, a falloff factor of 0.75 means each
   * octave will now have 75% impact (25% less) of the previous lower octave. While any number
   * between 0.0 and 1.0 is valid, note that values greater than 0.5 may result in <b>noise()</b>
   * returning values greater than 1.0.<br />
   * <br />
   * By changing these parameters, the signal created by the <b>noise()</b> function can be
   * adapted to fit very specific needs and characteristics.
   *
   * @webref math:random
   * @webBrief Adjusts the character and level of detail produced by the Perlin noise function
  ...
  /**
   *
   * Sets the seed value for <b>noise()</b>. By default, <b>noise()</b> produces different results
   * each time the program is run. Set the <b>value</b> parameter to a constant to return the same
  ...
  /**
   *
   * Loads an image into a variable of type <b>PImage</b>. Four types of images ( <b>.gif</b>,
   * <b>.jpg</b>, <b>.tga</b>, <b>.png</b>) images may be loaded. To load correctly, images must
   * be located in the data directory of the current sketch.<br />
   * <br />
   * In most cases, load all images in <b>setup()</b> to preload them at the start of the program.
   * Loading images inside <b>draw()</b> will reduce the speed of a program. Images cannot be
   * loaded outside <b>setup()</b> unless they're inside a function that's called after
   * <b>setup()</b> has already run.<br />
   * <br />
   * Alternatively, the file maybe be loaded from anywhere on the local computer using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows), or the
   * filename parameter can be a URL for a file found on a network.<br />
   * <br />
   * If the file is not available or an error occurs, <b>null</b> will be returned and an error
   * message will be printed to the console. The error message does not halt the program, however
   * the <b>null</b> value may cause a NullPointerException if your code does not check whether
   * the value returned is <b>null</b>.<br />
   * <br />
   * The <b>extension</b> parameter is used to determine the image type in cases where the image
   * filename does not end with a proper extension. Specify the extension as the second parameter
   * to <b>loadImage()</b>, as shown in the third example on this page. Note that CMYK images are
   * not supported.<br />
   * <br />
   * Depending on the type of error, a <b>PImage</b> object may still be returned, but the width
   * and height of the image will be set to -1. This happens if bad image data is returned or
   * cannot be decoded properly. Sometimes this happens with image URLs that produce a 403 error
   * or that redirect to a password prompt, because <b>loadImage()</b> will attempt to interpret
   * the HTML as image data.
   *
   * @webref image:loading & displaying
   * @webBrief Loads an image into a variable of type <b>PImage</b>
   * @param filename name of file to load, can be .gif, .jpg, .tga, or a handful of other image
  ...
  /**
   *
   * This function loads images on a separate thread so that your sketch doesn't freeze while
   * images load during <b>setup()</b>. While the image is loading, its width and height will be
   * 0. If an error occurs while loading the image, its width and height will be set to -1. You'll
   * know when the image has loaded properly because its <b>width</b> and <b>height</b> will be
   * greater than 0. Asynchronous image loading (particularly when downloading from a server) can
   ...
   *
   * @webref image:loading & displaying
   * @webBrief Loads images on a separate thread so that your sketch does not freeze while images
   *           load during <b>setup()</b>
   * @param filename  name of the file to load, can be .gif, .jpg, .tga, or a handful of other
  ...
  // DATA I/O
  /**
   * Reads the contents of a file or URL and creates an XML object with its values. If a file is
   * specified, it must be located in the sketch's "data" folder. The filename parameter can also
   * be a URL to a file found online.<br />
   * <br />
   * All files loaded and saved by the Processing API use UTF-8 encoding. If you need to load an
   * XML file that's not in UTF-8 format, see the
   * <a href="http://processing.github.io/processing-javadocs/core/processing/data/XML.html">
   * developer's reference</a> for the XML object.
   * 
   * @webref input:files
   * @webBrief Reads the contents of a file or URL and creates an <b>XML</b> object with its
   *           values
  ...
  /**
   * Takes a String, parses its contents, and returns an XML object. If the String does not
   * contain XML data or cannot be parsed, a <b>null</b> value is returned.<br />
   * <br />
   * <b>parseXML()</b> is most useful when pulling data dynamically, such as from third-party
   * APIs. Normally, API results would be saved to a String, and then can be converted to a
   * structured XML object using <b>parseXML()</b>. Be sure to check if <b>null</b> is returned
   * before performing operations on the new XML object, in case the String content could not be
   * parsed.<br />
   * <br />
   * If your data already exists as an XML file in the data folder, it is simpler to use
   * <b>loadXML()</b>.
   *
   * @webref input:files
  ...
  /**
   * Writes the contents of an XML object to a file. By default, this file is saved to the
   * sketch's folder. This folder is opened by selecting "Show Sketch Folder" from the "Sketch"
   * menu.<br />
   * <br />
   * Alternatively, the file can be saved to any location on the computer by using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows).<br />
   * <br />
  ...
  /**
   * Takes a <b>String</b>, parses its contents, and returns a <b>JSONObject</b>. If the
   * <b>String</b> does not contain <b>JSONObject</b> data or cannot be parsed, a <b>null</b>
   * value is returned.<br />
   * <br />
   * <b>parseJSONObject()</b> is most useful when pulling data dynamically, such as from
   * third-party APIs. Normally, API results would be saved to a <b>String</b>, and then can be
   * converted to a structured <b>JSONObject</b> using <b>parseJSONObject()</b>. Be sure to check
   * if <b>null</b> is returned before performing operations on the new <b>JSONObject</b> in case
   * the <b>String</b> content could not be parsed.<br />
   * <br />
   * If your data already exists as a <b>JSON</b> file in the data folder, it is simpler to use
   * <b>loadJSONObject()</b>.
   *
   * @webref input:files
   * @webBrief Takes a <b>String</b>, parses its contents, and returns a <b>JSONObject</b>
   ...
   *
   * @webref input:files
   * @webBrief Loads a JSON from the data folder or a URL, and returns a <b>JSONObject</b>
   * @param filename name of a file in the data folder or a URL
   * @see JSONObject
   * @see JSONArray
  ...
  /**
   * Writes the contents of a <b>JSONObject</b> object to a file. By default, this file is saved
   * to the sketch's folder. This folder is opened by selecting "Show Sketch Folder" from the
   * "Sketch" menu.<br />
   * <br />
   * Alternatively, the file can be saved to any location on the computer by using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows).<br />
  ...
  /**
   * Takes a <b>String</b>, parses its contents, and returns a <b>JSONArray</b>. If the
   * <b>String</b> does not contain <b>JSONArray</b> data or cannot be parsed, a <b>null</b> value
   * is returned.<br />
   * <br />
   * <b>parseJSONArray()</b> is most useful when pulling data dynamically, such as from
   * third-party APIs. Normally, API results would be saved to a <b>String</b>, and then can be
   * converted to a structured <b>JSONArray</b> using <b>parseJSONArray()</b>. Be sure to check if
   * <b>null</b> is returned before performing operations on the new <b>JSONArray</b> in case the
  ...
  /**
   * Loads an array of JSON objects from the data folder or a URL, and returns a <b>JSONArray</b>.
   * Per standard JSON syntax, the array must be enclosed in a pair of hard brackets <b>[]</b>,
   * and each object within the array must be separated by a comma.<br />
   * <br />
   * All files loaded and saved by the Processing API use UTF-8 encoding.
   *
   * @webref input:files
  ...
  /**
   * Writes the contents of a <b>JSONArray</b> object to a file. By default, this file is saved to
   * the sketch's folder. This folder is opened by selecting "Show Sketch Folder" from the
   * "Sketch" menu.<br />
   * <br />
   * Alternatively, the file can be saved to any location on the computer by using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows).<br />
  ...
  /**
   * Reads the contents of a file or URL and creates a Table object with its values. If a file is
   * specified, it must be located in the sketch's "data" folder. The filename parameter can also
   * be a URL to a file found online. The filename must either end in an extension or an extension
   * must be specified in the <b>options</b> parameter. For example, to use tab-separated data,
   * include "tsv" in the options parameter if the filename or URL does not end in <b>.tsv</b>.
   * Note: If an extension is in both places, the extension in the <b>options</b> is used.<br />
   * <br />
   * If the file contains a header row, include "header" in the <b>options</b> parameter. If the
   * file does not have a header row, then simply omit the "header" option.<br />
   * <br />
   * Some CSV files contain newline (CR or LF) characters inside cells. This is rare, but adding
   * the "newlines" option will handle them properly. (This is not enabled by default because the
   * parsing code is much slower.)<br />
   * <br />
   * When specifying multiple options, separate them with commas, as in: <b>loadTable("data.csv",
   * "header, tsv")</b><br />
   * <br />
   * All files loaded and saved by the Processing API use UTF-8 encoding.
   *
   * @webref input:files
   * @webBrief Reads the contents of a file or URL and creates a <b>Table</b> object with its
   *           values
   * @param filename name of a file in the data folder or a URL.
   * @see Table
   ...
   * <p/>
   * Another option is "dictionary=filename.tsv", which allows users to specify a "dictionary"
   * file that contains a mapping of the column titles and the data types used in the table file.
   * This can be far more efficient (in terms of speed and memory usage) for loading and parsing
   * tables. The dictionary file can only be tab-separated values (.tsv) and its extension will be
  ...
  /**
   * Writes the contents of a Table object to a file. By default, this file is saved to the
   * sketch's folder. This folder is opened by selecting "Show Sketch Folder" from the "Sketch"
   * menu.<br />
   * <br />
   * Alternatively, the file can be saved to any location on the computer by using an absolute
  ...
  /**
   *
   * Loads a .vlw formatted font into a <b>PFont</b> object. Create a .vlw font by selecting
   * "Create Font..." from the Tools menu. This tool creates a texture for each alphanumeric
   * character and then adds them as a .vlw file to the current sketch's data folder. Because the
   * letters are defined as textures (and not vector data) the size at which the fonts are created
   * must be considered in relation to the size at which they are drawn. For example, load a 32pt
   * font if the sketch displays the font at 32 pixels or smaller. Conversely, if a 12pt font is
   * loaded and displayed at 48pts, the letters will be distorted because the program will be
   * stretching a small graphic to a large size.<br />
   * <br />
   * Like <b>loadImage()</b> and other functions that load data, the <b>loadFont()</b> function
   * should not be used inside <b>draw()</b>, because it will slow down the sketch considerably,
   * as the font will be re-loaded from the disk (or network) on each frame. It's recommended to
   * load files inside <b>setup()</b><br />
   * <br />
   * To load correctly, fonts must be located in the "data" folder of the current sketch.
   * Alternatively, the file maybe be loaded from anywhere on the local computer using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows), or the
   * filename parameter can be a URL for a file found on a network.<br />
   * <br />
   ...
   * the value returned is <b>null</b>.<br />
   * <br />
   * Use <b>createFont()</b> (instead of <b>loadFont()</b>) to enable vector data to be used with
   * the default renderer setting. This can be helpful when many font sizes are needed, or when
   * using any renderer based on the default renderer, such as the PDF library.
   *
  ...
  /**
   *
   * Dynamically converts a font to the format used by Processing from a .ttf or .otf file inside
   * the sketch's "data" folder or a font that's installed elsewhere on the computer. If you want
   * to use a font installed on your computer, use the <b>PFont.list()</b> method to first
   * determine the names for the fonts recognized by the computer and are compatible with this
   * function. Not all fonts can be used and some might work with one operating system and not
   * others. When sharing a sketch with other people or posting it on the web, you may need to
   * include a .ttf or .otf version of your font in the data directory of the sketch because other
   * people might not have the font installed on their computer. Only fonts that can legally be
   * distributed should be included with a sketch.<br />
   * <br />
   * The <b>size</b> parameter states the font size you want to generate. The <b>smooth</b>
   * parameter specifies if the font should be anti-aliased or not. The <b>charset</b> parameter
   * is an array of chars that specifies the characters to generate.<br />
   * <br />
   * This function allows Processing to work with the font natively in the default renderer, so
   * the letters are defined by vector geometry and are rendered quickly. In the <b>P2D</b> and
   * <b>P3D</b> renderers, the function sets the project to render the font as a series of small
   * textures. For instance, when using the default renderer, the actual native version of the
   * font will be employed by the sketch, improving drawing quality and performance. With the
   * <b>P2D</b> and <b>P3D</b> renderers, the bitmapped version will be used to improve speed and
   * appearance, but the results are poor when exporting if the sketch does not include the .otf
   * or .ttf file, and the requested font is not available on the machine running the sketch.
   *
   * @webref typography:loading & displaying
   * @webBrief Dynamically converts a font to the format used by Processing
   * @param name    name of the font to load
   * @param size    point size of the font
   * @param smooth  true for an anti-aliased font, false for aliased
   ...
   * is made, the selected File will be passed to the 'callback' function. If the dialog is closed
   * or canceled, <b>null</b> will be sent to the function, so that the program is not waiting for
   ...
   * </pre>
   *
   * For advanced users, the method must be 'public', which is true for all methods inside a
   * sketch when run from the PDE, but must explicitly be set when using Eclipse or other
  ...
  /**
   * Opens a platform-specific file chooser dialog to select a file for output. After the
   * selection is made, the selected File will be passed to the 'callback' function. If the dialog
   * is closed or canceled, <b>null</b> will be sent to the function, so that the program is not
   * waiting for additional input. The callback is necessary because of how threading works.
   *
   * @webref output:files
  ...
  /**
   * Opens a platform-specific file chooser dialog to select a folder. After the selection is
   * made, the selection will be passed to the 'callback' function. If the dialog is closed or
   * canceled, null will be sent to the function, so that the program is not waiting for
   * additional input. The callback is necessary because of how threading works.
   *
   * @webref input:files
  ...
  /**
   *
   * Creates a <b>BufferedReader</b> object that can be used to read files line-by-line as
   * individual <b>String</b> objects. This is the complement to the <b>createWriter()</b>
   * function. For more information about the <b>BufferedReader</b> class and its methods like
   * <b>readLine()</b> and <b>close</b> used in the above example, please consult a Java
   * reference.<br />
   * <br />
   * Starting with Processing release 0134, all files loaded and saved by the Processing API use
   * UTF-8 encoding. In previous releases, the default encoding for your platform was used, which
   * causes problems when files are moved to other platforms.
   *
   * @webref input:files
   * @webBrief Creates a <b>BufferedReader</b> object that can be used to read files line-by-line
   *           as individual <b>String</b> objects
  ...
  /**
   * @nowebref I want to read lines from a stream. If I have to type the following lines anymore
   *           I'm gonna send Sun my medical bills.
   */
  ...
  /**
   *
   * Creates a new file in the sketch folder, and a <b>PrintWriter</b> object to write to it. For
   * the file to be made correctly, it should be flushed and must be closed with its
   * <b>flush()</b> and <b>close()</b> methods (see above example). <br/>
   * <br/>
   * Starting with Processing release 0134, all files loaded and saved by the Processing API use
   * UTF-8 encoding. In previous releases, the default encoding for your platform was used, which
   * causes problems when files are moved to other platforms.
   *
   * @webref output:files
   * @webBrief Creates a new file in the sketch folder, and a <b>PrintWriter</b> object to write
   *           to it
  ...
  /**
   * @nowebref I want to print lines to a file. I have RSI from typing these eight lines of code
   *           so many times.
   */
  ...
  /**
   * @nowebref I want to print lines to a file. Why am I always explaining myself? It's the
   *           JavaSoft API engineers who need to explain themselves.
   */
  ...
  // FILE INPUT
  /**
   * This is a function for advanced programmers to open a Java InputStream. It's useful if you
   * want to use the facilities provided by PApplet to easily open files from the data folder or
   * from a URL, but want an InputStream object so that you can use other parts of Java to take
   * more control of how the stream is read.<br />
   * <br />
   ...
   * <br />
   * If the requested item doesn't exist, <b>null</b> is returned. If not online, this will also
   * check to see if the user is asking for a file whose name isn't properly capitalized. If
   * capitalization is different, an error will be printed to the console. This helps prevent
   * issues that appear when a sketch is exported to the web, where case sensitivity matters, as
   * opposed to running from inside the Processing Development Environment on Windows or macOS,
   * where case sensitivity is preserved but ignored.<br />
   * <br />
   * If the file ends with <b>.gz</b>, the stream will automatically be gzip decompressed. If you
   * don't want the automatic decompression, use the related function <b>createInputRaw()</b>.
   * <br />
   ...
   * <p>
   * This method is useful if you want to use the facilities provided by PApplet to easily open
   * things from the data folder or from a URL, but want an InputStream object so that you can use
   * other Java methods to take more control of how the stream is read.
   * <p>
   * If the requested item doesn't exist, null is returned. (Prior to 0096, die() would be called,
   * killing the sketch)
   * <p>
   * For 0096+, the "data" folder is exported intact with subfolders, and openStream() properly
   * handles subdirectories from the data folder
   * <p>
   * If not online, this will also check to see if the user is asking for a file whose name isn't
   * properly capitalized. This helps prevent issues when a sketch is exported to the web, where
   * case sensitivity matters, as opposed to Windows and the macOS default where case sensitivity
   * is preserved but ignored.
   * <p>
   * It is strongly recommended that libraries use this method to open data files, so that the
   * loading sequence is handled in the same way as functions like loadBytes(), loadImage(), etc.
   * <p>
  ...
  /**
   *
   * Reads the contents of a file and places it in a byte array. If the name of the file is used
   * as the parameter, as in the above example, the file must be loaded in the sketch's "data"
   * directory/folder. <br />
   * <br />
   * Alternatively, the file maybe be loaded from anywhere on the local computer using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows), or the
  ...
  /**
   *
   * Reads the contents of a file and creates a String array of its individual lines. If the name
   * of the file is used as the parameter, as in the above example, the file must be loaded in the
   ...
   * <p>
   * Exceptions are handled internally, when an error, occurs, an exception is printed to the
   * console and 'null' is returned, but the program continues running. This is a tradeoff between
   * 1) showing the user that there was a problem but 2) not requiring that all i/o code is
   * contained in try/catch blocks, for the sake of new users (or people who are just trying to
   * get things done in an informal "scripting" fashion). If you want to handle exceptions, use
   * Java methods for I/O.
   *
   * @webref input:files
   * @webBrief Reads the contents of a file or url and creates a <b>String</b> array of its
   *           individual lines
  ...
  /**
   *
   * Similar to <b>createInput()</b>, this creates a Java <b>OutputStream</b> for a given filename
   * or path. The file will be created in the sketch folder, or in the same folder as an exported
   * application. <br />
   * <br />
   * If the path does not exist, intermediate folders will be created. If an exception occurs, it
   * will be printed to the console, and <b>null</b> will be returned. <br />
   * <br />
   * This function is a convenience over the Java approach that requires you to 1) create a
   * FileOutputStream object, 2) determine the exact file location, and 3) handle exceptions.
   * Exceptions are handled internally by the function, which is more appropriate for "sketch"
   * projects. <br />
   * <br />
   * If the output filename ends with <b>.gz</b>, the output will be automatically GZIP compressed
   * as it is written.
   *
   * @webref output:files
   * @webBrief Similar to <b>createInput()</b>, this creates a Java <b>OutputStream</b> for a
   *           given filename or path
   ...
   * <b>saveBytes(blah, loadBytes())</b>, but done more efficiently (and with less confusing
   * syntax).<br />
   * <br />
   * The <b>target</b> parameter can be either a String specifying a file name, or, for greater
   * control over the file location, a <b>File</b> object. (Note that, unlike some other
  ...
  /**
   * Identical to the other saveStream(), but writes to a File object, for greater control over
   * the file location.
   * <p/>
   * Note that unlike other api methods, this will not automatically compress or uncompress gzip
   * files.
   */
  ...
  /**
   *
   * As the opposite of <b>loadBytes()</b>, this function will write an entire array of bytes to a
   * file. The data is saved in binary format. This file is saved to the sketch's folder, which is
   * opened by selecting "Show Sketch Folder" from the "Sketch" menu. Alternatively, the files can
   * be saved to any location on the computer by using an absolute path (something that starts
   * with / on Unix and Linux, or a drive letter on Windows).
   *
   * @webref output:files
   * @webBrief Opposite of <b>loadBytes()</b>, will write an entire array of bytes to a file
   * @param filename name of the file to write to
  ...
  /**
   * Creates a temporary file based on the name/extension of another file and in the same parent
   * directory. Ensures that the same extension is used (i.e. so that .gz files are gzip
   * compressed on output) and that it's done from the same directory so that renaming the file
   * later won't cross file system boundaries.
   */
  ...
  /**
   *
   * Writes an array of Strings to a file, one line per String. By default, this file is saved to
   * the sketch's folder. This folder is opened by selecting "Show Sketch Folder" from the
   * "Sketch" menu.<br />
   * <br />
   * Alternatively, the file can be saved to any location on the computer by using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows).<br />
   * <br />
   * Starting with Processing 1.0, all files loaded and saved by the Processing API use UTF-8
   * encoding. In earlier releases, the default encoding for your platform was used, which causes
  ...
  /**
   * Prepend the sketch folder path to the filename (or path) that is passed in. External
   * libraries should use this function to save to the sketch folder.
   * <p/>
   * This will also cause an error if the sketch is not inited properly, meaning that init() was
   * never called on the PApplet when hosted my some other main() or by other code. For proper use
  ...
  /**
   * Returns a path adjacent the application to save to. Like sketchPath(), but creates any
   * in-between folders so that things save properly.
   * <p/>
   * All saveXxxx() functions use the path to the sketch folder, rather than its data folder. Once
   * exported, the data folder will be found inside the jar file of the exported application. In
   * this case, it's not possible to save data into the jar file, because it will often be running
   * from a server, or marked in-use if running from a local file system. With this in mind,
   * saving to the data path doesn't make sense anyway. If you know you're running locally, and
   * want to save to the data folder, use <TT>saveXxxx("data/blah.dat")</TT>.
   */
  ...
  /**
   * <b>This function almost certainly does not do the thing you want it to.</b> The data path is
   * handled differently on each platform, and should not be considered a location to write files.
   * It should also not be assumed that this location can be read from or listed. This function is
   * used internally as a possible location for reading files. It's still "public" as a holdover
   * from earlier code.
   * <p>
   * Libraries should use createInput() to get an InputStream or createOutput() to get an
   * OutputStream. sketchPath() can be used to get a location relative to the sketch. Again, <b>do
   * not</b> use this to get relative locations of files. You'll be disappointed when your app
  ...
  /**
   * Return a full path to an item in the data folder as a File object. See the dataPath() method
  ...
  /**
   * Takes a path and creates any in-between folders if they don't already exist. Useful when
   ...
   * mode is RGB values from 0 to 255 and, therefore, <b>color(255, 204, 0)</b> will return a
   * bright yellow color (see the first example above).<br />
   * <br />
   * Note that if only one value is provided to <b>color()</b>, it will be interpreted as a
   * grayscale value. Add a second value, and it will be used for alpha transparency. When three
   * values are specified, they are interpreted as either RGB or HSB values. Adding a fourth value
   * applies alpha transparency.<br />
   * <br />
   * Note that when using hexadecimal notation, it is not necessary to use <b>color()</b>, as in:
   * <b>color c = #006699</b><br />
   * <br />
   ...
   * controls the amount of each color to use where an amount of 0.0 will produce the first color,
   * 1.0 will return the second color, and 0.5 is halfway in between. Values between 0.0 and 1.0
   * will interpolate between the two colors in that proportion. <br />
   * An amount below 0 will be treated as 0. Likewise, amounts above 1 will be capped at 1. This
   * is different from the behavior of <b>lerp()</b>, but necessary because otherwise numbers
   * outside the range will produce strange and unexpected colors.
   *
   * @webref color:creating & reading
  ...
  //////////////////////////////////////////////////////////////
  /*
   * public void frameMoved(int x, int y) { if (!fullScreen) { System.err.println(EXTERNAL_MOVE +
   * " " + x + " " + y); System.err.flush(); // doesn't seem to help or hurt } }
   * 
   * 
  ...
  /**
   * Internal use only: called by Surface objects to queue a resize event to call windowResized()
   * when it's safe, which is after the beginDraw() call and before the draw(). Note that this is
   ...
   * Needs to remain a separate method so that it can be overridden by Python Mode.
   */
  ...
  /**
   * Internal use only: called by Surface objects to queue a position event to call
   * windowPositioned() when it's safe, which is after the beginDraw() call and before the draw().
   * Note that this is only the notification that the window is in a new position.
   */
  ...
  /**
   * Scale the sketch as if it fits this specific width and height. This will also scale the
   * mouseX and mouseY variables (as well as pmouseX and pmouseY). Note that it will not have an
   * effect on MouseEvent objects (i.e. event.getX() and event.getY()) because their exact
   * behavior may interact strangely with other libraries.
   */
   ...
   * <li>The class name is required. If you're running outside the PDE and your class is in a
   * package, this should include the full name. That means that if the class is called Sketchy
   * and the package is com.sketchycompany then com.sketchycompany.Sketchy should be used as the
   * class name.
   * <li>The [sketch args] are any command line parameters you want to send to the sketch itself.
   * These will be passed into the args[] array in PApplet.
   * <p>
   * The simplest way to turn and sketch into an application is to add the following code to your
   * program:
   * 
   * <PRE>
  ...
  /**
   * Convenience method so that PApplet.main(YourSketch.class) launches a sketch, rather than
   * having to call getName() on it.
   */
  static public void main(final Class<?> mainClass,String... args) {
    main(mainClass.getName(),args);
  }
  /**
   * Convenience method so that PApplet.main("YourSketch") launches a sketch, rather than having
   * to wrap it into a single element String array.
   * 
  ...
  /**
   * Convenience method so that PApplet.main("YourSketch", args) launches a sketch, rather than
   * having to wrap it into a String array, and appending the 'args' array when not null.
   * 
   * @param mainClass  name of the class to load (with package if any)
   * @param sketchArgs command line arguments to pass to the sketch's 'args' array. Note that this
   *                   is <i>not</i> the same as the args passed to (and understood by) PApplet
   *                   such as --display.
   */
  ...
  static public void runSketch(final String[] args,
    ...
    /*
     * // In an exported application, add the Contents/Java folder to the // java.library.path, so
     * that native libraries work properly. // Without this, the library path is only set to
     * Contents/MacOS // where the launcher binary lives. if (platform == MACOSX) { URL coreJarURL =
     * PApplet.class.getProtectionDomain().getCodeSource().getLocation(); // The jarPath from above
     * will/may be URL encoded (%20 for spaces) String coreJarPath =
     * urlDecode(coreJarURL.getPath()); if (coreJarPath.endsWith("/Contents/Java/core.jar")) { //
     * remove the /core.jar part from the end String javaPath = coreJarPath.substring(0,
     * coreJarPath.length() - 9); String libraryPath = System.getProperty("java.library.path");
     * libraryPath += File.pathSeparator + javaPath; System.setProperty("java.library.path",
     * libraryPath); } }
     */
    ...
    /*
     * // not always running externally when in present mode // moved above setVisible() in 3.0
     * alpha 11 if (sketch.external) { surface.setupExternalMessages(); }
     */
    sketch.showSurface();
    sketch.startSurface();
  }
  ...
  protected PSurface initSurface() {
    ...
    if(g.displayable()) {
      /*
       * if (!disableAWT) { frame = new Frame() {
       * 
       * @Override public void setResizable(boolean resizable) { deprecationWarning("setResizable");
       * surface.setResizable(resizable); }
       * 
       * @Override public void setVisible(boolean visible) { deprecationWarning("setVisible");
       * surface.setVisible(visible); }
       * 
       * @Override public void setTitle(String title) { deprecationWarning("setTitle");
       * surface.setTitle(title); }
       * 
       * @Override public void setUndecorated(boolean ignored) { throw new
       * RuntimeException("'frame' has been removed from Processing 3, " +
       ...
       * 
       * @Override public void setLocation(int x, int y) { deprecationWarning("setLocation");
       * surface.setLocation(x, y); }
       */
      /*
       * @Override public void setSize(int w, int h) { deprecationWarning("setSize");
       * surface.setSize(w, h); }
       * 
       * private void deprecationWarning(String method) { PGraphics.showWarning("Use surface." +
       * method + "() instead of " + "frame." + method + " in Processing 3"); //new
       * Exception(method).printStackTrace(System.out); } }; }
       */
  ...
  /**
   * Find the location of fenster.exe by walking through java.library.path. (It will be on the
   * path because it's part of core/library/windows-amd64)
   */
  ...
  /**
   * Get the display scaling for Windows by calling out to a helper app. More <a href=
   * "https://github.com/processing/processing4/tree/master/build/windows/fenster">here</a>.
   */
  ...
  /**
   * Convenience method for Python Mode to run an already-constructed sketch. This makes it easy
   * to launch a sketch in Jython:
   *
   * <pre>
  ...
  /**
   *
   * Opens a new file and all subsequent drawing functions are echoed to this file as well as the
   * display window. The <b>beginRecord()</b> function requires two parameters, the first is the
   * renderer and the second is the file name. This function is always used with
   * <b>endRecord()</b> to stop the recording process and close the file. <br />
   * <br />
   * Note that <b>beginRecord()</b> will only pick up any settings that happen after it has been
   * called. For instance, if you call <b>textFont()</b> before <b>beginRecord()</b>, then that
   * font will not be set for the file that you're recording to. <br />
   * <br />
   * <b>beginRecord()</b> works only with the PDF and SVG renderers.
   *
   * @webref output:files
   * @webBrief Opens a new file and all subsequent drawing functions are echoed to this file as
   ...
   *
   * @webref output:files
   * @webBrief Stops the recording process started by <b>beginRecord()</b> and closes the file
  ...
  /**
   *
   * To create vectors from 3D data, use the <b>beginRaw()</b> and <b>endRaw()</b> commands. These
   * commands will grab the shape data just before it is rendered to the screen. At this stage,
   * your entire scene is nothing but a long list of individual lines and triangles. This means
   * that a shape created with <b>sphere()</b> function will be made up of hundreds of triangles,
   * rather than a single object. Or that a multi-segment line shape (such as a curve) will be
   * rendered as individual segments. <br />
   * <br />
   * When using <b>beginRaw()</b> and <b>endRaw()</b>, it's possible to write to either a 2D or 3D
   * renderer. For instance, <b>beginRaw()</b> with the PDF library will write the geometry as
   * flattened triangles and lines, even if recording from the <b>P3D</b> renderer. <br />
   * <br />
   * If you want a background to show up in your files, use <b>rect(0, 0, width, height)</b> after
   * setting the <b>fill()</b> to the background color. Otherwise, the background will not be
   * rendered to the file because the background is not shape. <br />
   * <br />
   * Using <b>hint(ENABLE_DEPTH_SORT)</b> can improve the appearance of 3D geometry drawn to 2D
   * file formats. See the <b>hint()</b> reference for more details. <br />
   * <br />
   * See examples in the reference for the <b>PDF</b> and <b>DXF</b> libraries for more
   * information.
   *
   * @webref output:files
   * @webBrief To create vectors from 3D data, use the <b>beginRaw()</b> and <b>endRaw()</b>
   *           commands
   ...
   *           by preproc.pl) for clarity and so that it doesn't echo the command should
   *           beginRecord() be in use.
   *
  ...
  /**
   *
   * Complement to <b>beginRaw()</b>; they must always be used together. See the <b>beginRaw()</b>
  ...
  /**
   *
   * Loads the pixel data of the current display window into the <b>pixels[]</b> array. This
   * function must always be called before reading from or writing to <b>pixels[]</b>. Subsequent
   * changes to the display window will not be reflected in <b>pixels</b> until
   * <b>loadPixels()</b> is called again.
   *
   * <h3>Advanced</h3> Override the g.pixels[] function to set the pixels[] array that's part of
   * the PApplet object. Allows the use of pixels[] in the code, rather than g.pixels[].
   *
   * @webref image:pixels
   * @webBrief Loads the pixel data for the display window into the <b>pixels[]</b> array
   * @see PApplet#pixels
   * @see PApplet#updatePixels()
   */
  ...
  /**
   *
   * Updates the display window with the data in the <b>pixels[]</b> array. Use in conjunction
   * with <b>loadPixels()</b>. If you're only reading pixels from the array, there's no need to
   * call <b>updatePixels()</b> &mdash; updating is only necessary to apply changes.
   *
   * @webref image:pixels
  ...
  /**
   *
   * Using the <b>beginShape()</b> and <b>endShape()</b> functions allow creating more complex
   * forms. <b>beginShape()</b> begins recording vertices for a shape and <b>endShape()</b> stops
   * recording. The value of the <b>kind</b> parameter tells it which types of shapes to create
   * from the provided vertices. With no mode specified, the shape can be any irregular polygon.
   * The parameters available for beginShape() are POINTS, LINES, TRIANGLES, TRIANGLE_FAN,
   * TRIANGLE_STRIP, QUADS, and QUAD_STRIP. After calling the <b>beginShape()</b> function, a
   * series of <b>vertex()</b> commands must follow. To stop drawing the shape, call
   * <b>endShape()</b>. The <b>vertex()</b> function with two parameters specifies a position in
   * 2D and the <b>vertex()</b> function with three parameters specifies a position in 3D. Each
   * shape will be outlined with the current stroke color and filled with the fill color. <br />
   * <br />
   * Transformations such as <b>translate()</b>, <b>rotate()</b>, and <b>scale()</b> do not work
   * within <b>beginShape()</b>. It is also not possible to use other shapes, such as
   * <b>ellipse()</b> or <b>rect()</b> within <b>beginShape()</b>. <br />
   * <br />
   * The P2D and P3D renderers allow <b>stroke()</b> and <b>fill()</b> to be altered on a
   * per-vertex basis, but the default renderer does not. Settings such as <b>strokeWeight()</b>,
   * <b>strokeCap()</b>, and <b>strokeJoin()</b> cannot be changed while inside a
   * <b>beginShape()</b>/<b>endShape()</b> block with any renderer.
   *
   * @webref shape:vertex
   * @webBrief Using the <b>beginShape()</b> and <b>endShape()</b> functions allow creating more
   *           complex forms
   * @param kind Either POINTS, LINES, TRIANGLES, TRIANGLE_FAN, TRIANGLE_STRIP, QUADS, or
   *             QUAD_STRIP
   * @see PShape
  ...
  /**
   * Sets whether the upcoming vertex is part of an edge. Equivalent to glEdgeFlag(), for people
   ...
   * <b>normal()</b> specifies a vector perpendicular to a shape's surface which, in turn,
   * determines how lighting affects it. Processing attempts to automatically assign normals to
   * shapes, but since that's imperfect, this is a better option when you want more control. This
   * function is identical to <b>glNormal3f()</b> in OpenGL.
   *
  ...
  /**
   *
   * Sets the coordinate space for texture mapping. The default mode is <b>IMAGE</b>, which refers
   * to the actual coordinates of the image. <b>NORMAL</b> refers to a normalized space of values
   * ranging from 0 to 1. This function only works with the P2D and P3D renderers.<br />
   * <br />
   * With <b>IMAGE</b>, if an image is 100 x 200 pixels, mapping the image onto the entire size of
   * a quad would require the points (0,0) (100, 0) (100,200) (0,200). The same mapping in
   * <b>NORMAL</b> is (0,0) (1,0) (1,1) (0,1).
   *
  ...
  /**
   * Defines if textures repeat or draw once within a texture map. The two parameters are CLAMP
   * (the default behavior) and REPEAT. This function only works with the P2D and P3D renderers.
   *
   * @webref image:textures
  ...
  /**
   * Sets a texture to be applied to vertex points. The <b>texture()</b> function must be called
   * between <b>beginShape()</b> and <b>endShape()</b> and before any calls to <b>vertex()</b>.
   * This function only works with the P2D and P3D renderers.
   * <p/>
   * When textures are in use, the fill color is ignored. Instead, use <b>tint()</b> to specify
   * the color of the texture as it is applied to the shape.
   *
   * @webref image:textures
  ...
  /**
   * Used by renderer subclasses or PShape to efficiently pass in already formatted vertex
   * information.
   * 
  ...
  /**
   *
   * All shapes are constructed by connecting a series of vertices. <b>vertex()</b> is used to
   * specify the vertex coordinates for points, lines, triangles, quads, and polygons. It is used
   * exclusively within the <b>beginShape()</b> and <b>endShape()</b> functions. <br />
   * <br />
   * Drawing a vertex in 3D using the <b>z</b> parameter requires the P3D parameter in combination
   * with size, as shown in the above example. <br />
   * <br />
   * This function is also used to map a texture onto geometry. The <b>texture()</b> function
   * declares the texture to apply to the geometry and the <b>u</b> and <b>v</b> coordinates set
   * define the mapping of this texture to the form. By default, the coordinates used for <b>u</b>
   * and <b>v</b> are specified in relation to the image's size in pixels, but this relation can
   * be changed with <b>textureMode()</b>.
   *
   * @webref shape:vertex
  ...
  /**
   * Use the <b>beginContour()</b> and <b>endContour()</b> function to create negative shapes
   * within shapes such as the center of the letter "O". <b>beginContour()</b> begins recording
   * vertices for the shape and <b>endContour()</b> stops recording. The vertices that define a
   * negative shape must "wind" in the opposite direction from the exterior shape. First draw
   * vertices for the exterior shape in clockwise order, then for internal shapes, draw vertices
   * counterclockwise.<br />
   * <br />
   * These functions can only be used within a <b>beginShape()</b>/<b>endShape()</b> pair and
   * transformations such as <b>translate()</b>, <b>rotate()</b>, and <b>scale()</b> do not work
   * within a <b>beginContour()</b>/<b>endContour()</b> pair. It is also not possible to use other
   * shapes, such as <b>ellipse()</b> or <b>rect()</b> within.
   *
   * @webref shape:vertex
  ...
  /**
   *
   * The <b>endShape()</b> function is the companion to <b>beginShape()</b> and may only be called
   * after <b>beginShape()</b>. When <b>endshape()</b> is called, all the image data defined since
   * the previous call to <b>beginShape()</b> is written into the image buffer. The constant CLOSE
   * as the value for the MODE parameter to close the shape (to connect the beginning and the
   * end).
   *
   * @webref shape:vertex
  ...
  /**
   * Loads geometry into a variable of type <b>PShape</b>. SVG and OBJ files may be loaded. To
   * load correctly, the file must be located in the data directory of the current sketch. In most
   * cases, <b>loadShape()</b> should be used inside <b>setup()</b> because loading shapes inside
   * <b>draw()</b> will reduce the speed of a sketch.<br />
   * <br />
   * Alternatively, the file maybe be loaded from anywhere on the local computer using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows), or the
   * filename parameter can be a URL for a file found on a network.<br />
   ...
   * If the file is not available or an error occurs, <b>null</b> will be returned and an error
   * message will be printed to the console. The error message does not halt the program, however
   * the null value may cause a NullPointerException if your code does not check whether the value
   * returned is null.<br />
   *
   * @webref shape
  ...
  /**
   * The <b>createShape()</b> function is used to define a new shape. Once created, this shape can
   * be drawn with the <b>shape()</b> function. The basic way to use the function defines new
   * primitive shapes. One of the following parameters are used as the first parameter:
   * <b>ELLIPSE</b>, <b>RECT</b>, <b>ARC</b>, <b>TRIANGLE</b>, <b>SPHERE</b>, <b>BOX</b>,
   * <b>QUAD</b>, or <b>LINE</b>. The parameters for each of these different shapes are the same
   * as their corresponding functions: <b>ellipse()</b>, <b>rect()</b>, <b>arc()</b>,
   * <b>triangle()</b>, <b>sphere()</b>, <b>box()</b>, <b>quad()</b>, and <b>line()</b>. The first
   * example above clarifies how this works.<br />
   * <br />
   * Custom, unique shapes can be made by using <b>createShape()</b> without a parameter. After
   * the shape is started, the drawing attributes and geometry can be set directly to the shape
   * within the <b>beginShape()</b> and <b>endShape()</b> methods. See the second example above
   * for specifics, and the reference for <b>beginShape()</b> for all of its options.<br />
   * <br />
   * The <b>createShape()</b> function can also be used to make a complex shape made of other
   * shapes. This is called a "group" and it's created by using the parameter <b>GROUP</b> as the
   * first parameter. See the fourth example above to see how it works.<br />
   * <br />
   * After using <b>createShape()</b>, stroke and fill color can be set by calling methods like
   * <b>setFill()</b> and <b>setStroke()</b>, as seen in the examples above. The complete list of
   * methods and fields for the PShape class are in the
   * <a href="http://processing.github.io/processing-javadocs/core/">Processing Javadoc</a>.
   *
   * @webref shape
  ...
  /**
   * Loads a shader into the <b>PShader</b> object. The shader file must be loaded in the sketch's
   * "data" folder/directory to load correctly. Shaders are compatible with the P2D and P3D
   * renderers, but not with the default renderer.<br />
   * <br />
   * Alternatively, the file maybe be loaded from anywhere on the local computer using an absolute
   * path (something that starts with / on Unix and Linux, or a drive letter on Windows), or the
   * filename parameter can be a URL for a file found on a network.<br />
  ...
  /**
   *
   * Applies the shader specified by the parameters. It's compatible with the P2D and P3D
  ...
  /**
   * Restores the default shaders. Code that runs after <b>resetShader()</b> will not be affected
   * by previously defined shaders.
   *
   * @webref rendering:shaders
   ...
   * boundaries are drawn based on the state of the <b>imageMode()</b> function, either CORNER,
   * CORNERS, or CENTER.
   *
   * @webref rendering
   * @webBrief Limits the rendering to the boundaries of a rectangle defined by the parameters
  ...
  /**
   *
   * Blends the pixels in the display window according to a defined mode. There is a choice of the
   * following modes to blend the source pixels (A) with the ones of pixels already in the display
   * window (B). Each pixel's final color is the result of applying one of the blend modes with
   * each channel of (A) and (B) independently. The red channel is compared with red, green with
   * green, and blue with blue.<br />
   * <br />
   ...
   * <br />
   * REPLACE - the pixels entirely replace the others and don't utilize alpha (transparency)
   * values<br />
   * <br />
   * We recommend using <b>blendMode()</b> and not the previous <b>blend()</b> function. However,
   * unlike <b>blend()</b>, the <b>blendMode()</b> function does not support the following:
   * HARD_LIGHT, SOFT_LIGHT, OVERLAY, DODGE, BURN. On older hardware, the LIGHTEST, DARKEST, and
  ...
  /**
   *
   * Specifies vertex coordinates for Bézier curves. Each call to <b>bezierVertex()</b> defines
   * the position of two control points and one anchor point of a Bézier curve, adding a new
   * segment to a line or shape. The first time <b>bezierVertex()</b> is used within a
   * <b>beginShape()</b> call, it must be prefaced with a call to <b>vertex()</b> to set the first
   * anchor point. This function must be used between <b>beginShape()</b> and <b>endShape()</b>
   * and only when there is no MODE parameter specified to <b>beginShape()</b>. Using the 3D
   ...
   * @see PGraphics#vertex(float, float, float, float, float)
   * @see PGraphics#quadraticVertex(float, float, float, float, float, float)
   * @see PGraphics#bezier(float, float, float, float, float, float, float, float, float, float,
   *      float, float)
   */
   ...
   * <b>quadraticVertex()</b> defines the position of one control point and one anchor point of a
   * Bézier curve, adding a new segment to a line or shape. The first time
   * <b>quadraticVertex()</b> is used within a <b>beginShape()</b> call, it must be prefaced with
   * a call to <b>vertex()</b> to set the first anchor point. This function must be used between
   * <b>beginShape()</b> and <b>endShape()</b> and only when there is no MODE parameter specified
   * to <b>beginShape()</b>. Using the 3D version requires rendering with P3D (see the Environment
   ...
   * <b>beginShape()</b> and <b>endShape()</b> and only when there is no MODE parameter specified
   * to <b>beginShape()</b>. The first and last points in a series of <b>curveVertex()</b> lines
   * will be used to guide the beginning and end of the curve. A minimum of four points is
   * required to draw a tiny curve between the second and third points. Adding a fifth point with
   * <b>curveVertex()</b> will draw the curve between the second, third, and fourth points. The
   * <b>curveVertex()</b> function is an implementation of Catmull-Rom splines. Using the 3D
   * version requires rendering with P3D (see the Environment reference for more information).
   *
   * @webref shape:vertex
   ...
   * @see PGraphics#curve(float, float, float, float, float, float, float, float, float, float,
   *      float, float)
   * @see PGraphics#beginShape(int)
   * @see PGraphics#endShape(int)
   * @see PGraphics#vertex(float, float, float, float, float)
  ...
  /**
   *
   * Draws a point, a coordinate in space at the dimension of one pixel. The first parameter is
   * the horizontal value for the point, the second value is the vertical value for the point, and
   * the optional third value is the depth value. Drawing this shape in 3D with the <b>z</b>
   * parameter requires the P3D parameter in combination with <b>size()</b> as shown in the above
   * example. <br />
   * <br />
   ...
   * <b>strokeCap(PROJECT)</b>. Points are invisible with <b>strokeCap(SQUARE)</b> (no cap).
   * <br />
   * <br />
   * Using point() with strokeWeight(1) or smaller may draw nothing to the screen, depending on
   * the graphics settings of the computer. Workarounds include setting the pixel using
   * <b>set()</s> or drawing the point using either <b>circle()</b> or <b>square()</b>.
   *
  ...
  /**
   *
   * Draws a line (a direct path between two points) to the screen. The version of <b>line()</b>
   * with four parameters draws the line in 2D. To color a line, use the <b>stroke()</b> function.
   * A line cannot be filled, therefore the <b>fill()</b> function will not affect the color of a
   * line. 2D lines are drawn with a width of one pixel by default, but this can be changed with
   * the <b>strokeWeight()</b> function. The version with six parameters allows the line to be
   * placed anywhere within XYZ space. Drawing this shape in 3D with the <b>z</b> parameter
   ...
   * first point, the middle two arguments specify the second point, and the last two arguments
  ...
  /**
   *
   * Modifies the location from which rectangles are drawn by changing the way in which parameters
   * given to <b>rect()</b> are interpreted.<br />
   * <br />
   * The default mode is <b>rectMode(CORNER)</b>, which interprets the first two parameters of
   * <b>rect()</b> as the upper-left corner of the shape, while the third and fourth parameters
   * are its width and height.<br />
   * <br />
   * <b>rectMode(CORNERS)</b> interprets the first two parameters of <b>rect()</b> as the location
   * of one corner, and the third and fourth parameters as the location of the opposite
   * corner.<br />
   * <br />
   * <b>rectMode(CENTER)</b> interprets the first two parameters of <b>rect()</b> as the shape's
   * center point, while the third and fourth parameters are its width and height.<br />
   * <br />
   * <b>rectMode(RADIUS)</b> also uses the first two parameters of <b>rect()</b> as the shape's
   * center point, but uses the third and fourth parameters to specify half of the shape's width
   * and height.<br />
   * <br />
   ...
   * degrees. By default, the first two parameters set the location of the upper-left corner, the
   * third sets the width, and the fourth sets the height. The way these parameters are
   * interpreted, however, may be changed with the <b>rectMode()</b> function.<br />
   * <br />
   * To draw a rounded rectangle, add a fifth parameter, which is used as the radius value for all
   * four corners.<br />
   * <br />
   * To use a different radius value for each corner, include eight parameters. When using eight
   * parameters, the latter four set the radius of the arc at each corner separately, starting
   * with the top-left corner and moving clockwise around the rectangle.
   *
   * @webref shape:2d primitives
   ...
   * degrees and each side is the same length. By default, the first two parameters set the
   * location of the upper-left corner, the third sets the width and height. The way these
   * parameters are interpreted, however, may be changed with the <b>rectMode()</b> function.
   *
   * @webref shape:2d primitives
  ...
  /**
   *
   * Modifies the location from which ellipses are drawn by changing the way in which parameters
   * given to <b>ellipse()</b> are interpreted.<br />
   * <br />
   * The default mode is <b>ellipseMode(CENTER)</b>, which interprets the first two parameters of
   * <b>ellipse()</b> as the shape's center point, while the third and fourth parameters are its
   * width and height.<br />
   * <br />
   * <b>ellipseMode(RADIUS)</b> also uses the first two parameters of <b>ellipse()</b> as the
   * shape's center point, but uses the third and fourth parameters to specify half of the shape's
   * width and height.<br />
   * <br />
   * <b>ellipseMode(CORNER)</b> interprets the first two parameters of <b>ellipse()</b> as the
   * upper-left corner of the shape, while the third and fourth parameters are its width and
   * height.<br />
   * <br />
   * <b>ellipseMode(CORNERS)</b> interprets the first two parameters of <b>ellipse()</b> as the
   * location of one corner of the ellipse's bounding box, and the third and fourth parameters as
   * the location of the opposite corner.<br />
   * <br />
   * The parameter must be written in ALL CAPS because Processing is a case-sensitive language.
   *
   * @webref shape:attributes
   * @webBrief The origin of the ellipse is modified by the <b>ellipseMode()</b> function
  ...
  /**
   *
   * Draws an ellipse (oval) to the screen. An ellipse with equal width and height is a circle. By
   * default, the first two parameters set the location, and the third and fourth parameters set
   * the shape's width and height. The origin may be changed with the <b>ellipseMode()</b>
   * function.
   *
   * @webref shape:2d primitives
  ...
  /**
   *
   * Draws an arc to the screen. Arcs are drawn along the outer edge of an ellipse defined by the
   * <b>a</b>, <b>b</b>, <b>c</b>, and <b>d</b> parameters. The origin of the arc's ellipse may be
   * changed with the <b>ellipseMode()</b> function. Use the <b>start</b> and <b>stop</b>
   * parameters to specify the angles (in radians) at which to draw the arc. The start/stop values
   * must be in clockwise order. <br />
   * <br />
   ...
   * CHORD. The default mode is the OPEN stroke with a PIE fill. <br />
   * <br />
   * In some cases, the <b>arc()</b> function isn't accurate enough for smooth drawing. For
   * example, the shape may jitter on screen when rotating slowly. If you're having an issue with
   * how arcs are rendered, you'll need to draw the arc yourself with
   * <b>beginShape()</b>/<b>endShape()</b> or a <b>PShape</b>.
   *
   * @webref shape:2d primitives
  ...
  /**
   * A box is an extruded <b>rectangle</b>. A box with equal dimension on all sides is a cube.
   *
  ...
  /**
   *
   * Controls the detail used to render a sphere by adjusting the number of vertices of the sphere
   * mesh. The default resolution is 30, which creates a fairly detailed sphere definition with
   * vertices every 360/30 = 12 degrees. If you're going to render a great number of spheres per
   * frame, it is advised to reduce the level of detail using this function. The setting stays
   * active until <b>sphereDetail()</b> is called again with a new parameter and so should
   * <i>not</i> be called prior to every <b>sphere()</b> statement, unless you wish to render
   * spheres with different settings, e.g. using less detail for smaller spheres or ones further
   * away from the camera. To control the detail of the horizontal and vertical resolution
   * independently, use the version of the functions with two parameters.
   *
   * <h3>Advanced</h3> Code for sphereDetail() submitted by toxi [031031]. Code for enhanced u/v
   * version from davbol [080801].
   *
   * @param res number of segments (minimum 3) used per full circle revolution
   * @webref shape:3d primitives
   * @webBrief Controls the detail used to render a sphere by adjusting the number of vertices of
   *           the sphere mesh
   * @see PGraphics#sphere(float)
   */
   ...
   * <P>
   * cache all the points of the sphere in a static array top and bottom are just a bunch of
   * triangles that land in the center point
   * <P>
   * sphere is a series of concentric circles who radii vary along the shape, based on, err... cos
   * or something
   * 
   * <PRE>
  ...
  /**
   *
   * Evaluates the Bezier at point t for points a, b, c, d. The parameter t varies between 0 and
   * 1, a and d are points on the curve, and b and c are the control points. This can be done once
   * with the x coordinates and a second time with the y coordinates to get the location of a
   * Bézier curve at t.
   *
  ...
  /**
   * Calculates the tangent of a point on a Bézier curve. There is a good definition of
   * <a href="http://en.wikipedia.org/wiki/Tangent" target="new"><em>tangent</em> on
   * Wikipedia</a>.
   *
   * <h3>Advanced</h3> Code submitted by Dave Bollinger (davbol) for release 0136.
   *
   * @webref shape:curves
  ...
  /**
   * Sets the resolution at which Bézier curves display. The default value is 20. This function is
   * only useful when using the <b>P2D</b> or <b>P3D</b> renderer; the default (JAVA2D) renderer
   ...
   * control points. The first two parameters specify the first anchor point and the last two
   * parameters specify the other anchor point. The middle parameters specify the control points
   * which define the shape of the curve. The curves were developed by French engineer Pierre
   * Bezier. Using the 3D version requires rendering with P3D (see the Environment reference for
   * more information).
   *
   * <h3>Advanced</h3> Draw a cubic Bézier curve. The first and last points are the on-curve
   * points. The middle two are the 'control' points, or 'handles' in an application like
   * Illustrator.
   * <P>
   * Identical to typing:
   * 
  ...
  /**
   * Evaluates the curve at point <b>t</b> for points <b>a</b>, <b>b</b>, <b>c</b>, <b>d</b>. The
   * parameter <b>t</b> may range from 0 (the start of the curve) and 1 (the end of the curve).
   * <b>a</b> and <b>d</b> are the control points, and <b>b</b> and <b>c</b> are points on the
   * curve. As seen in the example above, this can be used once with the <b>x</b> coordinates and
   * a second time with the <b>y</b> coordinates to get the location of a curve at <b>t</b>.
   *
   * @webref shape:curves
   ...
   * <em><a href="http://en.wikipedia.org/wiki/Tangent" target="new">tangent</em> on
   * Wikipedia</a>.
   *
  ...
  /**
   * Sets the resolution at which curves display. The default value is 20. This function is only
   * useful when using the P3D renderer as the default P2D renderer does not use this information.
   *
   * @webref shape:curves
  ...
  /**
   *
   * Modifies the quality of forms created with <b>curve()</b> and <b>curveVertex()</b>. The
   * parameter <b>tightness</b> determines how the curve fits to the vertex points. The value 0.0
   * is the default value for <b>tightness</b> (this value defines the curves to be Catmull-Rom
   * splines) and the value 1.0 connects all the points with straight lines. Values within the
   * range -5.0 and 5.0 will deform the curves but will leave them recognizable and as values
   * increase in magnitude, they will continue to deform.
   *
   * @webref shape:curves
   * @webBrief Modifies the quality of forms created with <b>curve()</b> and <b>curveVertex()</b>
   ...
   * parameters specify the start and stop of the curve. Longer curves can be created by putting a
   * series of <b>curve()</b> functions together or using <b>curveVertex()</b>. An additional
   * function called <b>curveTightness()</b> provides control for the visual quality of the curve.
   * The <b>curve()</b> function is an implementation of Catmull-Rom splines. Using the 3D version
   * requires rendering with P3D (see the Environment reference for more information).
   *
   * <h3>Advanced</h3> As of revision 0070, this function no longer doubles the first and last
   * points. The curves are a bit more boring, but it's more mathematically correct, and properly
   * mirrored in curvePoint().
   * <p/>
  ...
  /**
   *
   * Modifies the location from which images are drawn by changing the way in which parameters
   * given to <b>image()</b> are interpreted.<br />
   * <br />
   * The default mode is <b>imageMode(CORNER)</b>, which interprets the second and third
   * parameters of <b>image()</b> as the upper-left corner of the image. If two additional
   * parameters are specified, they are used to set the image's width and height.<br />
   * <br />
   * <b>imageMode(CORNERS)</b> interprets the second and third parameters of <b>image()</b> as the
   * location of one corner, and the fourth and fifth parameters as the opposite corner.<br />
   * <br />
   * <b>imageMode(CENTER)</b> interprets the second and third parameters of <b>image()</b> as the
   * image's center point. If two additional parameters are specified, they are used to set the
   * image's width and height.<br />
   * <br />
   * The parameter must be written in ALL CAPS because Processing is a case-sensitive language.
   *
   * @webref image:loading & displaying
  ...
  /**
   *
   * The <b>image()</b> function draws an image to the display window. Images must be in the
   * sketch's "data" directory to load correctly. Select "Add file..." from the "Sketch" menu to
   * add the image to the data directory, or just drag the image file onto the sketch window.
   * Processing currently works with GIF, JPEG, and PNG images. <br />
   * <br />
   * The <b>img</b> parameter specifies the image to display and by default the <b>a</b> and
   * <b>b</b> parameters define the location of its upper-left corner. The image is displayed at
   * its original size unless the <b>c</b> and <b>d</b> parameters specify a different size. The
   * <b>imageMode()</b> function can be used to change the way these parameters draw the
   * image.<br />
   * <br />
   * The color of an image may be modified with the <b>tint()</b> function. This function will
   * maintain transparency for GIF and PNG images.
   *
   * <h3>Advanced</h3> Starting with release 0124, when using the default (JAVA2D) renderer,
  ...
  /**
   * Draw an image(), also specifying u/v coordinates. In this method, the u, v coordinates are
   * always based on image space location, regardless of the current textureMode().
   *
   * @nowebref
   */
  ...
  /**
   *
   * Modifies the location from which shapes draw. The default mode is <b>shapeMode(CORNER)</b>,
   * which specifies the location to be the upper left corner of the shape and uses the third and
   * fourth parameters of <b>shape()</b> to specify the width and height. The syntax
   * <b>shapeMode(CORNERS)</b> uses the first and second parameters of <b>shape()</b> to set the
   * location of one corner and uses the third and fourth parameters to set the opposite corner.
   * The syntax <b>shapeMode(CENTER)</b> draws the shape from its center point and uses the third
   * and forth parameters of <b>shape()</b> to specify the width and height. The parameter must be
   * written in "ALL CAPS" because Processing is a case-sensitive language.
   *
  ...
  /**
   * Draws shapes to the display window. Shapes must be in the sketch's "data" directory to load
   * correctly. Select "Add file..." from the "Sketch" menu to add the shape. Processing currently
   * works with SVG, OBJ, and custom-created shapes. The <b>shape</b> parameter specifies the
   * shape to display and the coordinate parameters define the location of the shape from its
   * upper-left corner. The shape is displayed at its original size unless the <b>c</b> and
   * <b>d</b> parameters specify a different size. The <b>shapeMode()</b> function can be used to
   ...
   * display characteristics of the letters in relation to the values for the <b>x</b> and
   * <b>y</b> parameters of the <b>text()</b> function. <br />
   * <br />
   * An optional second parameter can be used to vertically align the text. BASELINE is the
   * default, and the vertical alignment will be reset to BASELINE if the second parameter is not
   * used. The TOP and CENTER parameters are straightforward. The BOTTOM parameter offsets the
   * line based on the current <b>textDescent()</b>. For multiple lines, the final line will be
   * aligned to the bottom, with the previous lines appearing above it. <br />
   * <br />
   * When using <b>text()</b> with width and height parameters, BASELINE is ignored, and treated
   * as TOP. (Otherwise, text would by default draw outside the box, since BASELINE is the default
   * setting. BASELINE is not a useful drawing mode for text drawn in a rectangle.) <br />
   * <br />
   * The vertical alignment is based on the value of <b>textAscent()</b>, which many fonts do not
   * specify correctly. It may be necessary to use a hack and offset by a few pixels by hand so
   * that the offset looks correct. To do this as less of a hack, use some percentage of
   * <b>textAscent()</b> or <b>textDescent()</b> so that the hack works even if you change the
   * size of the font.
   *
   ...
   * determining the height of the font above the baseline.
   *
  ...
  /**
   * Returns descent of the current font at its current size. This information is useful for
   * determining the height of the font below the baseline.
   *
   * @webref typography:metrics
  ...
  /**
   *
   * Sets the current font that will be drawn with the <b>text()</b> function. Fonts must be
   * created for Processing with <b>createFont()</b> or loaded with <b>loadFont()</b> before they
   * can be used. The font set through <b>textFont()</b> will be used in all subsequent calls to
   * the <b>text()</b> function. If no <b>size</b> parameter is specified, the font size defaults
   * to the original size (the size in which it was created with the "Create Font..." tool)
   * overriding any previous calls to <b>textFont()</b> or <b>textSize()</b>.<br />
   * <br />
   * When fonts are rendered as an image texture (as is the case with the P2D and P3D renderers as
   * well as with <b>loadFont()</b> and vlw files), you should create fonts at the sizes that will
   * be used most commonly. Using <b>textFont()</b> without the size parameter will result in the
   * cleanest type.<br />
   * <br />
   *
   *
   * @webref typography:loading & displaying
   * @webBrief Sets the current font that will be drawn with the <b>text()</b> function
  ...
  /**
   * Sets the spacing between lines of text in units of pixels. This setting will be used in all
   * subsequent calls to the <b>text()</b> function. Note, however, that the leading is reset by
   * <b>textSize()</b>. For example, if the leading is set to 20 with <b>textLeading(20)</b>, then
   * if <b>textSize(48)</b> is run at a later point, the leading will be reset to the default for
   * the text size of 48.
   *
   * @webref typography:attributes
  ...
  /**
   * Sets the way text draws to the screen, either as texture maps or as vector geometry. The
   * default <b>textMode(MODEL)</b>, uses textures to render the fonts. The <b>textMode(SHAPE)</b>
   * mode draws text using the glyph outlines of individual characters rather than as textures.
   * This mode is only supported with the <b>PDF</b> and <b>P3D</b> renderer settings. With the
   * <b>PDF</b> renderer, you must call <b>textMode(SHAPE)</b> before any other drawing occurs. If
   * the outlines are not available, then <b>textMode(SHAPE)</b> will be ignored and
   * <b>textMode(MODEL)</b> will be used instead.<br />
   * <br />
   * The <b>textMode(SHAPE)</b> option in <b>P3D</b> can be combined with <b>beginRaw()</b> to
   * write vector-accurate text to 2D and 3D output files, for instance <b>DXF</b> or <b>PDF</b>.
   * The <b>SHAPE</b> mode is not currently optimized for <b>P3D</b>, so if recording shape data,
   * use <b>textMode(MODEL)</b> until you're ready to capture the geometry with <b>beginRaw()</b>.
   *
   * @webref typography:attributes
  ...
  /**
   * Sets the current font size. This size will be used in all subsequent calls to the
   * <b>text()</b> function. Font size is measured in units of pixels.
   *
   * @webref typography:attributes
   ...
   * unless a font is set with the <b>textFont()</b> function and a default size will be used
   * unless a font is set with <b>textSize()</b>. Change the color of the text with the
   * <b>fill()</b> function. The text displays in relation to the <b>textAlign()</b> function,
   * which gives the option to draw to the left, right, and center of the coordinates.<br />
   * <br />
   * The <b>x2</b> and <b>y2</b> parameters define a rectangular area to display within and may
   * only be used with string data. When these parameters are specified, they are interpreted
   * based on the current <b>rectMode()</b> setting. Text that does not fit completely within the
   * rectangle specified will not be drawn to the screen.<br />
   * <br />
   * Note that Processing now lets you call <b>text()</b> without first specifying a PFont with
   * <b>textFont()</b>. In that case, a generic sans-serif font will be used instead. (See the
   * third example above.)
   *
   * @webref typography:loading & displaying
   ...
   * ascii 10) are honored, but \r (carriage return, Windows and Mac OS) are ignored.
   */
  ...
  /**
   * <h3>Advanced</h3> Method to draw text from an array of chars. This method will usually be
   * more efficient than drawing from a String object, because the String will not be converted to
   * a char array before drawing.
   * 
  ...
  /**
   * <h3>Advanced</h3> Draw text in a box that is constrained to a particular size. The current
   * rectMode() determines what the coordinates mean (whether x1/y1/x2/y2 or x/y/w/h).
   * <P/>
   * Note that the x,y coords of the start of the box will align with the *ascent* of the text,
   * not the baseline, as is the case for the other text() functions.
   * <P/>
   * Newlines that are \n (Unix newline or linefeed char, ascii 10) are honored, and \r (carriage
  ...
  /**
   * This does a basic number formatting, to avoid the generally ugly appearance of printing
   * floats. Users who want more control should use their own nf() command, or if they want the
   * long, ugly version of float, use String.valueOf() to convert the float to a String first.
   *
  ...
  /**
   *
   * The <b>push()</b> function saves the current drawing style settings and transformations,
   * while <b>pop()</b> restores these settings. Note that these functions are always used
   * together. They allow you to change the style and transformation settings and later return to
   * what you had. When a new state is started with push(), it builds on the current style and
   * transform information.<br />
   * <br />
   * <b>push()</b> stores information related to the current transformation state and style
   * settings controlled by the following functions: <b>rotate()</b>, <b>translate()</b>,
   * <b>scale()</b>, <b>fill()</b>, <b>stroke()</b>, <b>tint()</b>, <b>strokeWeight()</b>,
   * <b>strokeCap()</b>, <b>strokeJoin()</b>, <b>imageMode()</b>, <b>rectMode()</b>,
   * <b>ellipseMode()</b>, <b>colorMode()</b>, <b>textAlign()</b>, <b>textFont()</b>,
   * <b>textMode()</b>, <b>textSize()</b>, <b>textLeading()</b>.<br />
   * <br />
   * The <b>push()</b> and <b>pop()</b> functions were added with Processing 3.5. They can be used
   * in place of <b>pushMatrix()</b>, <b>popMatrix()</b>, <b>pushStyles()</b>, and
   * <b>popStyles()</b>. The difference is that push() and pop() control both the transformations
   * (rotate, scale, translate) and the drawing styles at the same time.
   *
   * @webref structure
   * @webBrief The <b>push()</b> function saves the current drawing style settings and
   *           transformations, while <b>pop()</b> restores these settings
   * @see PGraphics#pop()
   */
  public void push() {
  ...
  /**
   *
   * The <b>pop()</b> function restores the previous drawing style settings and transformations
   * after <b>push()</b> has changed them. Note that these functions are always used together.
   * They allow you to change the style and transformation settings and later return to what you
   * had. When a new state is started with push(), it builds on the current style and transform
   * information. <br />
   * <br />
   * <b>push()</b> stores information related to the current transformation state and style
   * settings controlled by the following functions: <b>rotate()</b>, <b>translate()</b>,
   ...
   *
   * @webref structure
   * @webBrief The <b>pop()</b> function restores the previous drawing style settings and
   *           transformations after <b>push()</b> has changed them
   * @see PGraphics#push()
   */
  public void pop() {
   ...
   * <b>pushMatrix()</b> and <b>popMatrix()</b> requires understanding the concept of a matrix
   * stack. The <b>pushMatrix()</b> function saves the current coordinate system to the stack and
   * <b>popMatrix()</b> restores the prior coordinate system. <b>pushMatrix()</b> and
   * <b>popMatrix()</b> are used in conjunction with the other transformation functions and may be
  ...
  /**
   * Pops the current transformation matrix off the matrix stack. Understanding pushing and
   * popping requires understanding the concept of a matrix stack. The <b>pushMatrix()</b>
   * function saves the current coordinate system to the stack and <b>popMatrix()</b> restores the
   * prior coordinate system. <b>pushMatrix()</b> and <b>popMatrix()</b> are used in conjunction
   * with the other transformation functions and may be embedded to control the scope of the
   * transformations.
   *
   * @webref transform
  ...
  /**
   * Specifies an amount to displace objects within the display window. The <b>x</b> parameter
   * specifies left/right translation, the <b>y</b> parameter specifies up/down translation, and
   * the <b>z</b> parameter specifies translations toward/away from the screen. Using this
   * function with the <b>z</b> parameter requires using P3D as a parameter in combination with
   * size as shown in the above example.
   * <p/>
   * Transformations are cumulative and apply to everything that happens after and subsequent
   * calls to the function accumulates the effect. For example, calling <b>translate(50, 0)</b>
   * and then <b>translate(20, 0)</b> is the same as <b>translate(70, 0)</b>. If
   * <b>translate()</b> is called within <b>draw()</b>, the transformation is reset when the loop
   * begins again. This function can be further controlled by using <b>pushMatrix()</b> and
   * <b>popMatrix()</b>.
   *
   * @webref transform
  ...
  /**
   *
   * Rotates a shape the amount specified by the <b>angle</b> parameter. Angles should be
   * specified in radians (values from 0 to TWO_PI) or converted to radians with the
   * <b>radians()</b> function. <br/>
   * <br/>
   ...
   * <b>rotate(HALF_PI)</b> and then <b>rotate(HALF_PI)</b> is the same as <b>rotate(PI)</b>. All
   * transformations are reset when <b>draw()</b> begins again. <br/>
   * <br/>
   * Technically, <b>rotate()</b> multiplies the current transformation matrix by a rotation
   * matrix. This function can be further controlled by the <b>pushMatrix()</b> and
   * <b>popMatrix()</b>.
   *
   * @webref transform
  ...
  /**
   *
   * Rotates a shape around the x-axis the amount specified by the <b>angle</b> parameter. Angles
   * should be specified in radians (values from 0 to PI*2) or converted to radians with the
   * <b>radians()</b> function. Objects are always rotated around their relative position to the
   * origin and positive numbers rotate objects in a counterclockwise direction. Transformations
   * apply to everything that happens after and subsequent calls to the function accumulates the
   * effect. For example, calling <b>rotateX(PI/2)</b> and then <b>rotateX(PI/2)</b> is the same
   * as <b>rotateX(PI)</b>. If <b>rotateX()</b> is called within the <b>draw()</b>, the
   * transformation is reset when the loop begins again. This function requires using P3D as a
   * third parameter to <b>size()</b> as shown in the example above.
   *
   * @webref transform
   * @webBrief Rotates a shape around the x-axis the amount specified by the <b>angle</b>
   *           parameter
   * @param angle angle of rotation specified in radians
   * @see PGraphics#popMatrix()
   * @see PGraphics#pushMatrix()
  ...
  /**
   *
   * Rotates a shape around the y-axis the amount specified by the <b>angle</b> parameter. Angles
   * should be specified in radians (values from 0 to PI*2) or converted to radians with the
   * <b>radians()</b> function. Objects are always rotated around their relative position to the
   * origin and positive numbers rotate objects in a counterclockwise direction. Transformations
   * apply to everything that happens after and subsequent calls to the function accumulates the
   * effect. For example, calling <b>rotateY(PI/2)</b> and then <b>rotateY(PI/2)</b> is the same
   * as <b>rotateY(PI)</b>. If <b>rotateY()</b> is called within the <b>draw()</b>, the
   * transformation is reset when the loop begins again. This function requires using P3D as a
   * third parameter to <b>size()</b> as shown in the examples above.
   *
   * @webref transform
   * @webBrief Rotates a shape around the y-axis the amount specified by the <b>angle</b>
   *           parameter
   * @param angle angle of rotation specified in radians
   * @see PGraphics#popMatrix()
   * @see PGraphics#pushMatrix()
  ...
  /**
   *
   * Rotates a shape around the z-axis the amount specified by the <b>angle</b> parameter. Angles
   * should be specified in radians (values from 0 to PI*2) or converted to radians with the
   * <b>radians()</b> function. Objects are always rotated around their relative position to the
   * origin and positive numbers rotate objects in a counterclockwise direction. Transformations
   * apply to everything that happens after and subsequent calls to the function accumulates the
   * effect. For example, calling <b>rotateZ(PI/2)</b> and then <b>rotateZ(PI/2)</b> is the same
   * as <b>rotateZ(PI)</b>. If <b>rotateZ()</b> is called within the <b>draw()</b>, the
   * transformation is reset when the loop begins again. This function requires using P3D as a
   * third parameter to <b>size()</b> as shown in the examples above.
   *
   * @webref transform
   * @webBrief Rotates a shape around the z-axis the amount specified by the <b>angle</b>
   *           parameter
   * @param angle angle of rotation specified in radians
   * @see PGraphics#popMatrix()
   * @see PGraphics#pushMatrix()
  ...
  /**
   *
   * Increases or decreases the size of a shape by expanding and contracting vertices. Objects
   * always scale from their relative origin to the coordinate system. Scale values are specified
   * as decimal percentages. For example, the function call <b>scale(2.0)</b> increases the
   * dimension of a shape by 200%.<br />
   * <br />
   ...
   * same as <b>scale(3.0)</b>. If <b>scale()</b> is called within <b>draw()</b>, the
   * transformation is reset when the loop begins again. Using this function with the <b>z</b>
   * parameter requires using P3D as a parameter for <b>size()</b>, as shown in the third example
   * above. This function can be further controlled with <b>pushMatrix()</b> and
   * <b>popMatrix()</b>.
   *
   * @webref transform
   * @webBrief Increases or decreases the size of a shape by expanding and contracting vertices
   ...
   * <p/>
   * Not recommended for use in 3D, because the z-dimension is just scaled by 1, since there's no
   * way to know what else to scale it by.
   *
  ...
  /**
   *
   * Shears a shape around the x-axis the amount specified by the <b>angle</b> parameter. Angles
   * should be specified in radians (values from 0 to PI*2) or converted to radians with the
   * <b>radians()</b> function. Objects are always sheared around their relative position to the
   * origin and positive numbers shear objects in a clockwise direction. Transformations apply to
   * everything that happens after and subsequent calls to the function accumulates the effect.
   * For example, calling <b>shearX(PI/2)</b> and then <b>shearX(PI/2)</b> is the same as
   * <b>shearX(PI)</b>. If <b>shearX()</b> is called within the <b>draw()</b>, the transformation
   * is reset when the loop begins again. <br/>
   * <br/>
   * Technically, <b>shearX()</b> multiplies the current transformation matrix by a rotation
   * matrix. This function can be further controlled by the <b>pushMatrix()</b> and
   * <b>popMatrix()</b> functions.
   *
   * @webref transform
   * @webBrief Shears a shape around the x-axis the amount specified by the <b>angle</b> parameter
  ...
  /**
   *
   * Shears a shape around the y-axis the amount specified by the <b>angle</b> parameter. Angles
   * should be specified in radians (values from 0 to PI*2) or converted to radians with the
   * <b>radians()</b> function. Objects are always sheared around their relative position to the
   * origin and positive numbers shear objects in a clockwise direction. Transformations apply to
   * everything that happens after and subsequent calls to the function accumulates the effect.
   * For example, calling <b>shearY(PI/2)</b> and then <b>shearY(PI/2)</b> is the same as
   * <b>shearY(PI)</b>. If <b>shearY()</b> is called within the <b>draw()</b>, the transformation
   * is reset when the loop begins again. <br/>
   * <br/>
   * Technically, <b>shearY()</b> multiplies the current transformation matrix by a rotation
   * matrix. This function can be further controlled by the <b>pushMatrix()</b> and
   * <b>popMatrix()</b> functions.
   *
   * @webref transform
   * @webBrief Shears a shape around the y-axis the amount specified by the <b>angle</b> parameter
   * @param angle angle of shear specified in radians
   * @see PGraphics#popMatrix()
   * @see PGraphics#pushMatrix()
  ...
  /**
   * Multiplies the current matrix by the one specified through the parameters. This is very slow
   * because it will try to calculate the inverse of the transform, so avoid it whenever possible.
   * The equivalent function in OpenGL is <b>glMultMatrix()</b>.
   *
   * @webref transform
   * @webBrief Multiplies the current matrix by the one specified in the parameter
   * @see PGraphics#pushMatrix()
   * @see PGraphics#popMatrix()
  ...
  /**
   * Copy the current transformation matrix into the specified target. Pass in null to create a
   * new matrix.
   */
   ...
   *
   * @webref transform
   * @webBrief Prints the current matrix to the Console (the text window at the bottom of
   *           Processing)
   * @see PGraphics#pushMatrix()
   * @see PGraphics#popMatrix()
   * @see PGraphics#resetMatrix()
  ...
  /**
   * The <b>beginCamera()</b> and <b>endCamera()</b> functions enable advanced customization of
   * the camera space. The functions are useful if you want to more control over camera movement,
   * however for most users, the <b>camera()</b> function will be sufficient.<br />
   * <br />
   * The camera functions will replace any transformations (such as <b>rotate()</b> or
   * <b>translate()</b>) that occur before them in <b>draw()</b>, but they will not automatically
   * replace the camera transform itself. For this reason, camera functions should be placed at
   * the beginning of <b>draw()</b> (so that transformations happen afterwards), and the
   * <b>camera()</b> function can be used after <b>beginCamera()</b> if you want to reset the
   * camera before applying transformations.<br />
   * <br />
   * This function sets the matrix mode to the camera matrix so calls such as <b>translate()</b>,
   * <b>rotate()</b>, applyMatrix() and resetMatrix() affect the camera. <b>beginCamera()</b>
   * should always be used with a following <b>endCamera()</b> and pairs of <b>beginCamera()</b>
   * and <b>endCamera()</b> cannot be nested.
   *
   * @webref lights_camera:camera
   * @webBrief The <b>beginCamera()</b> and <b>endCamera()</b> functions enable advanced
  ...
  /**
   * The <b>beginCamera()</b> and <b>endCamera()</b> functions enable advanced customization of
   * the camera space. Please see the reference for <b>beginCamera()</b> for a description of how
  ...
  /**
   * Sets the position of the camera through setting the eye position, the center of the scene,
   * and which axis is facing upward. Moving the eye position and the direction it is pointing
   * (the center of the scene) allows the images to be seen from different angles. The version
   * without any parameters sets the camera to the default position, pointing to the center of the
   * display window with the Y axis as up. The default values are <b>camera(width/2.0, height/2.0,
   * (height/2.0) / tan(PI*30.0 / 180.0), width/2.0, height/2.0, 0, 0, 1, 0)</b>. This function is
  ...
  /**
   *
   * Prints the current camera matrix to the Console (the text window at the bottom of
   * Processing).
   *
   * @webref lights_camera:camera
   * @webBrief Prints the current camera matrix to the Console (the text window at the bottom of
   *           Processing)
   * @see PGraphics#camera(float, float, float, float, float, float, float, float, float)
   */
  ...
  /**
   *
   * Sets an orthographic projection and defines a parallel clipping volume. All objects with the
   * same dimension appear the same size, regardless of whether they are near or far from the
   * camera. The parameters to this function specify the clipping volume where left and right are
   * the minimum and maximum x values, top and bottom are the minimum and maximum y values, and
   * near and far are the minimum and maximum z values. If no parameters are given, the default is
   * used: <b>ortho(-width/2, width/2, -height/2, height/2)</b>.
   *
   * @webref lights_camera:camera
   ...
   * Objects near to the front of the volume appear their actual size, while farther objects
   * appear smaller. This projection simulates the perspective of the world more accurately than
   * orthographic projection. The version of perspective without parameters sets the default
   * perspective and the version with four parameters allows the programmer to set the area
   * precisely. The default values are: <b>perspective(PI/3.0, width/height, cameraZ/10.0,
   * cameraZ*10.0)</b> where cameraZ is <b>((height/2.0) / tan(PI*60.0/360.0))</b>
   *
   * @webref lights_camera:camera
   * @webBrief Sets a perspective projection applying foreshortening, making distant objects
   ...
   * <br />
   * A frustum is a geometric form: a pyramid with its top cut off. With the viewer's eye at the
   * imaginary top of the pyramid, the six planes of the frustum act as clipping planes when
   * rendering a 3D view. Thus, any form inside the clipping planes is rendered and visible;
   * anything outside those planes is not visible.<br />
   * <br />
   * Setting the frustum has the effect of changing the <em>perspective</em> with which the scene
   * is rendered. This can be achieved more simply in many cases by using
   * <strong>perspective()</strong>.<br />
   * <br />
   * Note that the near value must be greater than zero (as the point of the frustum "pyramid"
   * cannot converge "behind" the viewer). Similarly, the far value must be greater than the near
   * value (as the "far" plane of the frustum must be "farther away" from the viewer than the near
   * plane).<br />
   * <br />
   * Works like glFrustum, except it wipes out the current perspective matrix rather than
   * multiplying itself with it.
   *
   * @webref lights_camera:camera
  ...
  /**
   * Prints the current projection matrix to the Console (the text window at the bottom of
   * Processing).
   *
   * @webref lights_camera:camera
  ...
  /**
   * Takes a three-dimensional X, Y, Z position and returns the X value for where it will appear
   * on a (two-dimensional) screen.
   *
   * @webref lights_camera:coordinates
   * @webBrief Takes a three-dimensional X, Y, Z position and returns the X value for where it
  ...
  /**
   * Takes a three-dimensional X, Y, Z position and returns the Y value for where it will appear
   * on a (two-dimensional) screen.
   *
   * @webref lights_camera:coordinates
   * @webBrief Takes a three-dimensional X, Y, Z position and returns the Y value for where it
   *           will appear on a (two-dimensional) screen
   * @param x 3D x-coordinate to be mapped
   * @param y 3D y-coordinate to be mapped
  ...
  /**
   * Takes a three-dimensional X, Y, Z position and returns the Z value for where it will appear
   * on a (two-dimensional) screen.
   *
   * @webref lights_camera:coordinates
   * @webBrief Takes a three-dimensional X, Y, Z position and returns the Z value for where it
   *           will appear on a (two-dimensional) screen
   * @param x 3D x-coordinate to be mapped
   * @param y 3D y-coordinate to be mapped
   * @param z 3D z-coordinate to be mapped
  ...
  /**
   *
   * Returns the three-dimensional X, Y, Z position in model space. This returns the X value for a
   * given coordinate based on the current set of transformations (scale, rotate, translate, etc.)
   * The X value can be used to place an object in space relative to the location of the original
   * point once the transformations are no longer in use. <br/>
   * <br/>
   * In the example, the <b>modelX()</b>, <b>modelY()</b>, and <b>modelZ()</b> functions record
   * the location of a box in space after being placed using a series of translate and rotate
   * commands. After <b>popMatrix()</b> is called, those transformations no longer apply, but the
   * <b>(x, y, z)</b> coordinate returned by the model functions is used to place another box in
   * the same location.
   *
   * @webref lights_camera:coordinates
  ...
  /**
   * Returns the three-dimensional X, Y, Z position in model space. This returns the Y value for a
   * given coordinate based on the current set of transformations (scale, rotate, translate, etc.)
   * The Y value can be used to place an object in space relative to the location of the original
   * point once the transformations are no longer in use.<br />
   * <br />
   * In the example, the <b>modelX()</b>, <b>modelY()</b>, and <b>modelZ()</b> functions record
   * the location of a box in space after being placed using a series of translate and rotate
  ...
  /**
   *
   * Returns the three-dimensional X, Y, Z position in model space. This returns the Z value for a
   * given coordinate based on the current set of transformations (scale, rotate, translate, etc.)
   * The Z value can be used to place an object in space relative to the location of the original
   * point once the transformations are no longer in use.<br />
   * <br />
   * In the example, the <b>modelX()</b>, <b>modelY()</b>, and <b>modelZ()</b> functions record
   * the location of a box in space after being placed using a series of translate and rotate
  ...
  /**
   *
   * The <b>pushStyle()</b> function saves the current style settings and <b>popStyle()</b>
   * restores the prior settings. Note that these functions are always used together. They allow
   * you to change the style settings and later return to what you had. When a new style is
   * started with <b>pushStyle()</b>, it builds on the current style information. The
   * <b>pushStyle()</b> and <b>popStyle()</b> functions can be embedded to provide more control
   * (see the second example above for a demonstration.) <br />
   * <br />
   ...
   * <b>strokeCap()</b>,<b>strokeJoin()</b>, <b>imageMode()</b>, <b>rectMode()</b>,
   * <b>ellipseMode()</b>, <b>shapeMode()</b>, <b>colorMode()</b>, <b>textAlign()</b>,
   * <b>textFont()</b>, <b>textMode()</b>, <b>textSize()</b>, <b>textLeading()</b>,
  ...
  /**
   * The <b>pushStyle()</b> function saves the current style settings and <b>popStyle()</b>
   * restores the prior settings; these functions are always used together. They allow you to
   * change the style settings and later return to what you had. When a new style is started with
   * <b>pushStyle()</b>, it builds on the current style information. The <b>pushStyle()</b> and
   * <b>popStyle()</b> functions can be embedded to provide more control (see the second example
  ...
  /**
   * Sets the width of the stroke used for lines, points, and the border around shapes. All widths
   * are set in units of pixels. <br />
   * <br />
   * Using point() with strokeWeight(1) or smaller may draw nothing to the screen, depending on
   * the graphics settings of the computer. Workarounds include setting the pixel using
   * <b>set()</s> or drawing the point using either <b>circle()</b> or <b>square()</b>.
   *
   * @webref shape:attributes
   * @webBrief Sets the width of the stroke used for lines, points, and the border around shapes
  ...
  /**
   * Sets the style of the joints which connect line segments. These joints are either mitered,
   * beveled, or rounded and specified with the corresponding parameters MITER, BEVEL, and ROUND.
   ...
   * rounded, each of which specified with the corresponding parameters: SQUARE, PROJECT, and
   * ROUND. The default cap is ROUND. <br />
   * <br />
  ...
  /**
   * Disables drawing the stroke (outline). If both <b>noStroke()</b> and <b>noFill()</b> are
   ...
   * in terms of the RGB or HSB color depending on the current <b>colorMode().</b> The default
   * color space is RGB, with each value in the range from 0 to 255. <br />
   * <br />
   * When using hexadecimal notation to specify a color, use "<b>#</b>" or "<b>0x</b>" before the
   * values (e.g., <b>#CCFFAA</b> or <b>0xFFCCFFAA</b>). The <b>#</b> syntax uses six digits to
   * specify a color (just as colors are typically specified in HTML and CSS). When using the
   * hexadecimal notation starting with "<b>0x</b>", the hexadecimal value must be specified with
   * eight characters; the first two characters define the alpha component, and the remainder
   * define the red, green, and blue components. <br />
   * <br />
   * The value for the gray parameter must be less than or equal to the current maximum value as
   * specified by <b>colorMode()</b>. The default maximum value is 255. <br />
   * <br />
   * When drawing in 2D with the default renderer, you may need <b>hint(ENABLE_STROKE_PURE)</b> to
   * improve drawing quality (at the expense of performance). See the hint() documentation for
   * more details.
   *
   * @webref color:setting
  ...
  /**
   *
   * Removes the current fill value for displaying images and reverts to displaying images with
   * their original hues.
   *
   * @webref image:loading & displaying
   * @webBrief Removes the current fill value for displaying images and reverts to displaying
  ...
  /**
   *
   * Sets the fill value for displaying images. Images can be tinted to specified colors or made
   * transparent by including an alpha value.<br />
   * <br />
   * To apply transparency to an image without affecting its color, use white as the tint color
   * and specify an alpha value. For instance, <b>tint(255, 128)</b> will make an image 50%
   * transparent (assuming the default alpha range of 0-255, which can be changed with
   * <b>colorMode()</b>). <br />
   * <br />
   * When using hexadecimal notation to specify a color, use "<b>#</b>" or "<b>0x</b>" before the
   * values (e.g., <b>#CCFFAA</b> or <b>0xFFCCFFAA</b>). The <b>#</b> syntax uses six digits to
  ...
  /**
   *
   * Disables filling geometry. If both <b>noStroke()</b> and <b>noFill()</b> are called, nothing
  ...
  /**
   *
   * Sets the color used to fill shapes. For example, if you run <b>fill(204, 102, 0)</b>, all
   * subsequent shapes will be filled with orange. This color is either specified in terms of the
   * RGB or HSB color depending on the current <b>colorMode()</b> (the default color space is RGB,
   * with each value in the range from 0 to 255). <br/>
   * <br/>
   * When using hexadecimal notation to specify a color, use "#" or "0x" before the values (e.g.
   * #CCFFAA, 0xFFCCFFAA). The # syntax uses six digits to specify a color (the way colors are
   * specified in HTML and CSS). When using the hexadecimal notation starting with "0x", the
   * hexadecimal value must be specified with eight characters; the first two characters define
   * the alpha component and the remainder the red, green, and blue components. <br/>
   * <br/>
   * The value for the parameter "gray" must be less than or equal to the current maximum value as
   * specified by <b>colorMode()</b>. The default maximum value is 255. <br/>
   * <br/>
   ...
   * would cause all the red light to reflect and half of the green light to reflect. Used in
   * combination with <b>emissive()</b>, <b>specular()</b>, and <b>shininess()</b> in setting the
  ...
  /**
   * Sets the specular color of the materials used for shapes drawn to the screen, which sets the
   * color of highlights. Specular refers to light which bounces off a surface in a preferred
   * direction (rather than bouncing in all directions like a diffuse light). Used in combination
   * with <b>emissive()</b>, <b>ambient()</b>, and <b>shininess()</b> in setting the material
   * properties of shapes.
   *
   * @webref lights_camera:material properties
   * @webBrief Sets the specular color of the materials used for shapes drawn to the screen, which
   *           sets the color of highlights
   * @usage web_application
  ...
  /**
   *
   * Sets the amount of gloss in the surface of shapes. Used in combination with <b>ambient()</b>,
   * <b>specular()</b>, and <b>emissive()</b> in setting the material properties of shapes.
   *
   * @webref lights_camera:material properties
  ...
  /**
   *
   * Sets the emissive color of the material used for drawing shapes drawn to the screen. Used in
   * combination with <b>ambient()</b>, <b>specular()</b>, and <b>shininess()</b> in setting the
   * material properties of shapes.
   *
   * @webref lights_camera:material properties
   * @webBrief Sets the emissive color of the material used for drawing shapes drawn to the screen
   * @usage web_application
   * @param rgb color to set
   * @see PGraphics#ambient(float, float, float)
   * @see PGraphics#specular(float, float, float)
  ...
  /**
   *
   * Sets the default ambient light, directional light, falloff, and specular values. The defaults
   * are <b>ambientLight(128, 128, 128)</b> and <b>directionalLight(128, 128, 128, 0, 0, -1)</b>,
   * <b>lightFalloff(1, 0, 0)</b>, and <b>lightSpecular(0, 0, 0)</b>. Lights need to be included
   * in the <b>draw()</b> to remain persistent in a looping program. Placing them in the
   * <b>setup()</b> of a looping program will cause them to only have an effect the first time
   * through the loop.
   *
   * @webref lights_camera:lights
   * @webBrief Sets the default ambient light, directional light, falloff, and specular values
   * @usage web_application
   ...
   * @see PGraphics#spotLight(float, float, float, float, float, float, float, float, float,
   *      float, float)
  ...
  /**
   *
   * Disable all lighting. Lighting is turned off by default and enabled with the <b>lights()</b>
   * function. This function can be used to disable lighting so that 2D geometry (which does not
   * require lighting) can be drawn after a set of lighted 3D geometry.
   *
   * @webref lights_camera:lights
  ...
  /**
   *
   * Adds an ambient light. Ambient light doesn't come from a specific direction, the rays of
   * light have bounced around so much that objects are evenly lit from all sides. Ambient lights
   * are almost always used in combination with other types of lights. Lights need to be included
   * in the <b>draw()</b> to remain persistent in a looping program. Placing them in the
   * <b>setup()</b> of a looping program will cause them to only have an effect the first time
   * through the loop. The <b>v1</b>, <b>v2</b>, and <b>v3</b> parameters are interpreted as
   * either RGB or HSB values, depending on the current color mode.
   *
   * @webref lights_camera:lights
   ...
   * hitting a surface squarely and weaker if it hits at a gentle angle. After hitting a surface,
   * a directional lights scatters in all directions. Lights need to be included in the
   * <b>draw()</b> to remain persistent in a looping program. Placing them in the <b>setup()</b>
   * of a looping program will cause them to only have an effect the first time through the loop.
   * The affect of the <b>v1</b>, <b>v2</b>, and <b>v3</b> parameters is determined by the current
   * color mode. The <b>nx</b>, <b>ny</b>, and <b>nz</b> parameters specify the direction the
   * light is facing. For example, setting <b>ny</b> to -1 will cause the geometry to be lit from
  ...
  /**
   *
   * Adds a point light. Lights need to be included in the <b>draw()</b> to remain persistent in a
   * looping program. Placing them in the <b>setup()</b> of a looping program will cause them to
   * only have an effect the first time through the loop. The <b>v1</b>, <b>v2</b>, and <b>v3</b>
   * parameters are interpreted as either RGB or HSB values, depending on the current color mode.
   * The <b>x</b>, <b>y</b>, and <b>z</b> parameters set the position of the light.
   *
   * @webref lights_camera:lights
  ...
  /**
   *
   * Adds a spotlight. Lights need to be included in the <b>draw()</b> to remain persistent in a
   * looping program. Placing them in the <b>setup()</b> of a looping program will cause them to
   * only have an effect the first time through the loop. The <b>v1</b>, <b>v2</b>, and <b>v3</b>
   * parameters are interpreted as either RGB or HSB values, depending on the current color mode.
   * The <b>x</b>, <b>y</b>, and <b>z</b> parameters specify the position of the light and
   * <b>nx</b>, <b>ny</b>, <b>nz</b> specify the direction of light. The <b>angle</b> parameter
   * affects angle of the spotlight cone, while <b>concentration</b> sets the bias of light
   * focusing toward the center of that cone.
   *
   * @webref lights_camera:lights
  ...
  /**
   *
   * Sets the falloff rates for point lights, spotlights, and ambient lights. Like <b>fill()</b>,
   * it affects only the elements which are created after it in the code. The default value is
   * <b>lightFalloff(1.0, 0.0, 0.0)</b>, and the parameters are used to calculate the falloff with
   * the following equation:<br />
   * <br />
   * d = distance from light position to vertex position<br />
   * falloff = 1 / (CONSTANT + d * LINEAR + (d*d) * QUADRATIC)<br />
   * <br />
   * Thinking about an ambient light with a falloff can be tricky. If you want a region of your
   * scene to be ambient lit with one color and another region to be ambient lit with another
   * color, you could use an ambient light with location and falloff. You can think of it as a
   * point light that doesn't care which direction a surface is facing.
   *
   * @webref lights_camera:lights
   * @webBrief Sets the falloff rates for point lights, spotlights, and ambient lights
   * @usage web_application
  ...
  /**
   *
   * Sets the specular color for lights. Like <b>fill()</b>, it affects only the elements which
   * are created after it in the code. Specular refers to light which bounces off a surface in a
   * preferred direction (rather than bouncing in all directions like a diffuse light) and is used
   * for creating highlights. The specular quality of a light interacts with the specular material
  ...
  /**
   *
   * The <b>background()</b> function sets the color used for the background of the Processing
   * window. The default background is light gray. This function is typically used within
   * <b>draw()</b> to clear the display window at the beginning of each frame, but it can be used
   * inside <b>setup()</b> to set the background on the first frame of animation or if the
   * background need only be set once. <br />
   * <br />
   * An image can also be used as the background for a sketch, although the image's width and
   * height must match that of the sketch window. Images used with <b>background()</b> will ignore
   * the current <b>tint()</b> setting. To resize an image to the size of the sketch window, use
   * image.resize(width, height). <br />
   * <br />
   * It is not possible to use the transparency <b>alpha</b> parameter with background colors on
   * the main drawing surface. It can only be used along with a <b>PGraphics</b> object and
   * <b>createGraphics()</b>.
   *
   * <h3>Advanced</h3>
   * <p>
   * Clear the background with a color that includes an alpha value. This can only be used with
   * objects created by createGraphics(), because the main drawing surface cannot be set
   * transparent.
   * </p>
   * <p>
   * It might be tempting to use this function to partially clear the screen on each frame,
   * however that's not how this function works. When calling background(), the pixels will be
   * replaced with pixels that have that level of transparency. To do a semi-transparent overlay,
   * use fill() with alpha and draw a rectangle.
   * </p>
   *
   * @webref color:setting
  ...
  /**
   * Clears the pixels within a buffer. This function only works on <b>PGraphics</b> objects
   * created with the <b>createGraphics()</b> function. Unlike the main graphics context (the
   * display window), pixels in additional graphics areas created with <b>createGraphics()</b> can
   * be entirely or partially transparent. This function clears everything in a <b>PGraphics</b>
   * object to make all the pixels 100% transparent.
   *
   * @webref color:setting
  ...
  /**
   * Takes an RGB or ARGB image and sets it as the background. The width and height of the image
   * must be the same size as the sketch. Use image.resize(width, height) to make short work of
   * such a task.<br/>
   * <br/>
   * Note that even if the image is set as RGB, the high 8 bits of each pixel should be set opaque
   * (0xFF000000) because the image data will be copied directly to the screen, and non-opaque
   * background images may have strange behavior. Use image.filter(OPAQUE) to handle this
   * easily.<br/>
   * <br/>
   ...
   * <b>fill()</b>, <b>stroke()</b>, <b>background()</b>, and <b>color()</b> are defined by values
   * between 0 and 255 using the RGB color model. The <b>colorMode()</b> function is used to
   * change the numerical range used for specifying colors and to switch color systems. For
   * example, calling <b>colorMode(RGB, 1.0)</b> will specify that values are specified between 0
   * and 1. The limits for defining colors are altered by setting the parameters <b>max</b>,
   * <b>max1</b>, <b>max2</b>, <b>max3</b>, and <b>maxA</b>. <br />
   * <br />
   * After changing the range of values for colors with code like <b>colorMode(HSB, 360, 100,
   * 100)</b>, those ranges remain in use until they are explicitly changed again. For example,
   * after running <b>colorMode(HSB, 360, 100, 100)</b> and then changing back to
   * <b>colorMode(RGB)</b>, the range for R will be 0 to 360 and the range for G and B will be 0
   * to 100. To avoid this, be explicit about the ranges when changing the color mode. For
   * instance, instead of <b>colorMode(RGB)</b>, write <b>colorMode(RGB, 255, 255, 255)</b>.
   *
   * @webref color:setting
  ...
  /**
   *
   * Extracts the red value from a color, scaled to match current <b>colorMode()</b>. The value is
   * always returned as a float, so be careful not to assign it to an int value.<br />
   * <br />
   * The <b>red()</b> function is easy to use and understand, but it is slower than a technique
   * called bit shifting. When working in <b>colorMode(RGB, 255)</b>, you can achieve the same
   * results as <b>red()</b> but with greater speed by using the right shift operator (<b>>></b>)
   * with a bit mask. For example, the following two lines of code are equivalent means of getting
   * the red value of the color value <b>c</b>:<br />
   * <br />
   *
   * <pre>
  ...
  /**
   *
   * Extracts the green value from a color, scaled to match current <b>colorMode()</b>. The value
   * is always returned as a float, so be careful not to assign it to an int value.<br />
   * <br />
   * The <b>green()</b> function is easy to use and understand, but it is slower than a technique
   * called bit shifting. When working in <b>colorMode(RGB, 255)</b>, you can achieve the same
   * results as <b>green()</b> but with greater speed by using the right shift operator
   * (<b>>></b>) with a bit mask. For example, the following two lines of code are equivalent
   * means of getting the green value of the color value <b>c</b>:<br />
   * <br />
   *
   * <pre>
  ...
  /**
   *
   * Extracts the blue value from a color, scaled to match current <b>colorMode()</b>. The value
   * is always returned as a float, so be careful not to assign it to an int value.<br />
   * <br />
   * The <b>blue()</b> function is easy to use and understand, but it is slower than a technique
   * called bit masking. When working in <b>colorMode(RGB, 255)</b>, you can achieve the same
   * results as <b>blue()</b> but with greater speed by using a bit mask to remove the other color
   * components. For example, the following two lines of code are equivalent means of getting the
   * blue value of the color value <b>c</b>:<br />
   * <br />
   *
   * <pre>
  ...
  /**
   * @nowebref Interpolate between two colors. Like lerp(), but for the individual color
  ...
  /**
   * Display a warning that the specified method that takes x, y, z parameters can only be used
   * with x and y parameters in this renderer.
   * 
   * @param method The method name (no parentheses)
   */
  ...
  /**
   * Error that a particular variation of a method is unavailable (even though other variations
  ...
  /**
   * Display a warning that the specified method is not implemented, meaning that it could be
   * either a completely missing function, although other variations of it may still work
   * properly.
   */
  ...
  /**
   *
   * Reads the color of any pixel or grabs a section of an image. If no parameters are specified,
   * the entire image is returned. Use the <b>x</b> and <b>y</b> parameters to get the value of
   * one pixel. Get a section of the display window by specifying an additional <b>width</b> and
   * <b>height</b> parameter. When getting an image, the <b>x</b> and <b>y</b> parameters define
   * the coordinates for the upper-left corner of the image, regardless of the current
   * <b>imageMode()</b>.<br />
   * <br />
   * If the pixel requested is outside the image window, black is returned. The numbers returned
   * are scaled according to the current color ranges, but only RGB values are returned by this
   * function. For example, even though you may have drawn a shape with <b>colorMode(HSB)</b>, the
   ...
   * more information.
   *
   * <h3>Advanced</h3> Returns an ARGB "color" type (a packed 32-bit int) with the color. If the
   * coordinate is outside the image, zero is returned (black, but completely transparent).
   * <P>
  ...
  /**
   * Returns a copy of this PImage. Equivalent to get(0, 0, width, height). Deprecated, just use
   * copy() instead.
   */
  public PImage get() {
    return g.get();
   ...
   * channel. This mask image should only contain grayscale data, but only the blue color channel
   * is used. The mask image needs to be the same size as the image to which it is applied.<br />
   * <br />
   ...
   * only grayscale data of values between 0-255.
   *
   * <h3>Advanced</h3>
   *
   * Set alpha channel for an image. Black colors in the source image will make the destination
   * image completely transparent, and white will make things fully opaque. Gray values will be
   * in-between steps.
   * <P>
   ...
   * <br />
   * THRESHOLD<br />
   * Converts the image to black and white pixels depending on if they are above or below the
   * threshold defined by the level parameter. The parameter must be between 0.0 (black) and 1.0
   * (white). If no level is specified, 0.5 is used.<br />
   * <br />
   * GRAY<br />
   ...
   * <br />
   * POSTERIZE<br />
   * Limits each channel of the image to the number of colors specified as the parameter. The
   * parameter can be set to values between 2 and 255, but results are most noticeable in the
   * lower ranges.<br />
   * <br />
   * BLUR<br />
   * Executes a Gaussian blur with the level parameter specifying the extent of the blurring. If
   * no parameter is used, the blur is equivalent to Gaussian blur of radius 1. Larger values
   * increase the blur.<br />
   * <br />
   * ERODE<br />
   ...
   * aren't the same size, it will automatically resize source pixels to fit the specified target
   * region. No alpha information is used in the process, however if the source image has an alpha
   * channel set, it will be copied as well. <br />
   * <br />
  ...
  /**
   *
   * Blends a region of pixels into the image specified by the <b>img</b> parameter. These copies
   * utilize full alpha channel support and a choice of the following modes to blend the colors of
   * source pixels (A) with the ones of pixels in the destination image (B):<br />
   * <br />
   ...
   * SCREEN - Opposite multiply, uses inverse values of the colors.<br />
   * <br />
   * OVERLAY - A mix of MULTIPLY and SCREEN. Multiplies dark values, and screens light
   * values.<br />
   * <br />
   ...
   * Illustrator and Photoshop.<br />
   * <br />
   * All modes use the alpha information (the highest byte) of source image pixels as the blending
   ...
   * @param dw   destination image width
   * @param dh   destination image height
   * @param mode Either BLEND, ADD, SUBTRACT, LIGHTEST, DARKEST, DIFFERENCE, EXCLUSION, MULTIPLY,
