...
public interface PSurface{
  /**
   * Minimum dimensions for the window holding a sketch. This varies between platforms, Mac OS X
   * 10.3 (confirmed with 10.7 and Java 6) can do any height but requires at least 128 pixels
   * width. Windows XP has another set of limitations. And for all I know, Linux probably allows
   * window sizes to be negative numbers.
   */
  ...
  //
  /**
   * Get the native window object associated with this drawing surface. For Java2D, this will be
   * an AWT Frame object. For OpenGL, the window. The data returned here is subject to the whims
   * of the renderer, and using this method means you're willing to deal with underlying
   * implementation changes and that you won't throw a fit like a toddler if your code breaks
  ...
  /**
   * On the next trip through the animation thread, things should go sleepy-bye. Does not pause
   * the thread immediately because that needs to happen on the animation thread itself, so fires
   * on the next trip through draw().
   */
