...
/**
 * Surface that's not really visible. Used for PDF and friends, or as a base class for other
 * drawing surfaces. It includes the standard rendering loop.
 */
public class PSurfaceNone implements PSurface{
  ...
  public class AnimationThread extends Thread{
     ...
     * <A HREF="http://java.sun.com/products/jfc/tsc/articles/painting/">Painting in AWT and
     * Swing</A>
     */
    @Override
    public void run() { // not good to make this synchronized, locks things up
      ...
      /*
       * // If size un-initialized, might be a Canvas. Call setSize() here since // we now have a
       * parent object that this Canvas can use as a peer. if (graphics.image == null) { //
       * System.out.format("it's null, sketchW/H already set to %d %d%n", sketchWidth, sketchHeight);
       * try { EventQueue.invokeAndWait(new Runnable() { public void run() { setSize(sketchWidth,
       * sketchHeight); } }); } catch (InterruptedException ie) { ie.printStackTrace(); } catch
