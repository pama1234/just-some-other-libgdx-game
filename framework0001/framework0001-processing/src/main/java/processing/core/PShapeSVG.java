 ...
 * 
 * Copyright (c) 2012-22 The Processing Foundation Copyright (c) 2006-12 Ben Fry and Casey Reas
...
/**
 * This class is not part of the Processing API and should not be used directly. Instead, use
 * loadShape() and methods like it, which will make use of this class. Using this class directly
 * will cause your code to break when combined with future versions of Processing.
 * <p>
 * SVG stands for Scalable Vector Graphics, a portable graphics format. It is a vector format so
 * it allows for "infinite" resolution and relatively small file sizes. Most modern media
 * software can view SVG files, including Adobe products, Firefox, etc. Illustrator and Inkscape
 * can edit SVG files. View the SVG specification <A HREF="http://www.w3.org/TR/SVG">here</A>.
 * <p>
 * We have no intention of turning this into a full-featured SVG library. The goal of this
 * project is a basic shape importer that originally was small enough to be included with
 * applets, meaning that its download size should be in the neighborhood of 25-30 Kb. Though
 * we're far less limited nowadays on size constraints, we remain extremely limited in terms of
 * time, and do not have volunteers who are available to maintain a larger SVG library.
 * <p>
 * For more sophisticated import/export, consider the
 * <A HREF="http://xmlgraphics.apache.org/batik/">Batik</A> library from the Apache Software
 * Foundation.
 * <p>
 * Batik is used in the SVG Export library in Processing 3, however using it for full SVG import
 * is still a considerable amount of work. Wiring it to Java2D wouldn't be too bad, but using it
 * with OpenGL, JavaFX, and features like begin/endRecord() and begin/endRaw() would be
 * considerable effort.
 * <p>
 * Future improvements to this library may focus on this properly supporting a specific subset
 * of SVG, for instance the simpler SVG profiles known as
 * <A HREF="http://www.w3.org/TR/SVGMobile/">SVG Tiny or Basic</A>, although we still would not
 * support the interactivity options.
 *
 * <p>
 * <hr noshade>
 * <p>
...
public class PShapeSVG extends PShape{
  ...
  protected PShapeSVG(PShapeSVG parent,XML properties,boolean parseKids) {
    ...
    if(properties.getName().equals("svg")) {
      ...
      if(width<0||height<0) throw new RuntimeException("<svg>: width ("+
        width+
        ") and height ("+
        height+
  ...
  /**
   * Parse the specified SVG matrix into a PMatrix2D. Note that PMatrix2D is rotated relative to
   * the SVG definition, so parameters are rearranged here. More about the transformation matrices
   * in <a href="http://www.w3.org/TR/SVG/coords.html#TransformAttribute">this section</a> of the
   * SVG documentation.
   * 
  ...
  /**
   * Deliberately conforms to the HTML 4.01 color spec + en-gb grey, rather than the (unlikely to
   * be useful) entire 147-color system used in SVG.
   */
  ...
  /*
   * static protected Map<String, Integer> colorNames; static { colorNames = new HashMap<String,
   * Integer>(); colorNames.put("aqua", 0x00ffff); colorNames.put("black", 0x000000);
   * colorNames.put("blue", 0x0000ff); colorNames.put("fuchsia", 0xff00ff); colorNames.put("gray",
   * 0x808080); colorNames.put("grey", 0x808080); colorNames.put("green", 0x008000);
   * colorNames.put("lime", 0x00ff00); colorNames.put("maroon", 0x800000); colorNames.put("navy",
   * 0x000080); colorNames.put("olive", 0x808000); colorNames.put("purple", 0x800080);
   * colorNames.put("red", 0xff0000); colorNames.put("silver", 0xc0c0c0); colorNames.put("teal",
   * 0x008080); colorNames.put("white", 0xffffff); colorNames.put("yellow", 0xffff00); }
   */
  ...
  /**
   * Used in place of element.getFloatAttribute(a) because we can have a unit suffix (length or
   * coordinate).
   * 
   * @param element    what to parse
   * @param attribute  name of the attribute to get
   * @param relativeTo (float) Used for %. When relative to viewbox, should be svgWidth for
   *                   horizontal dimentions, svgHeight for vertical, and svgXYSize for anything
   *                   else.
  ...
  /**
   * Parse a size that may have a suffix for its units. This assumes 90dpi, which implies, as
   * given in the <A HREF="http://www.w3.org/TR/SVG/coords.html#Units">units</A> spec:
   * <UL>
    ...
    public FontGlyph(PShapeSVG parent,XML properties,Font font) {
      ...
      if(u!=null) {
        if(u.length()==1) {
          ...
          System.err.println("unicode for "+
            name+
  ...
  // . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
  /**
   * Get a particular element based on its SVG ID. When editing SVG by hand, this is the id="" tag
   * on any SVG element. When editing from Illustrator, these IDs can be edited by expanding the
   * layers palette. The names used in the layers palette, both for the layers or the shapes and
