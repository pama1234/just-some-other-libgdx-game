 ...
 * 
 * Copyright (c) 2012-19 The Processing Foundation Copyright (c) 2004-12 Ben Fry & Casey Reas
 * Copyright (c) 2001-04 Massachusetts Institute of Technology
 * 
 * This library is free software; you can redistribute it and/or modify it under the terms of
 * version 2.01 of the GNU Lesser General Public License as published by the Free Software
 * Foundation.
 * 
...
/**
 * PFont is the font class for Processing. To create a font to use with Processing, select
 * "Create Font..." from the Tools menu. This will create a font in the format Processing
 * requires and also adds it to the current sketch's data directory. Processing displays fonts
 * using the .vlw font format, which uses images for each letter, rather than defining them
 * through vector data. The <b>loadFont()</b> function constructs a new font and
 * <b>textFont()</b> makes a font active. The <b>list()</b> method creates a list of the fonts
 * installed on the computer, which is useful information to use with the <b>createFont()</b>
 * function for dynamically converting fonts into a format to use with Processing.<br />
 * <br />
 * To create a new font dynamically, use the <b>createFont()</b> function. Do not use the syntax
 * <b>new PFont()</b>.
 *
 * <h3>Advanced</h3>
 * <P>
...
public class PFont implements PConstants{
  ...
  /**
   * Actual glyph data. The length of this array won't necessarily be the same size as glyphCount,
   * in cases where lazy font loading is in use.
   */
  protected Glyph[] glyphs;
  /**
   * Name of the font as seen by Java when it was created. If the font is available, the native
  ...
  /**
   * The ascent of the font. If the 'd' character is present in this PFont, this value is replaced
   * with its pixel height, because the values returned by FontMetrics.getAscent() seem to be
   * terrible.
   */
  protected int ascent;
  /**
   * The descent of the font. If the 'p' character is present in this PFont, this value is
   * replaced with its lowest pixel height, because the values returned by
   * FontMetrics.getDescent() are gross.
   */
  protected int descent;
  /**
   * A more efficient array lookup for straight ASCII characters. For Unicode characters, a
   * QuickSort-style search is used.
   */
  protected int[] ascii;
  /**
   * True if this font is set to load dynamically. This is the default when createFont() method is
   * called without a character set. Bitmap versions of characters are only created when prompted
   * by an index() call.
   */
  protected boolean lazy;
  /**
   * Native Java version of the font. If possible, this allows the PGraphics subclass to just use
   * Java's font rendering stuff in situations where that's faster.
   */
  protected Font font;
  /**
   * True if this font was loaded from an InputStream, rather than by name from the OS. It's best
   * to use the native version of a font loaded from a TTF file, since that will ensure that the
   * font is available when the sketch is exported.
   */
  protected boolean stream;
  /**
   * True if this font should return 'null' for getFont(), so that the native font will be used to
   * create a subset, but the native version of the font will not be used.
   */
  ...
  /**
   * The name of the font that Java uses when a font isn't found. See {@link #findFont(String)}
   * and {@link #loadFonts()} for more info.
   */
  static protected String systemFontName;
  /**
   * Array of the native system fonts. Used to lookup native fonts by their PostScript name. This
   * is a workaround for a several year old Apple Java bug that they can't be bothered to fix.
   */
  ...
  /**
   * Create a new image-based font on the fly. If charset is set to null, the characters will only
   * be created as bitmaps when they're drawn.
   *
   * @nowebref
   ...
   * <p>
   * This is used by the Create Font tool, or whatever anyone else dreams up for messing with
   * fonts themselves.
   * <p>
  ...
  /**
   * Set the native complement of this font. Might be set internally via the findFont() function,
   * or externally by a deriveFont() call if the font is resized by PGraphicsJava2D.
   */
  ...
  //  }
  /**
   * Returns the size that will be used when textFont(font) is called. When drawing with 2x pixel
   * density, bitmap fonts in OpenGL need to be created (behind the scenes) at double the
   * requested size. This ensures that they're shown at half on displays (so folks don't have to
   * change their sketch code).
   */
  ...
  /**
   * Attempt to find the native version of this font. (Public so that it can be used by OpenGL or
   * other renderers.)
   */
   ...
   * <P>
   * This is the union of the Mac Roman and Windows ANSI (CP1250) character sets. ISO 8859-1 Latin
   * 1 is Unicode characters 0x80 -> 0xFF, and would seem a good standard, but in practice, most
   * P5 users would rather have characters that they expect from their platform's fonts.
   * <P>
   * This is more of an interim solution until a much better font solution can be determined.
   * (i.e. create fonts on the fly from some sort of vector format).
   * <P>
  ...
  /**
   *
   * Gets a list of the fonts installed on the system. The data is returned as a String array.
   * This list provides the names of each font for input into <b>createFont()</b>, which allows
   * Processing to dynamically format fonts.
   *
   *
   * @webref pfont
   ...
   *
   * This can take a while with a lot of fonts installed, but running it on a separate thread may
   * not help much. As of the commit that's adding this note, loadFonts() will only be called by
   * PFont.list() and when loading a font by name, both of which are occasions when we'd need to
   * block until this was finished anyway. It's also possible that running getAllFonts() on a
   * non-EDT thread could cause graphics system issues. Further, the first fonts are usually
   * loaded at the beginning of a sketch, meaning that sketch startup time will still be affected,
   * even with threading in place.
   *
  ...
  /**
   * Starting with Java 1.5, Apple broke the ability to specify most fonts. This bug was filed
   * years ago as #4769141 at bugreporter.apple.com. More:
   * <a href="https://download.processing.org/bugzilla/407.html">Bug 407</a>. <br>
   * This function displays a warning when the font is not found and Java's system font is used.
