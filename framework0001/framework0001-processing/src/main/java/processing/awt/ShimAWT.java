...
/**
 * This class exists as an abstraction layer to remove AWT from PApplet. It is a staging area
 * for AWT-specific code that's shared by the Java2D, JavaFX, and JOGL renderers. Once
 * PSurfaceFX and PSurfaceJOGL have their own implementations, these methods will move to
 * PSurfaceAWT.
 */
public class ShimAWT implements PConstants{
  /*
   * PGraphics graphics; PApplet sketch;
   * 
   * 
   * public ShimAWT(PApplet sketch) { this.graphics = graphics; this.sketch = sketch; }
   */
  ...
  /*
   * private int displayDensityImpl() { if (display != SPAN && (fullScreen || present)) { return
   * displayDensity(display); } // walk through all displays, use 2 if any display is 2 for (int i
   * = 0; i < displayDevices.length; i++) { if (displayDensity(i+1) == 2) { return 2; } } // If
   * nobody's density is 2 then everyone is 1 return 1; }
   */
  private int displayDensityImpl(int display) {
    ...
    if(graphicsConfig==null) {
      System.err.println("Display "+
        display+
        " does not exist, "+
        "returning 1 for displayDensity("+
        display+
        ")");
  ...
  static public Object getNativeImage(PImage img) {
    img.loadPixels();
    int type=(img.format==RGB)?BufferedImage.TYPE_INT_RGB:BufferedImage.TYPE_INT_ARGB;
  ...
  static public void resizeImage(PImage img,int w,int h) { // ignore
    ...
    BufferedImage bimg=shrinkImage((BufferedImage)img.getNative(),w*img.pixelDensity,h*img.pixelDensity);
  ...
  static public PImage loadImage(PApplet sketch,String filename,Object... args) {
    ...
    /*
     * if (extension.equals("tif") || extension.equals("tiff")) { InputStream input =
     * sketch.createInput(filename); PImage image = (input == null) ? null : PImage.loadTIFF(input);
     * return image; }
     */
        ...
        if(bytes==null) {
          ...
          if(awtImage instanceof BufferedImage) {
            ...
            if(space==ColorSpace.TYPE_CMYK) {
               ...
               * https://community.oracle.com/thread/1272045?start=0&tstart=0 BufferedImage destImage = new
               * BufferedImage(buffImage.getWidth(), buffImage.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
               * ColorConvertOp op = new ColorConvertOp(null); op.filter(buffImage, destImage); image = new
               * PImage(destImage);
               */
            }
          }
  ...
  static protected boolean saveImageIO(PImage image,String path,String... args) throws IOException {
    try {
      int outputFormat=(image.format==ARGB)?BufferedImage.TYPE_INT_ARGB:BufferedImage.TYPE_INT_RGB;
  ...
  /*
   * public int displayDensity() { if (sketch.display != PConstants.SPAN && (sketch.fullScreen ||
   * sketch.present)) { return displayDensity(sketch.display); } // walk through all displays, use
   * 2 if any display is 2 for (int i = 0; i < displayDevices.length; i++) { if
   * (displayDensity(i+1) == 2) { return 2; } } // If nobody's density is 2 then everyone is 1
   * return 1; }
   */
  /**
   ...
   */
  /*
   * public int displayDensity(int display) { if (display > 0 && display <= displayDevices.length)
   * { GraphicsConfiguration graphicsConfig = displayDevices[display -
   * 1].getDefaultConfiguration(); AffineTransform tx = graphicsConfig.getDefaultTransform();
   * return (int) Math.round(tx.getScaleX()); }
   * 
   * System.err.println("Display " + display + " does not exist, returning "); return 1; // not
   * the end of the world, so don't throw a RuntimeException }
   */
  ...
  /*
   * static public void selectOutput(String prompt, String callbackMethod, File file, Object
   * callbackObject, Frame parent) { selectImpl(prompt, callbackMethod, file, callbackObject,
   * parent, FileDialog.SAVE, null); }
   * 
   * 
   * static public void selectOutput(String prompt, String callbackMethod, File file, Object
   * callbackObject, Frame parent, PApplet sketch) { selectImpl(prompt, callbackMethod, file,
   * callbackObject, parent, FileDialog.SAVE, sketch); }
   */
   ...
   * https://github.com/processing/processing/issues/3831 static protected void selectEvent(final
   * String prompt, final String callbackMethod, final File defaultSelection, final Object
   * callbackObject, final Frame parentFrame, final int mode, final PApplet sketch) {
   * EventQueue.invokeLater(new Runnable() { public void run() { boolean hide = (sketch != null)
   * && (sketch.g instanceof PGraphicsOpenGL) && (PApplet.platform == PConstants.WINDOWS); if
   * (hide) sketch.getSurface().setVisible(false);
   * 
   * selectImpl(prompt, callbackMethod, defaultSelection, callbackObject, parentFrame, mode,
   * sketch);
   * 
  ...
  /*
   * static public void selectFolder(final String prompt, final String callbackMethod, final File
   * defaultSelection, final Object callbackObject, final Frame parentFrame) {
   * selectFolderEvent(prompt, callbackMethod, defaultSelection, callbackObject, parentFrame,
   * null); }
   * 
   * 
   * // Will remove the 'sketch' parameter once we get an upstream JOGL fix //
   * https://github.com/processing/processing/issues/3831 static public void
   * selectFolderEvent(final String prompt, final String callbackMethod, final File
   * defaultSelection, final Object callbackObject, final Frame parentFrame, final PApplet sketch)
   * { EventQueue.invokeLater(() -> { selectFolderImpl(prompt, callbackMethod, defaultSelection,
  ...
  /**
   * Initialize the Look & Feel if it hasn't been already. Call this before using any
   * Swing-related code in PApplet methods.
   */
