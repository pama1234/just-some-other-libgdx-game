 ...
 * 
 * Copyright (c) 2013-15 The Processing Foundation Copyright (c) 2005-13 Ben Fry and Casey Reas
 * 
 ...
 * </PRE>
 * 
 * This will let you do Graphics2D calls directly, but is not supported in any way shape or
 * form. Which just means "have fun, but don't complain if it breaks."
 * <p>
 * Advanced <a href=
 * "http://docs.oracle.com/javase/7/docs/webnotes/tsg/TSG-Desktop/html/java2d.html">debugging
 * notes</a> for Java2D.
 */
public class PGraphicsJava2D extends PGraphics{
  ...
  //  }
  /*
   * @Override protected void allocate() { // Tried this with RGB instead of ARGB for the
   * primarySurface version, // but didn't see any performance difference (OS X 10.6, Java 6u24).
   * // For 0196, also attempted RGB instead of ARGB, but that causes // strange things to happen
   * with blending. // image = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB); if
   * (primarySurface) { if (useCanvas) { if (canvas != null) { parent.removeListeners(canvas);
   * parent.remove(canvas); } canvas = new Canvas(); canvas.setIgnoreRepaint(true);
   * 
   * // parent.setLayout(new BorderLayout()); // parent.add(canvas, BorderLayout.CENTER);
   * parent.add(canvas); // canvas.validate(); // parent.doLayout();
   * 
   * if (canvas.getWidth() != width || canvas.getHeight() != height) {
   * PApplet.debug("PGraphicsJava2D comp size being set to " + width + "x" + height);
   * canvas.setSize(width, height); } else { PApplet.debug("PGraphicsJava2D comp size already " +
   * width + "x" + height); }
   * 
   * parent.addListeners(canvas); // canvas.createBufferStrategy(1); // g2 = (Graphics2D)
   * canvas.getGraphics();
   * 
   * } else { parent.updateListeners(parent); // in case they're already there
   * 
   * // using a compatible image here doesn't seem to provide any performance boost
   * 
   * if (useOffscreen) { // Needs to be RGB otherwise there's a major performance hit [0204] //
   * https://github.com/processing/processing/issues/768 image = new BufferedImage(width, height,
   * BufferedImage.TYPE_INT_RGB); // GraphicsConfiguration gc = parent.getGraphicsConfiguration();
   * // image = gc.createCompatibleImage(width, height); offscreen = new BufferedImage(width,
   * height, BufferedImage.TYPE_INT_ARGB); // offscreen = gc.createCompatibleImage(width, height);
   * g2 = (Graphics2D) offscreen.getGraphics();
   * 
   * } else { // System.out.println("hopefully faster " + width + " " + height); // new
   * Exception().printStackTrace(System.out);
   * 
   * GraphicsConfiguration gc = canvas.getGraphicsConfiguration(); // If not realized (off-screen,
   * i.e the Color Selector Tool), // gc will be null. if (gc == null) { GraphicsEnvironment ge =
   * GraphicsEnvironment.getLocalGraphicsEnvironment(); gc =
   * ge.getDefaultScreenDevice().getDefaultConfiguration(); }
   * 
   * image = gc.createCompatibleImage(width, height); g2 = (Graphics2D) image.getGraphics(); } } }
   * else { // not the primary surface // Since this buffer's offscreen anyway, no need for the
   * extra offscreen // buffer. However, unlike the primary surface, this feller needs to be //
   * ARGB so that blending ("alpha" compositing) will work properly. image = new
   * BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB); g2 = (Graphics2D)
   * image.getGraphics(); }
   */
  /*
   * if (primarySurface) { Canvas canvas = ((PSurfaceAWT) surface).canvas;
   * 
   * GraphicsConfiguration gc = canvas.getGraphicsConfiguration(); // If not realized (off-screen,
   * i.e the Color Selector Tool), // gc will be null. if (gc == null) { GraphicsEnvironment ge =
   * GraphicsEnvironment.getLocalGraphicsEnvironment(); gc =
  ...
  public void beginDraw() {
    ...
    handleSmooth();
    /*
     * // NOTE: Calling image.getGraphics() will create a new Graphics context, // even if it's for
     * the same image that's already had a context created. // Seems like a speed/memory issue, and
     * also requires that all smoothing, // stroke, font and other props be reset. Can't find a good
     * answer about // whether getGraphics() and dispose() on each frame is 1) better practice //
     * and 2) minimal overhead, however. Instinct suggests #1 may be true, // but #2 seems a
     * problem. if (primarySurface && !useOffscreen) { GraphicsConfiguration gc =
     * canvas.getGraphicsConfiguration(); if (false) { if (image == null || ((VolatileImage)
     * image).validate(gc) == VolatileImage.IMAGE_INCOMPATIBLE) { image =
     * gc.createCompatibleVolatileImage(width, height); g2 = (Graphics2D) image.getGraphics();
     * reapplySettings = true; } } else { if (image == null) { image =
     * gc.createCompatibleImage(width, height); PApplet.debug("created new image, type is " +
     * image); g2 = (Graphics2D) image.getGraphics(); reapplySettings = true; } } }
     * 
     * if (useCanvas && primarySurface) { if (parent.frameCount == 0) {
     * canvas.createBufferStrategy(2); strategy = canvas.getBufferStrategy();
     * PApplet.debug("PGraphicsJava2D.beginDraw() strategy is " + strategy); BufferCapabilities caps
     * = strategy.getCapabilities(); caps = strategy.getCapabilities();
     ...
     * caps.getBackBufferCapabilities().isAccelerated()); } GraphicsConfiguration gc =
     * canvas.getGraphicsConfiguration();
     * 
     * if (bimage == null || bimage.getWidth() != width || bimage.getHeight() != height) {
     * PApplet.debug("PGraphicsJava2D creating new image"); bimage = gc.createCompatibleImage(width,
     * height); // image = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB); g2 =
     * bimage.createGraphics(); defaultComposite = g2.getComposite(); reapplySettings = true; } }
     */
    checkSettings();
  ...
  /**
   * Smoothing for Java2D is 2 for bilinear, and 3 for bicubic (the default). Internally,
   * smooth(1) is the default, smooth(0) is noSmooth().
   */
  ...
  public void endDraw() {
    ...
    if(primaryGraphics) {
      /*
       * //if (canvas != null) { if (useCanvas) { //System.out.println(canvas);
       * 
       * // alternate version //canvas.repaint(); // ?? what to do for swapping buffers
       * 
       * // System.out.println("endDraw() frameCount is " + parent.frameCount); // if
       * (parent.frameCount != 0) { redraw(); // }
       * 
       * } else if (useOffscreen) { // don't copy the pixels/data elements of the buffered image
       * directly, // since it'll disable the nice speedy pipeline stuff, sending all drawing // into
       * a world of suck that's rough 6 trillion times slower. synchronized (image) {
       * //System.out.println("inside j2d sync"); image.getGraphics().drawImage(offscreen, 0, 0,
       * null); }
       * 
       * } else { // changed to not dispose and get on each frame, // otherwise a new Graphics context
       * is used on each frame // g2.dispose(); //
  ...
  /*
   * private void redraw() { // only need this check if the validate() call will use redraw() //
   * if (strategy == null) return; do {
   * PApplet.debug("PGraphicsJava2D.redraw() top of outer do { } block"); do {
   * PApplet.debug("PGraphicsJava2D.redraw() top of inner do { } block");
   * PApplet.debug("strategy is " + strategy); Graphics bsg = strategy.getDrawGraphics(); // if
   * (vimage != null) { // bsg.drawImage(vimage, 0, 0, null); // } else { bsg.drawImage(bimage, 0,
   * 0, null); // if (parent.frameCount == 0) { // try { // ImageIO.write(image, "jpg", new
   * java.io.File("/Users/fry/Desktop/buff.jpg")); // } catch (IOException e) { //
   * e.printStackTrace(); // } // } // } bsg.dispose();
   * 
   * // the strategy version // g2.dispose(); // if (!strategy.contentsLost()) { // if
   * (parent.frameCount != 0) { // Toolkit.getDefaultToolkit().sync(); // } // } else { //
   * System.out.println("XXXXX strategy contents lost"); // } // } // } } while
   * (strategy.contentsRestored());
   * 
   * PApplet.debug("PGraphicsJava2D.redraw() showing strategy"); strategy.show();
   * 
  ...
  //  }
  /*
   * protected PShape createShapeImpl(PGraphicsJava2D pg, int type) { PShape shape = null; if
   * (type == PConstants.GROUP) { shape = new PShape(pg, PConstants.GROUP); } else if (type ==
   * PShape.PATH) { shape = new PShape(pg, PShape.PATH); } else if (type == PShape.GEOMETRY) {
   * shape = new PShape(pg, PShape.GEOMETRY); } // defaults to false, don't assign it and make
   * complexity for overrides //shape.set3D(false); return shape; }
   */
  /*
   * static protected PShape createShapeImpl(PGraphicsJava2D pg, int kind, float... p) { PShape
   * shape = null; int len = p.length;
   * 
   * if (kind == POINT) { if (len != 2) { showWarning("Wrong number of parameters"); return null;
   * } shape = new PShape(pg, PShape.PRIMITIVE); shape.setKind(POINT); } else if (kind == LINE) {
   * if (len != 4) { showWarning("Wrong number of parameters"); return null; } shape = new
   * PShape(pg, PShape.PRIMITIVE); shape.setKind(LINE); } else if (kind == TRIANGLE) { if (len !=
   * 6) { showWarning("Wrong number of parameters"); return null; } shape = new PShape(pg,
   * PShape.PRIMITIVE); shape.setKind(TRIANGLE); } else if (kind == QUAD) { if (len != 8) {
   * showWarning("Wrong number of parameters"); return null; } shape = new PShape(pg,
   * PShape.PRIMITIVE); shape.setKind(QUAD); } else if (kind == RECT) { if (len != 4 && len != 5
   * && len != 8 && len != 9) { showWarning("Wrong number of parameters"); return null; } shape =
   * new PShape(pg, PShape.PRIMITIVE); shape.setKind(RECT); } else if (kind == ELLIPSE) { if (len
   * != 4 && len != 5) { showWarning("Wrong number of parameters"); return null; } shape = new
   * PShape(pg, PShape.PRIMITIVE); shape.setKind(ELLIPSE); } else if (kind == ARC) { if (len != 6
   * && len != 7) { showWarning("Wrong number of parameters"); return null; } shape = new
   * PShape(pg, PShape.PRIMITIVE); shape.setKind(ARC); } else if (kind == BOX) {
   * showWarning("Primitive not supported in 2D"); } else if (kind == SPHERE) {
   * showWarning("Primitive not supported in 2D"); } else {
   * showWarning("Unrecognized primitive type"); }
   * 
   * if (shape != null) { shape.setParams(p); }
   * 
   * // defaults to false, don't assign it and make complexity for overrides //shape.set3D(false);
   * 
   * return shape; }
   */
  //////////////////////////////////////////////////////////////
  ...
  public float textAscent() {
    ...
    /*
     * Font font = (Font) textFont.getNative(); if (font != null) { return
     * g2.getFontMetrics(font).getAscent(); }
     */
  ...
  public float textDescent() {
    ...
    /*
     * Font font = (Font) textFont.getNative(); if (font != null) { return
     * g2.getFontMetrics(font).getDescent(); }
     */
   ...
   * <p/>
   * Called from textFontImpl and textSizeImpl, so the metrics will get recorded properly.
   */
  @Override
  protected void handleTextSize(float size) {
    ...
    if(font!=null) {
      ...
      /*
       * Map<TextAttribute, ?> attrs = font.getAttributes(); for (TextAttribute ta : attrs.keySet()) {
    ...
    if(font!=null) {
      ...
      if(textFont.getName().equals(defaultFontName)) {
        if(font.canDisplayUpTo(buffer,start,stop)!=-1) {
          final String msg="Some characters not available in the current font, "+
            "use createFont() to specify a typeface the includes them.";
          showWarning(msg);
        }
      }
      /*
       * // save the current setting for text smoothing. note that this is // different from the
       * smooth() function, because the font smoothing // is controlled when the font is created, not
       * now as it's drawn. // fixed a bug in 0116 that handled this incorrectly. Object textAntialias
       * = g2.getRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING);
       * 
       * // override the current text smoothing setting based on the font // (don't change the global
       * smoothing settings) g2.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING, textFont.smooth
   ...
   * Toolkit toolkit;
   * 
   * @SuppressWarnings("deprecation") protected FontMetrics getFontMetrics(Font font) { if
   * (toolkit == null) { try { Canvas canvas = (Canvas) surface.getNative(); toolkit =
   * canvas.getToolkit(); } catch (Exception e) { // May error out if it's a PSurfaceNone or
   * similar toolkit = Toolkit.getDefaultToolkit(); } } return toolkit.getFontMetrics(font);
  ...
  protected WritableRaster getRaster() {
    ...
    if(primaryGraphics) {
      /*
       * // 'offscreen' will probably be removed in the next release if (useOffscreen) { raster =
       * offscreen.getRaster(); } else
