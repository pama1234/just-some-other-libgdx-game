...
public class PSurfaceAWT extends PSurfaceNone{
  ...
  public PSurfaceAWT(PGraphics graphics) {
    ...
    super(graphics);
    /*
     * if (checkRetina()) { // System.out.println("retina in use");
     * 
     * // The active-mode rendering seems to be 2x slower, so disable it // with retina. On a
     * non-retina machine, however, useActive seems // the only (or best) way to handle the
     * rendering. // useActive = false; // canvas = new JPanel(true) { // @Override // public void
     * paint(Graphics screen) { //// if (!sketch.insideDraw) { //
     ...
     * strategy is far faster, but perhaps only because it's blitting with // flicker--pushing
     * pixels out before the screen has finished rendering. // useStrategy = false; }
     */
  ...
  public void initFrame(final PApplet sketch) {/*
                                                * , int backgroundColor, int deviceIndex, boolean fullScreen, boolean spanDisplays) {
                                                */
    this.sketch=sketch;
    ...
    /*
     * // Sketch has already requested to be the same as the screen's // width and height, so let's
     * roll with full screen mode. if (screenRect.width == sketchWidth && screenRect.height ==
     * sketchHeight) { fullScreen = true; sketch.fullScreen(); // won't change the renderer }
     */
    ...
    /*
     * if (fullScreen) { // Called here because the graphics device is needed before we can //
     * determine whether the sketch wants size(displayWidth, displayHeight), // and getting the
     * graphics device will be PSurface-specific. PApplet.hideMenuBar();
     * 
     * // Tried to use this to fix the 'present' mode issue. // Did not help, and the screenRect
     * setup seems to work fine. //frame.setExtendedState(Frame.MAXIMIZED_BOTH);
     * 
     * // https://github.com/processing/processing/pull/3162 frame.dispose(); // release native
     * resources, allows setUndecorated() frame.setUndecorated(true); // another duplicate? // if
     * (backgroundColor != null) { // frame.getContentPane().setBackground(backgroundColor); // } //
     * this may be the bounds of all screens frame.setBounds(screenRect); // will be set visible in
     * placeWindow() [3.0a10] //frame.setVisible(true); // re-add native resources }
     */
  ...
  public void setVisible(boolean visible) {
    ...
    /*
     * if (visible && PApplet.platform == PConstants.LINUX) { // Linux doesn't deal with insets the
     * same way. We get fake insets // earlier, and then the window manager will slap its own insets
     * // onto things once the frame is realized on the screen. Awzm. if (PApplet.platform ==
     * PConstants.LINUX) { Insets insets = frame.getInsets(); frame.setSize(Math.max(sketchWidth,
     * MIN_WINDOW_WIDTH) + insets.left + insets.right, Math.max(sketchHeight, MIN_WINDOW_HEIGHT) +
  ...
  public void placeWindow(int[] location,int[] editorLocation) {
    ...
    if(!sketch.sketchFullScreen()) {
      if(location!=null) {
        ...
        if(locationX-window.width>10) {
          ...
          /*
           * if ((locationX + window.width > sketch.displayWidth - 33) || (locationY + window.height >
           * sketch.displayHeight - 33)) { // otherwise center on screen locationX = (sketch.displayWidth
           * - window.width) / 2; locationY = (sketch.displayHeight - window.height) / 2; }
           */
    ...
    /*
     * // If displayable() is false, then PSurfaceNone should be used, but... if
     * (sketch.getGraphics().displayable()) { frame.setVisible(true); //
     * System.out.println("setting visible on EDT? " + EventQueue.isDispatchThread());
     * //requestFocus(); // if (canvas != null) { // //canvas.requestFocusInWindow(); //
     * canvas.requestFocus(); // } }
     */
  ...
  /*
   * @Override public void initImage(PGraphics graphics) { GraphicsConfiguration gc =
   * canvas.getGraphicsConfiguration(); // If not realized (off-screen, i.e the Color Selector
   * Tool), gc will be null. if (gc == null) {
   * System.err.println("GraphicsConfiguration null in initImage()"); GraphicsEnvironment ge =
   * GraphicsEnvironment.getLocalGraphicsEnvironment(); gc =
   * ge.getDefaultScreenDevice().getDefaultConfiguration(); }
   * 
   * // Formerly this was broken into separate versions based on offscreen or // not, but we may
   * as well create a compatible image; it won't hurt, right? int wide = graphics.width *
   * graphics.pixelFactor; int high = graphics.height * graphics.pixelFactor; graphics.image =
  ...
  //  }
  /*
   * private boolean checkRetina() { if (PApplet.platform == PConstants.MACOSX) { // This should
   * probably be reset each time there's a display change. // A 5-minute search didn't turn up any
   * such event in the Java 7 API. // Also, should we use the Toolkit associated with the editor
   * window? final String javaVendor = System.getProperty("java.vendor"); if
   * (javaVendor.contains("Oracle")) { GraphicsEnvironment env =
   * GraphicsEnvironment.getLocalGraphicsEnvironment(); GraphicsDevice device =
   * env.getDefaultScreenDevice();
   * 
   * try { Field field = device.getClass().getDeclaredField("scale"); if (field != null) {
   * field.setAccessible(true); Object scale = field.get(device);
   * 
   * if (scale instanceof Integer && ((Integer)scale).intValue() == 2) { return true; } } } catch
  ...
  /*
   * private void checkDisplaySize() { if (canvas.getGraphicsConfiguration() != null) {
   * GraphicsDevice displayDevice = getGraphicsConfiguration().getDevice();
   * 
   * if (displayDevice != null) { Rectangle screenRect =
   * displayDevice.getDefaultConfiguration().getBounds();
   * 
   * displayWidth = screenRect.width; displayHeight = screenRect.height; } } }
   */
  //  /**
  ...
  //  }
  /**
   * Set up a listener that will fire proper component resize events in cases where
   * frame.setResizable(true) is called.
   */
  ...
  //////////////////////////////////////////////////////////////
  /*
   * // disabling for now; requires Java 1.7 and "precise" semantics are odd... // returns 0.1 for
   * tick-by-tick scrolling on OS X, but it's not a matter of // calling ceil() on the value: 1.5
   * goes to 1, but 2.3 goes to 2. // "precise" is a whole different animal, so add later API to
   * shore that up. static protected Method preciseWheelMethod; static { try { preciseWheelMethod
   * = MouseWheelEvent.class.getMethod("getPreciseWheelRotation", new Class[] { }); } catch
   * (Exception e) { // ignored, the method will just be set to null } }
   */
  /**
   * Figure out how to process a mouse event. When loop() has been called, the events will be
   * queued up until drawing is complete. If noLoop() has been called, then events will happen
   * immediately.
   */
  protected void nativeMouseEvent(java.awt.event.MouseEvent nativeEvent) {
    ...
    switch(nativeEvent.getID()) {
      ...
      case java.awt.event.MouseEvent.MOUSE_WHEEL:
        ...
        /*
         * if (preciseWheelMethod != null) { try { peAmount = ((Double)
         * preciseWheelMethod.invoke(nativeEvent, (Object[]) null)).floatValue(); } catch (Exception e)
  ...
  public void setCursor(PImage img,int x,int y) {
    ...
    Cursor cursor=canvas.getToolkit().createCustomCursor((Image)img.getNative(),
      new Point(x,y),
      "custom");
