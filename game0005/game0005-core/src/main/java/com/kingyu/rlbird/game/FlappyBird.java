...
public class FlappyBird extends Frame implements RlEnv{
  ...
  /**
   * {@inheritDoc} action[0] == 1 : do nothing action[1] == 1 : flap the bird
   */
  @Override
  ...
  /**
   * Convert image to CNN input. Copy the initial frame image, stack into NDList, then replace the
   * fourth frame with the current frame to ensure that the batch picture is continuous.
   *
