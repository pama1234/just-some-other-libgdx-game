...
import static pama1234.gdx.game.neat.util.Mutation.SUB_NODE;
import static pama1234.gdx.game.neat.util.Utils.pickRandom;
import static pama1234.gdx.game.neat.util.Utils.randBoolean;
import static pama1234.gdx.game.neat.util.Utils.randDouble;
import static pama1234.gdx.game.neat.util.Utils.randInt;
...
public class Network{
  ...
  public Network(final int input,final int output) {
    ...
    final float initWeight=this.input*Math.sqrt((float)2/this.input);
  ...
  public static Network crossover(final Network network1,final Network network2,final boolean equal) {
    ...
    final float score1=Double.isNaN(network1.score)?-Double.MAX_VALUE:network1.score;
    final float score2=Double.isNaN(network2.score)?-Double.MAX_VALUE:network2.score;
    ...
    connections.stream()
      ...
      .forEach(connectionData-> {
        final Connection connection=offspring.connect(
          offspring.nodes.get((int)(float)connectionData[1]),
          offspring.nodes.get((int)(float)connectionData[2]));
        connection.weight=connectionData[0];
        if(!Double.isNaN(connectionData[3])&&connectionData[3]<size) {
          offspring.gate(offspring.nodes.get((int)(float)connectionData[3]),connection);
        }
      });
  ...
  public float evolve(final float[][] inputs,final float[][] outputs,final EvolveOptions options) {
    ...
    if(options.getFitnessFunction()==null) {
      ...
      options.setFitnessFunction(genome-> {
        final float sum=IntStream.range(0,amount)
    ...
    float error=-Double.MAX_VALUE;
    float bestScore=-Double.MAX_VALUE;
