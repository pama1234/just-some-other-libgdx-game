...
public class Network{
	...
	public Network(final int input,final int output) {
		...
		final float initWeight=this.input*UtilMath.sqrt((float)2/this.input);
	...
	public static Network crossover(final Network network1,final Network network2,final boolean equal) {
		...
		final Map<Integer,float[]> network1Connections=makeConnections(network1);
		final Map<Integer,float[]> network2Connections=makeConnections(network2);
		final List<float[]> connections=new ArrayList<>();
		...
		connections.stream()
			...
			.forEach(connectionData-> {
				...
				if(!Float.isNaN(connectionData[3])&&connectionData[3]<size) {
	 ...
	 */
	private static Map<Integer,float[]> makeConnections(final Network network) {
		final Map<Integer,float[]> connections=new HashMap<>();
	...
	private float[] activate(final float[] input) {
		...
		float[] array=new float[output.size()];
		IntStream.range(0,array.length).mapToDouble(i->array[i]=output.get(i));//TODO 未验证
		return array;
