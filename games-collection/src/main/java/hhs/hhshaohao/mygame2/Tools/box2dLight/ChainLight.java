...
public class ChainLight extends Light{
  ...
  public void updateChain() {
    ...
    for(int i=0;i<segmentCount;i++) {
      ...
      previousAngle.set(
        (i==0)?segmentAngles.items[i]:segmentAngles.items[i-1]);
      currentAngle.set(segmentAngles.items[i]);
      nextAngle.set(
        (i==segmentAngles.size-1)?segmentAngles.items[i]:segmentAngles.items[i+1]);
      ...
      vDirection.set(
        chain.items[segmentVertex+2],chain.items[segmentVertex+3]).sub(vSegmentStart).nor();
