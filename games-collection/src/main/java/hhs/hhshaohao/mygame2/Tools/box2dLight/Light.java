...
public abstract class Light implements Disposable{
   ...
   *                        light color
   * @param distance
   *                        light distance (if applicable), soft shadow length is set to distance
   *                        * 0.1f
  ...
  boolean contactFilter(Fixture fixtureB) {
    ...
    if(filterA.groupIndex!=0&&
      filterA.groupIndex==filterB.groupIndex) return filterA.groupIndex>0;
  ...
  boolean globalContactFilter(Fixture fixtureB) {
    ...
    if(globalFilterA.groupIndex!=0&&
      globalFilterA.groupIndex==filterB.groupIndex) return globalFilterA.groupIndex>0;
