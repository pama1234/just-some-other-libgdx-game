package hhs.hhshaohao.mygame2.Tools.box2dLight;
...
import hhs.hhshaohao.mygame2.Tools.shaders.LightShader;
...
public class RayHandler implements Disposable{
  ...
  /**
   * Gamma correction value used if enabled TODO: remove final modifier and provide method to
   * change this default value if needed to anyone?
   */
  ...
  /**
   * if this is public why we have a setter? TODO: remove public modifier and add getter
   */
  ...
  /**
   * Class constructor specifying the physics world from where collision geometry is taken.
   * 
   * <p>
  ...
  /**
   * Class constructor specifying the physics world from where collision geometry is taken, and
   * size of FBO used for intermediate rendering.
   * 
   ...
   * 
   * <p>
   * Same as calling: {@code setCombinedMatrix(
   *                camera.combined,
   *                camera.position.x,
   *                camera.position.y,
   ...
   * 
   * <p>
   * Matrix must be set to work in box2d coordinates, it will be copied and used for culling and
   * rendering. Remember to update it if camera changes. This will work with rotated cameras.
   * 
   * <p>
   ...
   * @param viewPortWidth  NOTE!! use actual size, remember to multiple with zoom value if pulled
   *                       from OrthoCamera
   * @param viewPortHeight NOTE!! use actual size, remember to multiple with zoom value if pulled
   *                       from OrthoCamera
   * 
   ...
   * 
   * @return true if camera screen intersects or contains provided light, represented by
   *         circle/box area
   */
   ...
   * 
   * <p>
   * <b>NOTE!</b> Remember to set combined matrix before this method.
   * 
   * <p>
   * Don't call this inside of any begin/end statements. Call this method after you have rendered
   * background but before UI. Box2d bodies can be rendered before or after depending how you want
   ...
   *
   * <p>
   * You should need to use this method only if you want to render lights on a frame buffer
   * object. Use {@link #render()} otherwise.
   *
   * <p>
   * <b>NOTE!</b> Don't call this inside of any begin/end statements.
   *
   * @see #renderOnly()
   * @see #render()
   */
   ...
   *
   * <p>
   * <b>NOTE!</b> Remember to set combined matrix and update lights before using this method
   * manually.
   *
   * <p>
   * Don't call this inside of any begin/end statements. Call this method after you have rendered
  ...
  /**
   * Manual rendering method for all lights tha can be used inside of begin/end statements
   *
   * <p>
   * Use this method if you want to render lights in a frame buffer object. You must call
   * {@link #prepareRender()} before calling this method. Also, {@link #prepareRender()} must not
   * be inside of any begin/end statements
   *
   * @see #prepareRender()
   */
   ...
   * 
   * <p>
   * This make lights much more softer and realistic look but cost some precious shader time. With
   * default FBO size on android cost around 1ms.
   * 
   * <p>
   * Default = true
   * 
   ...
   * 
   * <p>
   * Blurring can be pretty heavy weight operation, 1-3 should be safe. Setting this to 0 is the
   ...
   * Default = Color(0, 0, 0, 0)
   * 
   * @param ambientLightColor color whose RGB components specify the shadows coloring and alpha
   *                          specify shadows brightness
   * 
   * @see #setAmbientLight(float)
   * @see #setAmbientLight(float, float, float, float)
   */
   ...
   * 
   * <p>
   * NOTE: To match the visuals with gamma uncorrected lights the light distance parameters is
   * modified implicitly.
   */
   ...
   * 
   * <p>
   * If set to true lights are blended using the diffuse shader. This is more realistic model than
   * normally used as it preserve colors but might look bit darker and also it might improve
   ...
   * <p>
   * Note: you will be responsible for update of viewport via this method in case of any changes
   * (on resize)
   */
   ...
   * 
   * <p>
   * If set to false user needs to use the {@link #getLightMapTexture()} and render that or use it
   * as a light map when rendering. Example shader for spriteBatch is given. This is faster way to
   * do if there is not that much overdrawing or if just couple object need light/shadows.
   * 
   * <p>
   * Default = true
   */
   ...
   * 
   * @return Texture that contain lightmap texture that can be used as light texture in your
   *         shaders
   */
