 ...
 * <p>
 * The preferred height of a text field is the height of the {@link TextFieldStyle#font} and
 * {@link TextFieldStyle#background}. The preferred width of a text field is 150, a relatively
 * arbitrary size.
 * <p>
 * The text field will copy the currently selected text when ctrl+c is pressed, and paste any
 * text in the clipboard when ctrl+v is pressed. Clipboard functionality is provided via the
 * {@link Clipboard} interface. Currently there are two standard implementations, one for the
 * desktop and one for Android. The Android clipboard is a stub, as copy & pasting on Android is
 * not supported yet.
 * <p>
 * The text field allows you to specify an {@link OnscreenKeyboard} for displaying a
 * softkeyboard and piping all key events generated by the keyboard to the text field. There are
 * two standard implementations, one for the desktop and one for Android. The desktop keyboard
 * is a stub, as a softkeyboard is not needed on the desktop. The Android
...
public class TextField extends Widget implements Disableable{
  ...
  public TextField(String string,CodeTextFieldStyle codeTextFieldStyle,RectF rectF,GetFloat object) {}
  ...
  /**
   * When false, text set by {@link #setText(String)} may contain characters not in the font, a
   * space will be displayed instead. When true (the default), characters not in the font are
   * stripped by setText. Characters not in the font are always stripped when typed or pasted.
   */
   ...
   * {@link #setStyle(TextFieldStyle)} is called.
   */
  ...
  /**
   * Copies the contents of this TextField to the {@link Clipboard} implementation set on this
   * TextField.
   */
  public void copy() {
  ...
  /**
   * Sets the {@link Stage#setKeyboardFocus(Actor) keyboard focus} to the next TextField. If no
   * next text field is found, the onscreen keyboard is hidden. Does nothing if the text field is
   * not in a stage.
   * 
   * @param up If true, the text field with the same or next smallest y coordinate is found, else
   *           the next highest.
   */
  ...
  /**
   * If false, methods that change the text will not fire {@link ChangeEvent}, the event will be
  ...
  /**
   * Sets the password character for the text field. The character must be present in the
   * {@link BitmapFont}. Default is 149 (bullet).
   */
  ...
  /**
   * An interface for onscreen keyboards. Can invoke the default keyboard or render your own
   * keyboard!
   * 
   * @author mzechner
   */
   ...
   * {@link Input#setOnscreenKeyboardVisible(boolean)} as appropriate. Might overlap your actual
   * rendering, so use with care!
   * 
   * @author mzechner
   */
  ...
  public class TextFieldClickListener extends ClickListener{
    ...
    /**
     * Checks if focus traversal should be triggered. The default implementation uses
     * {@link TextField#focusTraversal} and the typed character, depending on the OS.
     * 
     * @param character The character that triggered a possible focus traversal.
     * @return true if the focus should change to the {@link TextField#next(boolean) next} input
     *         field.
     */
