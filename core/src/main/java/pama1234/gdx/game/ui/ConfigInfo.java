...
public class ConfigInfo extends Entity<Screen0003>{
  public boolean active;
  public ConfigInfo(Screen0003 p) {
    super(p);
  }
  @Override
  ...
  public void display() {
    ...
    p.text("视野距离   "+p.viewDist,tx+p.pus,ty+p.pus+p.pu*2);
    p.text("位置缩放   "+p.multDist,tx+p.pus,ty+p.pus+p.pu*3);
    p.text(p.serverInfo.toString(),tx+p.pus,ty+p.pus+p.pu*4);
    p.endBlend();
  }
}
