...
public class TextureTool{
  ...
  public static TextureRegion[] getLoop(String l,int m,String ed) {
    TextureRegion[] tmp=new TextureRegion[m];
    for(int i=0;i<m;++i) {
      tmp[i]=new TextureRegion(new Texture(l+m+ed));
    }
    return tmp;
  }
  public static TextureRegion[] getLoop(TextureAtlas atlas,String l,int m) {
    TextureRegion[] tmp=new TextureRegion[m];
    for(int i=0;i<m;++i) {
      tmp[i]=atlas.findRegion(l+i);
    }
    return tmp;
  }
  public static TextureRegion[] getLoop(TextureAtlas atlas,String l,int m,String e) {
    TextureRegion[] tmp=new TextureRegion[m];
    for(int i=0;i<m;++i) {
      tmp[i]=atlas.findRegion(l+i+e);
    }
    return tmp;
  }
}
