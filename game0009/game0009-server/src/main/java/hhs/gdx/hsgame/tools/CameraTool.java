...
public class CameraTool{
  ...
  public static Rectangle getCameraRect(OrthographicCamera cam) {
    float cw=getCamWidth(cam)/2;
    float ch=getCamHeight(cam)/2;
    return camRect.set(cam.position.x-cw,cam.position.y-ch,cw*2,ch*2);
  }
  public static float getCamWidth(OrthographicCamera cam) {
    return cam.viewportWidth*cam.zoom;
  }
  public static float getCamHeight(OrthographicCamera cam) {
