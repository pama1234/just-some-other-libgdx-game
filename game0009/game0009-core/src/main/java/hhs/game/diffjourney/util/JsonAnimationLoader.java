...
public class JsonAnimationLoader{
  ...
  public static AnimationSet<Character.State,TextureRegion> getAnimationSet(
    ...
    XmlReader.Element stop=xml.getChildByName("stop");
    t=Resource.asset.get(stop.get("texture"));
    set.set(Character.State.stop,AnimationBuilder.createAnim(t,stop.getFloat("time",.1f),
      stop.getInt("rows",1),stop.getInt("columns",1)));
